...
| `preflight_missing` | `score` found a leg directory without `preflight.json` | re-run without `--skip-preflight`, or copy the PROV-01 artifact into each `legs/<stack_id>/` before scoring. |
| `leg_outcome_unreadable` | `leg_outcome.json` exists but is unreadable or not JSON | delete or rewrite the latch; a torn file is treated as a terminal refusal, not as absence. |
