...
if [ -n "${OCIR_TEST_PUT_FAIL_VALUE:-}" ] && [ "$value" = "$OCIR_TEST_PUT_FAIL_VALUE" ]; then
    printf 'simulated Vault write failure for %s\n' "$secret_name" >&2
    exit 75
fi
EOF
...
if [ -f "${config_dir}/config.json" ]; then
    cp "${config_dir}/config.json" "$OCIR_TEST_DOCKER_INPUT_CONFIG_LOG"
fi
if [ "${OCIR_TEST_DOCKER_SLEEP_AT:-}" = "$count" ]; then
    sleep "${OCIR_TEST_DOCKER_SLEEP_SECONDS:-3}"
fi
mkdir -p "$config_dir"
...
docker_input_config_log="${fixture_dir}/docker-input-config.log"
...
reset_case() {
    : >"$put_log"
    : >"$docker_log"
    : >"$docker_input_config_log"
    : >"$ssh_log"
    rm -f "$docker_count"
    mkdir -p "$(dirname "$home_config")"
    printf '%s\n' "$pristine_home_config" >"$home_config"
    unset ACX_VAULT_FETCH_TIMEOUT OCIR_TEST_DOCKER_FAIL_AT \
        OCIR_TEST_DOCKER_SLEEP_AT OCIR_TEST_DOCKER_SLEEP_SECONDS \
        OCIR_TEST_PUT_FAIL_VALUE OCIR_TEST_NEW_USERNAME OCIR_TEST_SSH_MODE \
...
run_rotate() {
    ...
    (
        ...
        export OCIR_TEST_DOCKER_INPUT_CONFIG_LOG="$docker_input_config_log"
        ...
        export OCIR_TEST_DOCKER_SLEEP_AT="${OCIR_TEST_DOCKER_SLEEP_AT:-}"
        export OCIR_TEST_DOCKER_SLEEP_SECONDS="${OCIR_TEST_DOCKER_SLEEP_SECONDS:-3}"
        export OCIR_TEST_PUT_FAIL_VALUE="${OCIR_TEST_PUT_FAIL_VALUE:-}"
        ...
        printf '%s' "$token" | bash "$rotate_script" --stdin "$@" \
            3<<<"${OCIR_TEST_NEW_USERNAME:-$known_user}"
...
reset_case
run_rotate "$known_token" --set-username --readable-timeout 0
...
assert_eq "fresh proof exposes no credential material in argv" \
    'login iad.ocir.io' \
    "$(sed -n '1s/^[^|]*|\([^|]*\)|.*$/\1/p' "$docker_log")"
assert_eq "fresh proof exposes no credential material on stdin" '' \
    "$(sed -n '1s/^[^|]*|[^|]*|\([^|]*\)|.*$/\1/p' "$docker_log")"
expected_proof_auth="$(printf '%s:%s' "$known_user" "$known_token" | base64 | tr -d '\n')"
assert_contains "fresh proof reads credentials from its private ephemeral config" \
    "\"auth\": \"${expected_proof_auth}\"" "$docker_input_config_log"
...
assert_eq "Vault username is kept out of proof-login argv" \
    'login iad.ocir.io' \
    "$(sed -n '1s/^[^|]*|\([^|]*\)|.*$/\1/p' "$docker_log")"
...
# The first registry proof is subject to the operator-selected watchdog too.
reset_case
ACX_VAULT_FETCH_TIMEOUT=1
OCIR_TEST_DOCKER_SLEEP_AT=1
run_rotate "$known_token" --set-username --skip-verify
if [ "$rotate_rc" -ne 0 ]; then
    echo "ok   slow fresh-token OCIR proof is bounded"
else
    echo "FAIL slow fresh-token OCIR proof is bounded: exit 0"
    failures=$((failures + 1))
fi
assert_contains "fresh-proof timeout is OCIR-classified" \
    '[ocir_unreachable]' "$rotate_stderr"
assert_contains "fresh-proof timeout identifies the bounded OCIR leg" \
    'acx-timeout:ocir' "$rotate_stderr"
assert_no_file "timed-out fresh proof leaves Vault untouched" "$put_log"
...
run_rotate 'bad-token' --set-username
if [ "$rotate_rc" -ne 0 ]; then
...
# A failed second half of the pair update is retried idempotently, then the
# first half is compensated with the prior token rather than leaving a mixed
# old-username/new-token credential in Vault.
reset_case
new_token='ROTATE_SENTINEL_replacement-token'
new_username='tenant/replacement@example.test'
OCIR_TEST_NEW_USERNAME="$new_username"
OCIR_TEST_PUT_FAIL_VALUE="$new_username"
run_rotate "$new_token" --set-username --readable-timeout 0 --skip-verify
if [ "$rotate_rc" -ne 0 ]; then
    echo "ok   failed username commit fails the rotation"
else
    echo "FAIL failed username commit exits zero"
    failures=$((failures + 1))
fi
assert_eq "username commit is replayed before compensating the token" \
    "OCIR_AUTH_TOKEN|0|${new_token}
OCIR_USERNAME|0|${new_username}
OCIR_USERNAME|0|${new_username}
OCIR_USERNAME|0|${new_username}
OCIR_AUTH_TOKEN|0|${known_token}" "$(cat "$put_log")"
assert_contains "compensation reports restoration of the prior pair" \
    'previous OCIR token restored' "$rotate_stderr"
...
reset_case
run_rotate "$known_token" --set-username --readable-timeout 2.5 --skip-verify
...
for valid_timeout in .5 5.; do
    reset_case
    run_rotate "$known_token" --set-username --readable-timeout "$valid_timeout" --skip-verify
...
run_rotate "$known_token" --set-username
if [ "$rotate_rc" -ne 0 ]; then
...
(
    ...
    printf '%s' "$known_token" | bash -x "$rotate_script" --stdin \
        --set-username --skip-verify 3<<<"$known_user"
