...
- **Throughput is excellent: ~0.58 s/image** (plan assumed ~5 s — 9× better). Inference
...
| Boot volume | disk throughput | model_load | warm-start p95 | ~$/mo | verdict |
| --- | --- | --- | --- | --- | --- |
...
- **"Big Balanced volume" does NOT beat high-VPU** (disproven): 750 GB Balanced = 212 MB/s
  < 400 GB @ 60 VPU = 264 MB/s. Balanced is not cheaper-per-MB/s at scale.
...
- **Relaxed target (recommended) →** `400 @ 30 VPU` or `750 Balanced` (~$31-32/mo,
  ~106-111 s). Justified because **warm-start is NOT user-facing** — the
...
- **Golden image** `acx-gpu-qwen3vl30b-golden` =
  `ocid1.image.oc1.iad.aaaaaaaadgio4ul74nehyiqv2vtgjdvk76mzfatxlbhootf6gmuwurgoox6a`
  (AVAILABLE). Reprovision a serving A10 in ~2 min from it — no re-bake.
- **NSG** `acx-gpu-bench-nsg`
  (`ocid1.networksecuritygroup.oc1.iad.aaaaaaaa3suxv4t4g743ke6lnfu334ohcsh2ixdpo3bfgzfkylzvwwr3k2ca`)
...
- Bake cloud-init: `infra/oci/gpu-bake-cloud-init.yaml` (resilient curl download, serial
  console heartbeat, self-STOP on completion). Full OCID map: `infra/oci/INFRA-TOPOLOGY.md`
  + `infra/oci/acx-oci.env` (on `feature/secoff-1`).
