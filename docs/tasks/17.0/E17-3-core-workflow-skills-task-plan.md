...
|----------|-------|------------------|-----------------|----------------------|--------------|
| `.claude/settings.json` PostToolUse | repo-wide | Existing hooks: `review_findings` → `ace-detect.py`; `get_handoff_state`/`load_session` → `slim-handoff-response.py`; `Bash` → `filter-test-output.py` | Add new PostToolUse hook matching `record_event\|review_findings\|review_runs\|set_handoff_state\|update_task_status` → regeneration script | No conflict — new matcher is additive; `review_findings` already has a hook, new hook adds a second handler | Manual: verify both hooks fire on `review_findings`; `DASHBOARD.md` updates after covered writes while `CURRENT_TASK.json` remains on-demand |
...
- Manual verification:
  ...
  - PostToolUse hook regenerates DASHBOARD.md only on writes; CURRENT_TASK.json timestamp must NOT change; hook skips on read ops (`list`, `get`, `coverage`) — verify via timestamp checks
...
- Create `.claude/skills/handoff-lifecycle/SKILL.md`:
  ...
  - Documents: `switch_task` as safe task-transition entry point; `archive_task_state` must only run after `update_task_status(done)`; DASHBOARD.md and CURRENT_TASK.json are generated views — never hand-edit
...
**Goal**: Wire a PostToolUse hook in `.claude/settings.json` that keeps `DASHBOARD.md` current after covered state-changing MCP writes while reserving `CURRENT_TASK.json` for explicit on-demand regeneration.
...
- Manual: trigger `record_event` write; confirm DASHBOARD.md timestamp updates but CURRENT_TASK.json timestamp does NOT change; confirm a `review_findings(operation="list")` call also does NOT update DASHBOARD.md
...
- [x] Verify hook fires after `record_event` — `DASHBOARD.md` updates while `CURRENT_TASK.json` remains unchanged unless explicitly regenerated
...
- [x] PostToolUse hook regenerates `DASHBOARD.md` after any covered state-changing write; `CURRENT_TASK.json` remains on-demand only
