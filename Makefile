...
# invocation). The ratchet freezes that debt in config/lint-baseline/*.json and
# fails on any movement away from it: a new violation or a raised count is a
# REGRESSION, a disabled rule or widened ignore list is a SUPPRESSION (sr-001),
# and a *paid-down* count is STALE until recorded. Failing on improvement is
# deliberate — canon OBS-11 requires the bar to ratchet from the best
# demonstrated result rather than track whatever the tree happens to look like.
lint-ratchet:
	@python3 scripts/lint_ratchet.py --tool all

# Record paid-down debt. Refuses to raise any count or weaken any config guard,
# so it can only tighten the bar. Commit the updated baseline.
lint-ratchet-accept:
	@python3 scripts/lint_ratchet.py --tool $(or $(TOOL),all) --accept
...
test-deploy-contract:
	@python3 -m pytest scripts/test_e15_31_admin_deploy_contract.py scripts/test_e15_33_deploy_convergence.py scripts/test_e15_33_boot_smoke.py scripts/test_deploy_workflow_gate.py scripts/test_ocirv1_vault_readiness.py scripts/deploy/tests/test_gpu_lifecycle_install.py scripts/deploy/tests/test_gpu_lifecycle_deploy_wiring.py -q --tb=short
