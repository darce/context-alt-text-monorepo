...
| E21-5 | Owns CTA hierarchy between "Analyze selected" and "Describe with AI" in the media footer (one primary per screen state). Also owns the `DebugMetricsPanel` dev-flag gate; the hard-examples "coming soon" chip is **omitted** until its review contract lands, and the undo announcement is an **inline live region**, not a toast (amendments recorded in the E21-5 plan, §6 and §3). Acceptance includes the state matrix (loading/empty/error/offline × focus + announcement — [A11Y-24]) and the undo announcement ([A11Y-21]). |
