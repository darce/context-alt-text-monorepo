"""Daemon startup helpers (E17-10 Slice 4 follow-up).
...
_LOGGER = logging.getLogger("agent_orchestrator_mcp.daemon_startup")
...
def emit_daemon_startup_warning(daemon_kind: str, *, poll_interval: int) -> None:
    ...
    _LOGGER.warning(
        "agent-orchestrator-mcp: %s daemon enabled (poll_interval=%ds, %s).%s "
