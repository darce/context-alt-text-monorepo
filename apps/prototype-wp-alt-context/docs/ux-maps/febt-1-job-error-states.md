...
**Source fixture:** `workbench-operator-loop`
...
| `error-action` | Recovery by AppError tag — Retry: http, parse, nonce_refresh, transport, timeout and unknown; Reload page: auth_expired only; no banner or action: abort; Wait countdown then auto-retry: http cooldown. Retry is the screen's one static primary; Reload and Wait are subordinate, and only the tag-appropriate recovery renders. | form | error, offline, degraded |
...
|   [secondary] Reload page -> reload-page                   |
