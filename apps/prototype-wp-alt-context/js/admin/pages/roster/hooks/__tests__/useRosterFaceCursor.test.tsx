...
describe('useRosterFaceCursor', () => {
  it('returns a visible face on the first paint after prune or unmatched deep-link', () => {
    ...
    const { rerender } = render(
      <Probe visibleIds={['cluster-a:identity-1', 'cluster-a:identity-2']} requestedId="cluster-a:identity-2" seen={seen} />,
    );
    ...
    rerender(
      <Probe visibleIds={['cluster-a:identity-1']} requestedId="cluster-a:identity-2" seen={seen} />,
    );
    ...
    seen.length = 0;
    rerender(
      <Probe visibleIds={['cluster-a:identity-1']} requestedId="missing-face" seen={seen} />,
    );
