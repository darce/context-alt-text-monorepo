...
4. **Offline sovereignty for display.** Rendering media, computed clusters, and inferred captions reads **only** from the WP sovereign mirror and MUST NOT depend on remote reachability. Remote calls are required solely to compute *new* recognition/inference. A down remote surface disables "analyze new," never "view existing."
   - **Consistency model (stated).** The sovereign mirror is an **eventually-consistent read model** of remotely-computed recognition/captions, refreshed via the existing sync/outbox path (ADR-003 durable outbox, ADR-004 sync completion). Display tolerates stale reads: a just-computed result becomes visible after the next sync, not synchronously. New inference is the only remote-synchronous operation.
   - **Slow-failure resilience.** Because remote-only makes the single shared backend a hard dependency for new inference, every remote call (whoami, health, analyze) MUST have a **bounded timeout** and **fast-fail** — a *slow* backend must degrade to "existing data shown, new analysis unavailable," never hang the WP admin. The existing recognition **circuit breaker** (`class-recognition-circuit-keys.php`) is the mechanism; remote-only must keep it in the path.
