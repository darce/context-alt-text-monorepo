...
> **Metadata**
>
> - **Date**: 2026-07-17 (r3 — r2 revised `UXP-3-PR-01..15` after run `-r1` (fail); r3 folds run `-r2` conditions `UXP-3-PR-16..20` (conditional_pass): option-value namespacing, `selectClusterSuggestions` pipeline, pre-dedupe duplicate guard, 0b-1/0b-2 co-ship, optimistic cache shape)
...
- **Shared option builder** `buildNamingOptions(rosterEntries, labelMatches)` (new, beside the panel): each option is `{ value, label, source: 'person' | 'cluster' }` with **values namespaced by source** — `person:<roster-id>` / `cluster:<cluster-id>` — so an option value can never be mistaken for a bare cluster id (PR-16). Display: deduped case-insensitively **preferring the person entry** (its casing is canonical), persons listed before clusters ([COG-02] — recognition over recall; [PERC-02] — badge encodes the real behavioral difference: person = names an entity, cluster = merges into an existing cluster). Filter-before-slice: the union is prefix-filtered, then truncated. The builder also returns the **pre-dedupe collision set** (see duplicate guard).
...
- [ ] 0b-1+0b-2 co-shipped (PR-19): inline depth 5 + predicate + projection key; dropdown single-id batch at same depth; `identityFor` deleted; no consumer left on `fetchIdentitySuggestions`; `cluster-*` change pinned by test; route-retirement follow-up recorded for UXP-2 absorption, not implemented.
...
- [ ] Pre-save duplicate guard over the **pre-dedupe universe** with explicit merge/rename/cancel (person+cluster same-name fixture — PR-18); dead 409 `duplicateMatch` branch deleted.
