...
> **Cold-start dispatcher for coding agents.** Full protocol: [docs/workstate/instructions.md](docs/workstate/instructions.md).
...
2. **Apply the [MCP Loading Protocol](docs/workstate/rules/mcp-loading-protocol.md)** before fetching state. Read [`docs/workstate/maps/mcp-tool-routing.yaml`](docs/workstate/maps/mcp-tool-routing.yaml) and surface only the MCP servers whose triggers match the current prompt / task scope. `workstate-handoff-mcp` is always loaded; `workstate-orchestrator-mcp`, `context7`, and `computer-use` are on-demand. Concretely on Claude Code: `ToolSearch select:mcp__<server>__*` to surface a deferred server when its triggers fire.
...
If MCP tool calls unavailable (ToolSearch returns nothing for `mcp__workstate-handoff-mcp__*`), or if you need a query the MCP tools don't directly expose: **use the Python API via Bash as the primary fallback.**
...
| ------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------- |
| **Backend (Python)**            | [maps/backend.md](docs/workstate/maps/backend.md)                | [rules/backend-python-guidelines.md](docs/workstate/rules/backend-python-guidelines.md) | [rules/testing-python.md](docs/workstate/rules/testing-python.md)         | [maps/tech-stack.md#backend-python](docs/workstate/maps/tech-stack.md#backend-python)     | `apps/prototype-description-service/` |
| **Frontend (React/TS)**         | [maps/frontend.md](docs/workstate/maps/frontend.md)              | [rules/frontend-guidelines.md](docs/workstate/rules/frontend-guidelines.md)             | [rules/testing-typescript.md](docs/workstate/rules/testing-typescript.md) | [maps/tech-stack.md#frontend-reactts](docs/workstate/maps/tech-stack.md#frontend-reactts) | `apps/prototype-wp-alt-context/js/`   |
| **PHP Plugin**                  | [maps/php-plugin.md](docs/workstate/maps/php-plugin.md)          | [rules/backend-php-guidelines.md](docs/workstate/rules/backend-php-guidelines.md)       | [rules/testing-php.md](docs/workstate/rules/testing-php.md)               | [maps/tech-stack.md#php-plugin](docs/workstate/maps/tech-stack.md#php-plugin)             | `apps/prototype-wp-alt-context/src/`  |
| **Cross-Service Integration**   | [maps/integration.md](docs/workstate/maps/integration.md)        | [contracts/](docs/workstate/contracts/)                                                 | [rules/testing-principles.md](docs/workstate/rules/testing-principles.md) | [maps/tech-stack.md#orchestration](docs/workstate/maps/tech-stack.md#orchestration)       | `docs/workstate/contracts/`             |
| **Infrastructure / Deployment** | [self-hosting-epic.md](docs/epics/v0.3.1/self-hosting-epic.md) | [development-workflow.md](docs/workstate/rules/development-workflow.md)                 | [testing-principles.md](docs/workstate/rules/testing-principles.md)       | [maps/tech-stack.md#orchestration](docs/workstate/maps/tech-stack.md#orchestration)       | `infra/oci/`                          |

Additional routing: see [docs/workstate/instructions.md](docs/workstate/instructions.md#additional-routing).
...
Full surface table: [instructions.md § Output Brevity](docs/workstate/instructions.md#output-brevity-mandatory).
...
See [development-workflow.md § Pre-Merge Gate](docs/workstate/rules/development-workflow.md#pre-merge-gate-mandatory) for the full pre-merge sequence and recovery steps.
...
See [development-workflow.md](docs/workstate/rules/development-workflow.md#branch-isolation-protocol-mandatory) for isolation tiers, worktree recovery, and rationale.
...
Treat the standalone `darce/mcp-workstate-handoff` repo as out of bounds unless the workspace is opened there directly. `workstate-handoff-mcp` no longer lives in this monorepo checkout.
...
/tmp/e17-13-external-mcp/bin/pip install --quiet \
    "git+ssh://git@github.com/darce/mcp-workstate-handoff.git@v0.4.3" \
    "git+ssh://git@github.com/darce/mcp-workstate-orchestrator.git@v0.1.4"
...
See [docs/workstate/rules/testing-python.md § External MCP Package Verification](docs/workstate/rules/testing-python.md#external-mcp-package-verification-e17-13) for the live convention used by this cleanup task.
...
See [docs/workstate/rules/testing-python.md § Commit SHA Provenance Discipline](docs/workstate/rules/testing-python.md#commit-sha-provenance-discipline-mandatory) for the full rationale and the AHMCP-10/AHMCP-11 audit-trail bug that motivated the enforcement.
...
See [docs/workstate/rules/development-workflow.md § Branch Isolation](docs/workstate/rules/development-workflow.md#branch-isolation-protocol-mandatory) for the full rationale.

### Review Findings Placement

> **Review findings live in workstate-handoff-mcp, not in task plans. Never paste a finding list into a markdown file.**
...
See [docs/workstate/rules/branch-review-guide.md § Review Findings Placement](docs/workstate/rules/branch-review-guide.md#review-findings-placement-mandatory) for the full rationale and the AHMCP-14 incident that motivated the enforcement.
...
See the installed `workstate-handoff-mcp` package documentation for the full set of bounded-read levers and example call patterns.
...
- Slice completion format (enforced at write time): [docs/workstate/templates/slice-complete-template.md](docs/workstate/templates/slice-complete-template.md).
...
- Full handoff protocol: [docs/workstate/instructions.md](docs/workstate/instructions.md#mcp-handoff-contract-mandatory).
...
Derived copy from the canonical source: [docs/workstate/constitution.md](docs/workstate/constitution.md#short-rules). Edit rules there first, then sync this injected surface in the same slice.
...
Derived copy from the canonical source: [docs/workstate/constitution.md](docs/workstate/constitution.md#cross-branch-regression-guards). Edit guards there first, then sync this injected surface in the same slice.
...
- [rg-013] helpful=1 harmful=0 :: `workstate_handoff_mcp/core.py` must remain pure handoff-state CRUD. No orchestration imports, no subprocess calls, no lock management. Scope: the standalone `workstate-handoff-mcp` package available to the active workspace. Enforce during code review.
- [rg-014] helpful=1 harmful=0 :: `workstate_orchestrator_mcp` modules must use late-binding imports (function-level) for `workstate_handoff_mcp` symbols to preserve the clean split seam and avoid load-time coupling. Scope: the standalone `workstate-orchestrator-mcp` package available to the active workspace.
...
**Branch review**: route matching requests through the `branch-review` skill first (`.claude/skills/branch-review/SKILL.md` or `/branch-review`). Use [rules/branch-review-guide.md](docs/workstate/rules/branch-review-guide.md) as the detailed checklist/reference surface behind the skill, not as a separate ad-hoc entry point.

**Planning review**: route matching requests through the `planning-review` skill first (`.claude/skills/planning-review/SKILL.md` or `/planning-review`). Use [rules/planning-review-guide.md](docs/workstate/rules/planning-review-guide.md) as the detailed checklist/reference surface behind the skill, and use `plan-analyze` as the required pre-review triage step.
