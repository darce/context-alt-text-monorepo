...
Lease renewal, binding, replacement, and release compare the complete owner snapshot in the database write, so a delayed request cannot overwrite or delete a newer owner. Before backend dispatch the lease records `submission_started`. Local attachment/input validation failures release it; accepted runs whose tracking write failed retain their run ID for status reconciliation. Transport errors or malformed acceptance without a run ID leave a pending lease with `submission_started: true`, which never expires into free capacity. For that exceptional state, an operator must establish that no public run is active on the backend before clearing `acx_public_demo_inflight`; the adapter has no run ID with which to reconcile automatically. Pending leases acquired before dispatch can still be reclaimed after expiry.
