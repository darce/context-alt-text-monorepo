...
² MiniCPM latency inflated by thinking-token generation on failed items; successful items ran ~230–330 s.
³ `openbmb/MiniCPM-V-4_5` is tagged `apache-2.0` on HF — the assessment §13 registration concern does not apply to this release. Moot given disqualification.
...
1. **Wrong-fact signal.** CapRL embellishes with unverifiable specifics: float branding `“Abdul’s Blue Boutique x SUNNYLIFE”` (`maria-pool`), poster text `“RASPBERRY OAT MILK”` attributed to a Pink Panther poster (`ryann-bar`), `“likely a Rottweiler”` (`nina-machiavelli`), and location inference `“likely in Iceland … as noted in the album’s description”` (`mcm-planecrash` — the album description says no such thing). None are gate-detectable; all are exactly the class of plausible-but-unverified detail the product must not emit. Qwen adds no such specifics.
2. **Context-conflicts-pixels behavior** (`mcm-planecrash`, context claims a garden picnic): Qwen anchors on pixels **and explicitly flags the conflict** (“contrasting with the caption’s description of a summer garden picnic”) — the ideal behavior under the anchor-visual/inject-factual contract. CapRL anchors on pixels but half-launders its inference through the context.
...
- Highest RSS (11.2 GB) and slowest cold load (73 s); both attempts' run records are preserved for audit (`MiniCPM-V-4.5-run-record` is the `/no_think` attempt; the thinking-mode attempt is retained in `scripts/eval_harness/out/`, git-ignored).
