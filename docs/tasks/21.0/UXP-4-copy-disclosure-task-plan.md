...
**1. The "single vocabulary source" is not single.** `SYNC_VOCABULARY` (`js/admin/pages/workbench/syncVocabulary.ts`) is the declared canonical copy module (its header says so), yet `degradedModeBannerLogic.ts:57–61` re-declares `'Working offline'`, `'Sync attention needed'`, and `'Showing your local copy; changes will sync when the service returns.'` as inline `__()` literals — byte-duplicates of `SYNC_VOCABULARY.offlineBannerTitle` / `.attentionBannerTitle` / `.offlineDetail` (lines 23–25). One copy edit now fans out to two modules or silently diverges ([REF-19]; UXG-15's lockstep risk realized).
...
- **Collision policy (mandatory)**: E15-37-FE, E21-9, and E21-10 touch overlapping workbench/roster surfaces and merge separately. UXP-4 is therefore **string-level and rebase-tolerant**: edits change `__()` literals, vocabulary keys, and copy constants; the one permitted structural change is the help-card → disclosure swap inside `ConfirmTabContent.tsx` (slice 4), a file none of those tasks claims. No JSX restructuring of `SyncStatusIndicator`, `DashboardPage` stat grids, roster tables, or media-table components. No tab/route id changes (`rosterRoute.ts` **ids** stay `entries`/`clusters`; only `label` strings may change) — E21-9 owns the Clusters-tab retirement.
...
- **E21-1 banned-strings discipline**: additions to `BANNED_STRINGS` must be exact operator-visible phrases ("Delta sync", "Machine sync", "Sync backlog", "Retention posture", "Managed Identities") — never bare common words ("Queued", "projection"-substring traps) that legitimate job-phase badges or code identifiers would trip.
...
**Broken or drifting:** the banner logic duplicates three vocabulary strings; six-plus vocabulary entries carry ops dialect onto primary surfaces; the retention card renders a dead jargon-titled state; the drawer's explainer is unconditional and its zero state shows "Pending" with no job; roster says "Entries", "Clusters", and "Managed Identities" (`RosterEntriesSection.tsx:169`) for overlapping concepts — three names, two referents ([WRIT-06]).

**Deliberately untouched:** `phaseQueued`/`describeQueued` ("Queued" as a *job phase* badge) — a queued job is plain language, not sync topology; the scope's "Queued becomes internal-only" applies to the sync-strip queued state (`queuedBadge`), whose replacement copy must name the consequence instead. Recorded here so review argues with the decision, not the omission.
...
**Copy specification.** Rewrites below are the deliverable strings. Reviewers challenge them here, at plan review — not mid-lane.
...
| retention unavailable body | "Retention status is unavailable right now." | endpoint configured but errored: "Retention status could not load. Check the connection on the Settings page." + existing `#/retention` link retained; endpoint **not configured** (`available:false`): card not rendered |
...
| banner | `pages/workbench/degradedModeBannerLogic.ts` : `getDegradedBannerTitle`, `getDegradedBannerMessage` | Replace inline literals with `SYNC_VOCABULARY` reads |
...
- Manual (one pass, post-slices, not mid-slice): Workbench strip through healthy/queued/offline states, dashboard with and without a configured retention endpoint, drawer open/close with keyboard.

## Slice Delivery

### Slice 1: Single-source the banner (structure only, zero copy change)

**Goal**: `SYNC_VOCABULARY` is provably the only declaration site for banner strings.

Changes: `getDegradedBannerTitle` returns `SYNC_VOCABULARY.offlineBannerTitle` / `.attentionBannerTitle`; `getDegradedBannerMessage` returns `SYNC_VOCABULARY.offlineDetail`; the three inline `__()` literals are deleted. Rendered output is byte-identical.

**TEST_CMD**: `npm --prefix apps/prototype-wp-alt-context test -- degradedModeBannerLogic DegradedModeBanner`

Proof: new unit asserts `getDegradedBannerTitle(offlineHealth) === SYNC_VOCABULARY.offlineBannerTitle` (and the two siblings) by identity against the module export — red against the pre-slice tree because the literals are distinct string instances only coincidentally equal; mutating a vocabulary entry must flip the banner ([TEST-15] discrimination: temporarily editing `offlineBannerTitle` in a scratch run changes the banner assertion's expected value). Existing `DegradedModeBanner.test.tsx` stays green unmodified (no copy change).
...
Changes: copy spec rows 1–8 (rewrites, `syncBacklog*` → `pendingWorkSummary*` key renames with all consumers — `SyncStatusIndicator.tsx`, `DashboardSyncHealthSection.tsx`; delete `syncModeDelta`/`syncModeFull`, `formatSyncModeLabel`, and the `syncModeDetails` block). Placeholder arity preserved (`%1$d…%4$d`).

**TEST_CMD**: `npm --prefix apps/prototype-wp-alt-context test -- syncVocabulary syncPresentation SyncStatusIndicator DashboardSyncHealthSection banned-vocabulary`

Proof: `BANNED_STRINGS` gains "Delta sync", "Machine sync", "Sync backlog". Targeted fixture tests render `SyncStatusIndicator` with `sync_mode: 'delta'` and `topology_commands: {pending:2, applied:1, failed:1, conflict:1}` and assert no banned phrase and the new counts copy — red pre-change (badge renders "Delta sync" today). A vocabulary unit test asserts no `SYNC_VOCABULARY` value matches `/machine sync|delta sync|sync backlog/i`, so the module itself is swept, not just page fixtures. `phasePresentation.test.ts` stays green (job-phase "Queued" untouched).

### Slice 3: Retention card — operator heading, remediation, no dead card

**Goal**: The dashboard retention panel helps or disappears.

Changes: heading "Your data & retention"; `available:false` → panel not rendered; `available:true` with load error → remediation copy per spec (settings pointer + retained `#/retention` link — the unavailable state keeps a designed action, [RLSE-04]/[A11Y-24]). Strings live in a `retentionCardCopy.ts` copy-constant module (sweep pattern) since `DashboardPage` renders it conditionally.

**TEST_CMD**: `npm --prefix apps/prototype-wp-alt-context test -- DashboardPage banned-vocabulary`

Proof: `BANNED_STRINGS` gains "Retention posture" — red pre-change (the sweep's retention fixture is `available:true`, heading renders). Unit: `available:false` fixture → `Retention` panel absent from the DOM; error fixture → remediation copy + link present. Copy constants swept via `import *` like `personCommitCopy`.
...
Proof: component tests — explainer body absent from the accessible tree until the summary is toggled (red today: always rendered); `ConfirmPanel` with `jobId=null` shows the zero-state line and no "Pending" (red today); with a job id, `Status` row renders as before. New/updated copy exported as constants and added to the sweep's constant sweep so page-level mocks can't hide it.
...
- [ ] Loaded frontend/testing rules; confirmed E15-37-FE / E21-9 / E21-10 file overlap is limited to string-level edits per the collision policy.

### Checklist for Slice 1: Single-source the banner

- [ ] Inline banner literals deleted; `getDegradedBannerTitle`/`getDegradedBannerMessage` read `SYNC_VOCABULARY`.
- [ ] Identity assertions bind banner output to vocabulary exports; rendered copy byte-identical.

### Checklist for Slice 2: Sync vocabulary rewrite

- [ ] Copy spec rows applied; `syncBacklog*` keys renamed with all consumers; `syncModeDelta`/`Full` + `formatSyncModeLabel` + mode badge deleted.
- [ ] `BANNED_STRINGS` += "Delta sync", "Machine sync", "Sync backlog"; demonstrated red pre-change under rendering fixtures.
...
- [ ] Heading replaced; `available:false` renders no card; error state carries remediation + `#/retention` link.
...
- [ ] No page surface renders "Delta sync", "Machine sync", "Sync backlog", "Retention posture", or "Managed Identities" — enforced by the banned-vocabulary sweep, each addition proven capable of failing.
- [ ] Every sync-strip and banner state string names consequence + next action, and each such string has exactly one declaration site (`SYNC_VOCABULARY` or a swept copy-constant module).
- [ ] Dashboard with no retention endpoint configured shows no retention card; with a configured-but-failing endpoint it shows remediation with a working link.
- [ ] Advanced drawer opens to recovery tools; the clustering explainer appears only on demand and contains no "embeddings"; keyboard toggling works via native `<details>` semantics.
