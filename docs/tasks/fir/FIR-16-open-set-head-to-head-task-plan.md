...
| tests | `apps/prototype-description-service/recognition/tests/api/test_media_identities_match_score.py` | New (replaces `scene/tests/test_export_service_match_score.py`) — `test_list_by_media_ids_round_trips_match_score`, `test_rejected_face_keeps_best_centroid_score` (D3). |
| tests | `apps/prototype-description-service/recognition/tests/unit/test_centroid_discovery.py` | New/extended cases: all-negative-similarity gallery still returns the true best match; empty gallery returns `(None, None)` (D3). |
| tests | `apps/prototype-description-service/scripts/bench/tests/test_export_map_match_score.py` | New — `test_export_rows_carry_match_score_and_cluster` (D3). |
...
- `score_report.py::score_open_set_report(run_dir: Path | str) -> Path` reads both stacks' `GateSummary`s (S1, D5) and the ratified `GateContract` (FIR-13), calls `gate_contract.select_gate_point(summary.points, max_fpi=contract.max_fpi)` per stack to fill in `GateSummary.selected`/`.tau`, and writes `benchmarks/results/crossbench-<stamp>/open_set.json` + `.md`.
...
- [ ] `MediaIdentity.match_score`/`match_cluster_id` added; captured in `CentroidDiscovery.discover` before the `similarity_threshold` filter (D3); `list_by_media_ids` and `export_service.py` both emit the two fields.
- [ ] `_find_best_centroid_match` fixed: `best_similarity` starts at `float("-inf")`; empty gallery returns `(None, None)` (D3).
...
- [ ] `score.py.score_open_set` returns one `GateSummary` per stack (`points`, `selected`, `tau`, `probe_outcomes`); scores each stack independently, never pooling embedding spaces; `RunReport` itself is unmodified (D5).
...
- [ ] Runbook builds an explicit enrollment-only gallery reproducing frozen `RunPlan` G1 before any probe runs, disables auto-accept during the run, and derives matedness via `_validate_search_against_entry` against that gallery (D18).
...
- [ ] `score_open_set` (returning `GateSummary` per stack, D5), `to_open_set_searches`, `score_open_set_report`, and `propose-gate` exist, are imported by the bench CLI, and pass their test suites.
- [ ] No probe ever joins the frozen enrollment gallery during the S4 live run (D18, leakage test green).
