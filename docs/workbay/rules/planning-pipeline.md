...
For single-package tasks, replace "Owning Epic" with "Project" and "Epic Short ID" with "Task ID". Use the package short name as prefix (e.g. `AHMCP` for `workbay-handoff-mcp`).
...
| Stage | Artifact | Path |
|-------|----------|------|
| Assessment | Output state-keeping report | external `darce/mcp-workbay-handoff` docs |
| Spec | Output contract v2 spec | external `darce/mcp-workbay-handoff` docs |
| ADR | Typed tool surface consolidation | `docs/adrs/ADR-005-workbay-handoff-mcp-typed-tool-surface-consolidation.md` |
| Task plan (Tier 1) | Bounded rendering + mutation output | external `darce/mcp-workbay-handoff` docs |
| Task plan (Tier 2) | Response envelope rollout | external `darce/mcp-workbay-handoff` docs |
| Task plan (design) | Tool surface consolidation ADR | external `darce/mcp-workbay-handoff` docs |
