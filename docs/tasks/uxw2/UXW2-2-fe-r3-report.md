...
| R5-07 | `test(fe): UXW2-2 pin review-queue invalidation header and key proofs` | `R5-07: the queue header count decrements after a full bulk accept` (`ReviewQueue.test.tsx:3277`) — renders `.acx-review-queue__count`; ReviewQueue has no Bulk-accept chrome (`:1104`); drives `mutations.bulkAccept` `{accepted_count:2,skipped_count:0}` on the shared QueryClient and waits for the namePending refetch to drain the span | delete `namePendingKey` invalidate in `bulkAcceptMutation.onSuccess` → `AssertionError: expected <span …(2)></span> to be null` / Received span `2 left to review on this page`. `length >= 0` so the span still renders at zero → `AssertionError: expected <span …(2)></span> to be null` / Received span `0 left to review on this page` | `npx vitest run …/ReviewQueue.test.tsx -t 'R5-07: the queue header count decrements after a full bulk accept'` → `Tests  1 passed \| 93 skipped (94)` |
