...
> **Referenced, not read end-to-end.** This doc is the concept vocabulary for agent skills and rule guides — like the Fowler smell table is for `refactor`. Activation surfaces inline the **cue** (trigger + question); this lexicon holds the **rationale**.
...
**Consumption**: canonical source. Review/planning/reasoning surfaces in consuming repos inline these cues by phase tag and cite rules by ID (`[RES-01]`) — never copy row bodies.
