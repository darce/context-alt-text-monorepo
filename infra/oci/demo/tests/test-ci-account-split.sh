...
assert_file_grep "bootstrap calls converge_wp_ci_account" \
    "$bootstrap_file" 'converge_wp_ci_account "\$WP_CI_USER" "\$WP_CI_PASSWORD" "\$WP_CI_EMAIL" "\$WP_ADMIN_USER" "\$WP_ADMIN_EMAIL"'
...
wpcli() {
    ...
    case "$1 ${2:-}" in
        ...
        "user check-password")
            ...
            ;;
        "user update")
            local arg u="$3" pass="" email=""
            for arg in "$@"; do
                case "$arg" in
                    --user_pass=*) pass="${arg#--user_pass=}" ;;
                    --user_email=*) email="${arg#--user_email=}" ;;
                esac
            done
            if [ -n "$pass" ]; then
            ...
            if [ -n "$email" ]; then
                EMAIL_UPDATE_CALLS=$((EMAIL_UPDATE_CALLS + 1))
                CI_EMAIL_STORE="$email"
            fi
            return 0
            ;;
        *)
...
rc=0
converge_wp_ci_account "acx-demo-admin" "ci-secret" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null 2>/dev/null || rc=$?
assert_eq "CI user equal to admin exits 2" "2" "$rc"
assert_eq "collision does not create a user" "0" "$CREATE_CALLS"

rc=0
converge_wp_ci_account "ACX-DEMO-ADMIN" "ci-secret" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null 2>/dev/null || rc=$?
assert_eq "case-insensitive CI user collision exits 2" "2" "$rc"
assert_eq "case-insensitive collision does not create a user" "0" "$CREATE_CALLS"

rc=0
converge_wp_ci_account "acx-demo-ci" "ci-secret" "Admin@Example.com" "acx-demo-admin" "admin@example.com" >/dev/null 2>/dev/null || rc=$?
assert_eq "CI email colliding with admin email exits 2" "2" "$rc"
assert_eq "email collision does not create a user" "0" "$CREATE_CALLS"

# Missing fields fail loud
rc=0
converge_wp_ci_account "" "ci-secret" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null 2>/dev/null || rc=$?
...
rc=0
CHECK_CALLS=0
converge_wp_ci_account "acx-demo-ci" "ci-secret" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null || rc=$?
...
assert_eq "create path verifies password via check-password" "1" "$CHECK_CALLS"
...
rc=0
EMAIL_UPDATE_CALLS=0
CI_EMAIL_STORE="old-ci@example.com"
converge_wp_ci_account "acx-demo-ci" "ci-secret" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null || rc=$?
...
assert_eq "existing CI user email is converged" "1" "$EMAIL_UPDATE_CALLS"
assert_eq "existing CI user email store matches secrets" "ci@example.com" "$CI_EMAIL_STORE"

# Changed CI secret rotates
UPDATE_CALLS=0
rc=0
converge_wp_ci_account "acx-demo-ci" "ci-rotated" "ci@example.com" "acx-demo-admin" "admin@example.com" >/dev/null || rc=$?
