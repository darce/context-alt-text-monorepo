...
| L3 `febt-1-hooks-sweep` | W2 | `hooks/useJobStateMachine.ts`, `useJobStateMachineMutations.ts`, `useJobStateMachineEffects.ts`, `clusterAutoRetry.ts`, `recognitionJobHistoryUtils.ts` (+tests) | L1, L2 | replaces `isNotFoundError` message sniff with `classifyError(...)._tag === 'http' && status === 404`; console → logger |
| L4 `febt-1-clusters-sweep` | W2 | all `pages/workbench/identity-clusters/*` error sites incl. `useClusterSuggestionsLoader.ts` console.warn | L1, L2 | — |
| L5 `febt-1-api-retention-sweep` | W2 | `api/wpErrorMessage.ts`, `api/describeApi.ts`, `api/recognition/scanApiError.ts`, `pages/retention/useRetentionPageState.ts` | L1 | — |
| L6b `febt-1-stream-reducer` | W2 | `hooks/useJobProgressStream.ts` (+tests; its 5 console sites) | L2, L6a | stream hook drives `jobReducer` |
