...
- Produce `docs/tasks/15.0/E15-5a-hetzner-fallback-plan.md` covering:
  ...
  - Which env vars + secrets move (canonical surface: `apps/prototype-description-service/.env.prod.example`; deployed file on the OCI VM is `/opt/acx-backend/prod/.env`, a regular 0600 file; Vault-held secrets are not in it).
