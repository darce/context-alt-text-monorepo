...
- notes: FIR-11-S2R3-14 identification/caption rebaseline disclosed: against the current golden the detection-refusal wave scored faces.identification.true_rejections=10 (was 6) and caption.must_right_defined_images=37 / must_right_failed_images=34 (was 0 / 0). Those values are the current golden's score of the same run-record — the seed already carried 37 rubric rows. A later S2R3-08 refusal set identification to REFUSED so true_rejections is now null; the caption 37/34 numbers remain. This is a latent re-score drift the refusal wave surfaced, not a number the wave invented.
