...
The orchestrator preserves review commit `59ced8b` (report-only, `docs/workbay/reports/GPUFLOW-1-plan-review.md`) as review evidence, not implementation. Re-review the revised plan with luna max before freezing/materializing implementation lanes.
...
| tests | `apps/prototype-description-service/scene/tests/test_shared_schema_documents.py` (new, owner: contracts) | well-formed JSON Schemas and embedded hand-written success/error examples; no builder imports or execution |
| tests | `apps/prototype-description-service/scene/tests/test_shared_schema_multipart.py` (new, owner: svc-cold-gpu) | multipart success and error envelopes validated against the shared schema using the real response builder |
| reports | `docs/assessments/GPUFLOW-1-rebaseline-20260914.md` (new) | Slice 0 rebaseline: recorded SHA/routes, symptom evidence and live suggestion row |
| reports | `docs/assessments/GPUFLOW-1-wrong-match-and-occlusion-calibration-20260913.md` | calibration-lane output; consumes the rebaseline report read-only |
...
- **Contract checks:**
  - Wave 1 contracts runs `apps/prototype-description-service/scene/tests/test_shared_schema_documents.py` (new): validate each schema as well-formed JSON Schema and validate hand-written success/error envelopes embedded in the test against each schema. This schema/fixture-only test must not import or run any response builder or describe-run contract test.
  - Wave 3 svc-cold-gpu owns and runs `apps/prototype-description-service/scene/tests/test_shared_schema_multipart.py` (new), checking real multipart response-builder success/error envelopes, including `timing` and error `code`, against the contracts-owned multipart schema. Wave 3 svc-run-timing solely owns and runs `apps/prototype-description-service/scene/tests/test_describe_run_contract.py` against the real describe-run builder and contracts-owned run schema. Both lanes consume their schema read-only through the explicit table edges.
...
- Rebaseline lane (luna max): classify persisting defects and record the live Trudeau/Watson suggestion row (similarity, band, representative identity id, quality, occlusion score) in the new `docs/assessments/GPUFLOW-1-rebaseline-20260914.md`. Calibration consumes this artifact read-only in C3 and solely owns `docs/assessments/GPUFLOW-1-wrong-match-and-occlusion-calibration-20260913.md`.

Proof:

- `docs/assessments/GPUFLOW-1-rebaseline-20260914.md` records the image SHA, route status codes, symptom evidence and re-baseline table.
- A handoff decision references the committed rebaseline artifact.
...
- Persist each tenant-scoped operation lease in the shared service database before publishing or responding. A 503 does not release it in `finally`. Retries renew the same operation lease; concurrent operations have separate leases. Every periodic, async and sync publication aggregates unexpired leases with queued/running async GPU work under the existing service writer/fence. Lease aggregation into the global load snapshot MUST use the existing dedicated RLS-bypassed system session: the `load_snapshot` contract counts across tenants and fails closed for tenant-scoped sessions. The demand publisher also persists the first observed ready transition for active startup/operation records on its periodic pass, so readiness between HTTP retries is retained. Serialize database snapshot and file publication so an older snapshot cannot overwrite newer demand; never introduce a lifecycle writer.
- Lease lifetime L must exceed both controller poll period P + worst scheduling jitter J + publication delay D and maximum advertised Retry-After + the client's worst additional retry gap. SPA auto-retry honours Retry-After up to the 120 s public ceiling; a compliant client must retain demand between retries while warming. Refresh interval R + D must remain below load-reader freshness F. The contract producer records deployed P/J/D/F, maximum advertised Retry-After, the bounded client retry gap and an explicit bounded L before dispatch; unavailable bounds block startup implementation. Lease expiry removes abandoned demand; renewal cannot extend lifecycle max-lease or override STOP.
...
- Aggregation tests count tenant A's lease while a tenant-B-scoped session is active, and verify that a tenant-scoped aggregation attempt fails closed.
- A fake-clock client retrying exactly at maximum advertised Retry-After never observes lost demand while warming.
...
In every dependency cell, the prefix names the producer and the row names the read-only consumer. Paths identify the committed artifacts transferred, including existing schema/API modules and new test fixtures that the producer must commit before consumption. New fixture ownership is explicit below; downstream lanes may read but never edit producer tests or fixtures. Operator promotion, P/J/D/F and Retry-After/client retry-gap bounds, and API-R-23 acceptance remain dispatch gates, not artifact-free DAG edges. Assessment and contract-document edges retain their canonical repository paths outside the application trees.

| Lane ID | Slice | Owned Paths | Upstream Dependencies (artifact transferred) | Required Tests |
| --- | --- | --- | --- | --- |
| `rebaseline` | 0 | `docs/assessments/GPUFLOW-1-rebaseline-20260914.md` (new) | — (operator promotion gate) | recorded SHA/routes and symptom evidence |
| `contracts` | A1/B1 | `packages/shared-contracts/schemas/scene-describe-multipart.schema.json` (new), `packages/shared-contracts/schemas/scene-describe-run.schema.json`, `docs/workbay/contracts/gpu-lifecycle.md`; `apps/prototype-description-service/scene/tests/test_shared_schema_documents.py` (new test) | rebaseline: `docs/assessments/GPUFLOW-1-rebaseline-20260914.md` | `python3 -m pytest apps/prototype-description-service/scene/tests/test_shared_schema_documents.py -q -p no:cacheprovider`; well-formed schemas and embedded hand-written success/error examples only; no builder imports or execution; lease/freshness contract review |
...
| `spa-description-service` | A3 | `js/admin/pages/settings/GpuControlCard.tsx`, `js/admin/pages/retention/RetentionPage.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/settings/__tests__/fixtures/gpuflow-service-states.json` (new test fixture) | rebaseline: `docs/assessments/GPUFLOW-1-rebaseline-20260914.md` | npx vitest run js/admin/pages/settings js/admin/pages/retention |
