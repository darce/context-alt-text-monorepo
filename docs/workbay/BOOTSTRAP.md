...
### Core Ledger Server (`workbay-handoff-mcp`)
...
```text
.vscode/mcp.json  →  env { PATH, AGENT_HANDOFF_ENFORCE_BRANCH=1 }  →  uvx "mcp-workbay-handoff==0.2.0" --workspace-root ${workspaceFolder} --state-dir ${workspaceFolder}/.task-state --exports-dir ${workspaceFolder}/.task-state/exports serve-stdio
```

### Orchestration Server (`workbay-orchestrator-mcp`)
...
```text
.vscode/mcp.json  →  env { PATH, AGENT_HANDOFF_ENFORCE_BRANCH=1 }  →  uvx "mcp-workbay-orchestrator==0.2.0" --workspace-root ${workspaceFolder} --state-dir ${workspaceFolder}/.task-state --exports-dir ${workspaceFolder}/.task-state/exports serve-stdio
```
...
```bash
uv tool install "mcp-workbay-handoff==0.2.0"
uv tool install "mcp-workbay-orchestrator==0.2.0"
```
...
- Installed `mcp-workbay-handoff` and `mcp-workbay-orchestrator` from PyPI
...
Command Palette → `MCP: List Servers` → both "mcp-workbay-handoff" and "mcp-workbay-orchestrator" should appear.

```bash
mcp-workbay-handoff --workspace-root "$(pwd)" doctor       # prints the live registered tool list
mcp-workbay-orchestrator --workspace-root "$(pwd)" doctor  # prints the live registered tool list
```

### Available Tools

**`workbay-handoff-mcp`** (core ledger): task state, decisions, findings, blockers, tests, actions, artifacts, export/import, close check, session loading, slice closure, and handoff search.

**`workbay-orchestrator-mcp`**: daemon lifecycle, workers, lane management, plan cursors, turn metrics, dispatch, backends, cross-task tools (`switch_task`, `get_review_findings_summary`, `reconcile_review_findings`, `get_latest_slice_review_packet`).

Example CLI equivalents:

```bash
# Core ledger
mcp-workbay-handoff --workspace-root "$(pwd)" state
mcp-workbay-handoff --workspace-root "$(pwd)" dashboard
mcp-workbay-handoff --workspace-root "$(pwd)" handoff-close-check

# Orchestration
mcp-workbay-orchestrator --workspace-root "$(pwd)" orchestrator-start --task-ref <task-ref> --backend codex-cli --model o3-mini
mcp-workbay-orchestrator --workspace-root "$(pwd)" orchestrator-status
mcp-workbay-orchestrator --workspace-root "$(pwd)" orchestrator-pause
mcp-workbay-orchestrator --workspace-root "$(pwd)" orchestrator-resume
mcp-workbay-orchestrator --workspace-root "$(pwd)" orchestrator-stop
mcp-workbay-orchestrator --workspace-root "$(pwd)" dispatch \
  --lane-id <lane-id> \
  --task-ref <task-ref> \
...
which registers the local stdio server as `mcp-workbay-handoff` with the required
...
3. Test manually: `mcp-workbay-handoff --workspace-root "$(pwd)" serve-stdio` (should block on stdin)
...
3. **5.3 Handoff Closure**: `mcp-workbay-handoff handoff-close-check --task-ref <task>`.
...
- To switch between tasks, use `switch_task(task_ref)` on `workbay-orchestrator-mcp`. It auto-archives the outgoing task and restores the target's objective from its archive. This replaces the multi-step `archive_task_state` + `set_handoff_state` workflow.
...
- `reconcile_review_findings` (on `workbay-orchestrator-mcp`) validates state integrity (duplicates, done+open mismatch, stale open findings, provenance completeness, reopen metadata coherence) and can apply safe dedupe fixes.
...
mcp-workbay-handoff --workspace-root "$(pwd)" handoff-search \
...
1. `workbay_orchestrator_mcp.orchestration.lane_prompt` renders an actionable worker prompt from MCP state (open lane messages, pending actions, open blockers, open findings).
