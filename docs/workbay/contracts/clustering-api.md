...
- Invariant: `identity_count ≥ representatives.length`; `representatives` is non-empty (`minItems: 1`); `identity_count` is the observed member-row count when the preview fetch is not truncated (fetch cap+1 so `observed == 4` is exact and `observed > 4` is truncation). When `observed > cap` the fetch is truncated: republish the projected column (even if stale-low) and request repair — do not publish `observed` as an exact count. The queue predicate is backed by `acx_identity_members` rows (`COUNT(m) >= 2`), so a cluster whose members were reprojected away is not served.
