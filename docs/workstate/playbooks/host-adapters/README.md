...
Canonical, agent-agnostic procedures belong in `docs/workstate/playbooks/` (the parent). When a procedure applies to all agents and human operators, it goes there. When a procedure is specific to one host runtime (e.g., Codex bootstrapping, Codex MCP attachment), it goes here.
...
- [codex-custom-mcp-playbook.md](codex-custom-mcp-playbook.md): **Codex adapter** — how to attach the `workstate-handoff-mcp` server to a Codex session over HTTP.
