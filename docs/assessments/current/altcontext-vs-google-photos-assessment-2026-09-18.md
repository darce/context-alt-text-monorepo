...
- **ACX better at:** accessibility output as a first-class artifact; provenance (which face, which phrase, which policy allowed a name); tenant isolation, purge, export and retention as product features; naming-agreement consent gate before any name appears in text; tenant-scoped hosted recognition API (self-hosting is a future tier, privacy assessment 2026-06-13); roster as a *publishing* entity, not a private label.
...
| Merge two groups of the same person | "Same or different person?" prompt; merge from person page | `POST /clusters/{id}/merge`, `/merge-candidates`, `PersonMergeService` preview/commit/undo | **Match** in capability; GP ahead in UX (prompted, one click). Bind (`cluster_person_bound`, carries `person_uuid`) and person merge are separate operations; open finding REL0024-H-06 reports duplicate groups after bind — cause still to be reproduced |
...
| Person detail page | Person page: cover, all photos, name, merge, hide | Roster row → `PersonWorkspacePanel` (header, face groups, representatives, instances); `GET` person-media route. Remaining gap: cover choice and full photo grid (REL0024-M-11) | **Near match**; GP more polished |
...
| Face regions persisted in the file | Not documented in the cited help pages | XMP embedding writes curated face regions (`ImageRegion`), names and metrics into the image file; alt text is **not** embedded today | **ACX only** for regions/names; alt-text embedding is a gap |
...
| Alt text / description for publishing | Not documented in the cited help pages (publishable description export unverified, §7) | `POST /describe/run`, review queue, Ready-to-apply, writes `<img alt>` and Gutenberg block alt (E20-6); public guide | **ACX only** |
...
| Consent / opt-in for recognition | Face grouping toggle; regional defaults | Recognition setting (`RecognitionPolicy::DEFAULT = true` — **default on today**, contrary to SECD-12), naming agreement (`routers/tenant.py`), retention policy/preset | **Gap on default** (must flip to off); ACX ahead on granularity (naming ≠ grouping) |
| Data lifecycle | Delete photos; turn off grouping deletes face models | `/retention/policy`, `/retention/policy/preset`, `/retention/export`, `/retention/purge`, `/retention/audit`, TTL modes (privacy assessment 2026-06-13) | **ACX ahead** — lifecycle is a product feature with an audit trail |
| Tenancy / hosting | Google account | Tenant-scoped API keys, hosted on OCI (self-hosting a future tier), no cross-tenant gallery | **ACX ahead** for publishers |
...
1. **The output is publishable text with named people.** GP names are private and never leave the account. ACX names flow into alt text and captions on a public site, gated by the naming agreement and the per-face policy (`resolve_naming_allowed`). No consumer photo product does this. Canon: A11Y-59; ATTRIB-02 (carry the confidence of an attribution into the published text).
...
3. **Consent is layered.** Grouping, naming agreement and per-identity suppression are separate switches. GP has one toggle. Caveat: the recognition switch defaults **on** today; this advantage holds only after it flips to off (SECD-12). Canon: SECD-12 (biometric opt-in default), SECD-13 (necessity gate), MLDATA-13/14 (body-derived data is its own class with a capture-basis column).
...
7. **Portable output.** XMP face-region embedding and retention export mean the curated roster outlives the hosting platform (alt text itself is not yet embedded in the file). GP's names and groups are locked to the account.
...
| **Person page** | Cover, every photo, rename, merge, hide in one place | Extend `PersonWorkspacePanel`: cover pick + full photo grid | `PersonWorkspacePanel`, person-media route, `/representatives/{id}/pin` | HAI-17, COG-02 |
...
| **Single opt-in switch with regional defaults** | Compliance is a default, not a policy page | Flip recognition to off by default (`RecognitionPolicy::DEFAULT`); add a **new** jurisdiction/recognition preset surface (distinct from `/retention/policy/preset`) | `class-recognition-policy.php`, privacy assessment jurisdiction controls | SECD-12, SECD-13 |
...
| Adaptive clustering (reliable cores, provisional attachments, cannot-links) | Not documented for GP | ACX design analogy to Apple's 2021 staged precision/recall (FIR v11), not a GP equivalent; ACX adds human cannot-links from the split action |

## 5. Risks and cautions

- **Region defaults.** GP ships face grouping off or unavailable in the EU, UK, Illinois and Texas. ACX currently defaults recognition **on**; it must flip to off and expose a jurisdiction preset (privacy assessment: BIPA, GDPR, EU AI Act). SECD-12.
...
0. Flip `RecognitionPolicy::DEFAULT` to off (SECD-12) — the one privacy invariant this comparison shows unmet.
...
- Google Photos Help: "Find people & pets", "Turn face grouping on or off", "Merge or hide face groups" (support.google.com/photos; from prior knowledge, not re-accessed 2026-09-18 — every GP claim here is bounded by these pages)
