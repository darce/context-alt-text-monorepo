...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-curation-backend", default=None, help="Optional backend for explicitly budgeted batch ACE curation."
    )
    parser.add_argument(
        "--model-curation-model", default=None, help="Optional model override for model-backed ACE curation."
    )
    parser.add_argument(
        "--model-curation-reasoning-effort",
        default=None,
        help="Optional reasoning effort for model-backed ACE curation.",
    )
    parser.add_argument(
        "--model-curation-threshold",
        type=int,
        default=5,
        help="Minimum pending or pruning-candidate count required before model-backed curation can run.",
    )
    parser.add_argument(
        "--model-curation-budget-tokens",
        type=int,
        default=20000,
        help="Maximum cumulative token budget for model-backed ACE curation runs logged in this state-dir.",
    )
