...
- [ ] Phase 0: Install and authenticate Codex CLI (`npm i -g @openai/codex@latest`, `codex login`)
- [ ] Phase 0: Configure `~/.codex/config.toml` with `mini` and `full` profiles
- [ ] Phase 0: Wire `--model` flag through `lane_exec.py` `run_lane_exec()`
- [ ] Phase 0: Wire `--model` flag through `review_runner.py` `_codex_exec()`
- [ ] Phase 0: Wire `model` parameter through `worker_daemon.py` `worker_loop()`
- [ ] Phase 0: Wire `MODEL=` through `mk/lane-worker.mk` (`lane-run`, `worker-daemon` targets)
- [ ] Phase 0: Wire `MODEL=` through `mk/handoff.mk` (`orchestrator-daemon` target)
- [ ] Phase 0: Wire `model` parameter through `orchestrator_daemon.py` CLI + `orchestrator_loop()`
- [ ] Phase 0: Wire `model` parameter through `api.py` (`orchestrator_start`, `worker_start`, `worker_start_all`)
- [ ] Phase 0: Verify `codex exec -m gpt-5.4-mini` completes on dev machine
- [ ] Phase 1: Define `BackendAdapter` Protocol and `BackendResult` dataclass
- [ ] Phase 1: Implement `CodexCliAdapter` with moved `find_codex()` logic and `--model`/`--profile` support
- [ ] Phase 1: Implement `CodexSubagentAdapter` wrapping bridge module
- [ ] Phase 1: Refactor `backend_registry.py` to return adapter instances
- [ ] Phase 1: Refactor `lane_exec.py` to use `adapter.execute()`
- [ ] Phase 2: Remove `find_codex()` and `_run_subagent()` from `lane_exec.py`
- [ ] Phase 2: Remove `_find_codex_path()` and `_codex_exec()` from `review_runner.py`
- [ ] Phase 2: Remove `apply_codex_runtime_hints()` from `_env.py`
- [ ] Phase 2: Remove `_resolve_reasoning_effort()` from `worker_daemon.py`
- [ ] Phase 2: Replace `"codex"` default in `orchestrator_guidance.py`
- [ ] Phase 3: Add `dispatch_lane_work` MCP tool with `model` parameter
- [ ] Phase 3: Add `list_available_backends` MCP tool
- [ ] Phase 3: Worker daemon reads dispatch parameters (including `model`) from MCP state
- [ ] Phase 4: Implement `ClaudeCodeAdapter`
- [ ] Phase 4: Implement `LocalModelAdapter`
- [ ] Phase 4: Manifest-to-agent-config generation utility
- [ ] Phase 4: Add `preferred_backend` and `preferred_model` to lane manifest schema
- [ ] Phase 5: Update `instructions.md` -- model selection in worker/orchestrator daemon commands
- [ ] Phase 5: Update `instructions.md` -- MCP orchestration commands with `model` parameter
- [ ] Phase 5: Update `worktree-codex-playbook.md` -- Quick Reference with `MODEL=` parameter
- [ ] Phase 5: Update `worktree-codex-playbook.md` -- Execution backends section with model guidance table
- [ ] Phase 5: Update `worktree-codex-playbook.md` -- Replace `CODEX_ARGS='--model o3'` with `MODEL=`
- [ ] Phase 5: Add new "Recipe: Codex CLI setup" to playbook
- [ ] Phase 5: Update playbook MCP orchestration commands with `model` parameter
