...
- **Dogfooded face curation**: provision a dedicated eval tenant on the existing dev/staging stack (`scripts/provision_demo.py` flow); ingest Golden-100 via the eval-harness REST client — `client.analyze([(media_id, filename, image_bytes)])`, the existing path in `scripts/eval_harness/cli.py:229`, against `ACX_EVAL_BASE_URL` with eval-tenant credentials — so the recognition service's own HDBSCAN clustering (`recognition/infrastructure/clustering/hdbscan_adapter.py`) proposes entity clusters; operator confirms/corrects clusters into identity ground truth. **No WordPress instance is involved**: corpus separation is manifest + tenant-scoped (stable `media_id`s in golden.json), so no WP media-library grouping ("gallery") is needed and no new local WP install is spun up. **Anti-circularity rule**: service clustering output is a *draft only* — human confirmation is what makes it ground truth; recognition metrics are always scored against the curated truth, never against the service's own uncurated output. This doubles as an end-to-end exercise of the ingest→embed→cluster pipeline on hard strata.
...
- [ ] Eval tenant provisioned; Golden-100 ingested; dogfooded cluster drafts curated into face identity ground truth (all 100)
