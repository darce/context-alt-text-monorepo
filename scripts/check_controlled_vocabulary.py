    ...
    for form_index, raw_form in enumerate(forms_value):
        ...
        if form in claimed_forms:
            raise VocabularyConfigError(
                f"{location}.forms[{form_index}] duplicates the form {form!r}"
            )
...
def load_vocabulary(path: Path) -> Vocabulary:
    ...
    try:
        ...
        raise VocabularyConfigError(
            f"controlled_vocabulary.gate_mode must be one of: {allowed}"
        ) from exc
    ...
    terms = tuple(
        _load_term(value, index, claimed_forms) for index, value in enumerate(terms_value)
    )
    ...
    baseline = tuple(
        _load_baseline_entry(value, index, banned_terms)
        for index, value in enumerate(baseline_value)
    )
    baseline_keys = [entry.key for entry in baseline]
    if len(set(baseline_keys)) != len(baseline_keys):
        raise VocabularyConfigError(
            "controlled_vocabulary.known_violations contains duplicate entries"
        )
...
def scan_sources(source_root: Path, vocabulary: Vocabulary) -> list[Violation]:
    ...
    for path in paths:
        relative_parts = (
            path.relative_to(source_root).parts if path != source_root else (path.name,)
        )
...
def run(config_path: Path, source_root: Path, *, report_all: bool = False) -> int:
    ...
    baseline_counts = Counter(
        {entry.key: entry.occurrences for entry in vocabulary.known_violations}
    )
    ...
    for violation in violations:
        ...
        allowance = (
            baseline_counts[violation.key] if vocabulary.gate_mode is GateMode.BASELINE else 0
        )
    ...
    if stale:
        print(
            f"INFO: {stale} baseline entries are no longer fully observed; "
            "remove them in the vocabulary cleanup wave."
        )
    if introduced:
        print(
            f"Controlled-vocabulary check failed: {len(introduced)} "
            "new translatable string violation(s)."
        )
        return 1
