...
```text
.vscode/mcp.json  →  env { PATH, AGENT_HANDOFF_ENFORCE_BRANCH=1 }  →  uvx "mcp-workstate-handoff==0.12.3" --workspace-root ${workspaceFolder} --state-dir ${workspaceFolder}/.task-state --exports-dir ${workspaceFolder}/.task-state/exports serve-stdio
```
...
```text
.vscode/mcp.json  →  env { PATH, AGENT_HANDOFF_ENFORCE_BRANCH=1 }  →  uvx "mcp-workstate-orchestrator==0.6.0" --workspace-root ${workspaceFolder} --state-dir ${workspaceFolder}/.task-state --exports-dir ${workspaceFolder}/.task-state/exports serve-stdio
```
