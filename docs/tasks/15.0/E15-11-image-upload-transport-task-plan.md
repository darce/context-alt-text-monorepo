...
- Deterministic tests:
  - `cd apps/prototype-description-service && VIRTUAL_ENV= uv run --locked --extra dev python -m pytest recognition/tests/api/test_analyze_multipart.py recognition/tests/application/test_object_store_filesystem.py -q`
  - `cd apps/prototype-wp-alt-context && composer test -- --filter AnalysisJobsControllerTransportTest`
- Runtime-parity / environment checks:
  - `cd apps/prototype-description-service && VIRTUAL_ENV= uv run --locked --extra dev uvicorn api.main:app --port 8001` then `curl -F 'request=@req.json;type=application/json' -F 'image_42=@./fixture.jpg'` against the new multipart endpoint with a known-good auth header
...
- `cd apps/prototype-description-service && VIRTUAL_ENV= uv run --locked --extra dev python -m pytest recognition/tests/api/test_analyze_multipart.py recognition/tests/application/test_object_store_filesystem.py -q` exits 0 with the multi-part suite green
