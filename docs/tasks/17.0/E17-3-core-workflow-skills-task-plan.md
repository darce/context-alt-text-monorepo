...
|----------|-------|------------------|-----------------|----------------------|--------------|
| `.claude/settings.json` PostToolUse | repo-wide | Existing hooks: `review_findings` → `ace-detect.py`; `get_handoff_state`/`load_session` → `slim-handoff-response.py`; `Bash` → `filter-test-output.py` | Add new PostToolUse hook matching `record_event\|review_findings\|review_runs\|set_handoff_state\|update_task_status` → regeneration script | No conflict — new matcher is additive; `review_findings` already has a hook, new hook adds a second handler | Manual: verify both hooks fire on `review_findings`; `DASHBOARD.txt` updates after covered writes while `CURRENT_TASK.json` remains on-demand |
...
| Script | `scripts/hooks/regenerate-task-views.sh` | New shell script: calls `agent-handoff-mcp write-dashboard` only (`CURRENT_TASK.json` stays on-demand) |
...
- Manual verification:
  ...
  - PostToolUse hook regenerates DASHBOARD.txt only on writes; CURRENT_TASK.json timestamp must NOT change; hook skips on read ops (`list`, `get`, `coverage`) — verify via timestamp checks
...
- Create `.claude/skills/handoff-lifecycle/SKILL.md`:
  ...
  - Core process: `make context` → `load_session` (hot state) → verify alignment (branch matches task target_branch) → work loop → record decisions → `generate_dashboard_md` after every state write (CURRENT_TASK.json on demand) → session end
  - Documents: `switch_task` as safe task-transition entry point; `archive_task_state` must only run after `update_task_status(done)`; DASHBOARD.txt and CURRENT_TASK.json are generated views — never hand-edit
  - Common rationalizations: "I'll archive while in_progress, it's fine"; "I already know what state I'm in, I don't need load_session"; "I'll update CURRENT_TASK.json directly instead of regenerating"
...
**Goal**: Wire a PostToolUse hook in `.claude/settings.json` that keeps `DASHBOARD.txt` current after covered state-changing MCP writes while reserving `CURRENT_TASK.json` for explicit on-demand regeneration.
...
- Create `scripts/hooks/regenerate-task-views.sh`:
  ...
  - Calls `agent-handoff-mcp --workspace-root "$WORKSPACE_ROOT" write-dashboard` to regenerate DASHBOARD.txt
  - Does NOT call `agent-handoff-mcp task` — CURRENT_TASK.json is on-demand only, regenerated explicitly by agents when a task-scoped snapshot is needed
...
- Manual: trigger `record_event` write; confirm DASHBOARD.txt timestamp updates but CURRENT_TASK.json timestamp does NOT change; confirm a `review_findings(operation="list")` call also does NOT update DASHBOARD.txt
...
- [x] Create `scripts/hooks/regenerate-task-views.sh` (read-op filter, DASHBOARD.txt only — no task call, exits 0, suppresses output)
- [x] Add PostToolUse hook entry to `.claude/settings.json`
- [x] Verify hook fires after `record_event` — `DASHBOARD.txt` updates while `CURRENT_TASK.json` remains unchanged unless explicitly regenerated
...
- [x] PostToolUse hook regenerates `DASHBOARD.txt` after any covered state-changing write; `CURRENT_TASK.json` remains on-demand only
