...
def test_review_mode_summary_scopes_counts(isolated_handoff: RuntimeConfig) -> None:
    _parse(
        mcp_server.set_handoff_state(
            task_ref="review-guide-hardening",
            objective="Test review mode summary",
            status="review",
        )
    )
    _parse(
    ...
    branch_summary = _parse(mcp_server.get_review_findings_summary(review_mode="branch"))
    audit_summary = _parse(mcp_server.get_review_findings_summary(review_mode="release_audit"))

    assert branch_summary["counts"]["total"] == 1
    assert branch_summary["counts"]["severity"]["medium"] == 1
    assert audit_summary["counts"]["total"] == 1
    assert audit_summary["counts"]["severity"]["high"] == 1
