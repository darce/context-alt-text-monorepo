...
import { useRecognitionClusters } from '../hooks/useRecognitionHooks';
...
export const RosterPage = (): React.JSX.Element => {
	...
	const [selectedCluster, setSelectedCluster] = React.useState<ClusterSummary | null>(null);

	const clustersQuery = useRecognitionClusters({ limit: 20 });
	const clusters = clustersQuery.data ?? [];
	const mediaMap = useClusterMediaMap(clusters);
	...
	const handleRescanCluster = (cluster: ClusterSummary): void => {
		const mediaIds = Array.from(new Set(cluster.sample_faces.map((face) => face.media_id)));
	...
	const handleCommitCluster = (cluster: ClusterSummary, assignment: { rosterEntryId?: number; newEntryName?: string }) => {
		...
		<section className="acx-roster" aria-labelledby="acx-roster-title">
			<header className="acx-roster__hero">
				...
				<p className="acx-roster__subtitle">
					{__('Review roster entries and fine-tune facial clusters to keep recognition accurate across batches.', 'alt-context')}
				</p>
			</header>
