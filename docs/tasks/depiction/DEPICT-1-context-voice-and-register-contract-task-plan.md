...
DEPICT-0 section **Consumers:** (`:454-536`; re-verify after concurrent
...
- **Prompt text is out of scope.** The weave-instruction half of F1 has nowhere
  ...
  body export (DEPICT-0 section **Consumers:**, `:454-536`). The
...
- Production weave clause ("Weave the people's names and factual details…") is
  ...
  `:454-536`: selected-config consumers bind via the accessor functions; the
...
| `scene/infrastructure/vlm/gpu_remote_adapter.py` | Today: `_SYSTEM_PROMPT` L28–37 + weave L33–34; production serialization path `_render_context` L73–87 / `_user_text` L90–101 / `_describe` L176–182. Post-DEPICT-0: local constant deleted; body lives in `caption_system.py`; adapter **snapshots once** at `__init__` via `production_prompt()` and posts `self._production_prompt.body` (DEPICT-0 section **Consumers:** `:454-536` — not a describe-time `production_system_prompt()` re-read; not the accessor-definition block at DEPICT-0 `:405-433`, which itself reads `.body` inside the accessor). F1 (b) weave is OUT of this wave / unowned. |
| `scene/prompts/caption_system.py` | **Created by DEPICT-0.** Post-seam home of selected frozen `PRODUCTION_PROMPT` (`body` + `lineage_version`) and accessors `production_prompt()` / `production_system_prompt()` / `production_prompt_or_task_version()`. **Consumer bind rule** (DEPICT-0 section **Consumers:** `:454-536`): only permitted binding form for production consumers **of the selected config** is the accessor functions; do **not** read `PRODUCTION_PROMPT.body` / `.lineage_version` at the call site **via `from … import PRODUCTION_PROMPT`**, and do **not** introduce a dedicated frozen body export. The adapter posts `self._production_prompt.body` off its construction-time snapshot (required; not a call-site module bind). **Mutation rule:** REBIND-ONLY — replace `PRODUCTION_PROMPT` with a new `ProductionPrompt(body=…, lineage_version=…)`; never assign to `.body` on the frozen instance (`FrozenInstanceError`). A rebind is **not** retroactive on already-constructed adapters that bound body/stamp earlier. No separate `PRODUCTION_SYSTEM_PROMPT` string global. |
| `scripts/eval_harness/bakeoff.py` | Lane A / DEPICT-0 — harness prompt seam. After DEPICT-0, bakeoff v1 system text obtains the selected body via `production_system_prompt()` (accessor call at use site — not `from … import PRODUCTION_PROMPT`; DEPICT-0 section **Consumers:** `:454-536`). A future F1 (b) rebind of `PRODUCTION_PROMPT` changes bakeoff system **wording** only for subsequent accessor reads. Full harness F1 voice-honouring is **OUT / unowned** (no harness pack `voice`). |
...
|---|---|---|---|
| Prompt seam (no wording) | `gpu_remote_adapter.py`, `bakeoff.py`, `scene/prompts/caption_system.py` (new) | Single source for production system prompt text; delete local `_SYSTEM_PROMPT`; consumers of the selected config bind via the accessor functions; adapter snapshots once at `__init__` and posts `self._production_prompt.body` (DEPICT-0 section **Consumers:** `:454-536` — no describe-time accessor re-read; no `from … import PRODUCTION_PROMPT` bind; DEPICT-0 `:405-433` is the accessor *definition* that itself reads `.body`, not the consumer prohibition) | **DEPICT-0 / Lane A** — lands **first**; Not-Does F1 weave rewrite (DEPICT-0 §Not-Doing) |
...
- [ ] Confirmed no edits under Lane A or Lane C paths; cross-lane deps recorded
      ...
      `:454-536`; never mutate frozen `.body`; rebind not retroactive on
...
- [ ] F1 production weave explicitly **not** claimed closed — F1 (b) is **OUT
      ...
      `PRODUCTION_PROMPT`; consumers of the selected config bind via accessors;
      adapter posts `self._production_prompt.body` from construction-time
      snapshot — DEPICT-0 section **Consumers:** `:454-536`; no
...
- Prompt / weave instruction edits — **F1 (b) production-weave**, **OUT of this
  ...
  `:454-536` forbids `from … import PRODUCTION_PROMPT` and a dedicated frozen
  body export, and forbids describe-time accessor re-read of the body; do
  **not** cite the accessor-definition block at DEPICT-0 `:405-433` as that
