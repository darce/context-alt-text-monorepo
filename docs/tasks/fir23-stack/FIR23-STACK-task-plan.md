...
- Deploy driver `recognition-service.sh` env-mapping case blocks: `env_to_tag`(:95), `env_to_unit`(:103),
  `env_to_remote_dir`(:109), `env_to_health_url`(:117), `env_to_ready_url`(:125), `env_to_compose_files`(:302).
  Also `do_status()` hardcodes `for env in dev staging prod`(:606). `sync-compose.sh:22` allow-list.
...
- Systemd unit from `systemd/acx-env.service.template`; `WorkingDirectory=/opt/acx-backend/{{ENV}}`,
  ...
  (`recognition-service.sh:325,336`). **WorkingDirectory basename must equal ENV** → remote dir MUST be
  `/opt/acx-backend/dev-fir` (NOT `/opt/acx-backend/fir`).
- Caddy multi-homed via `/opt/acx-backend/docker-compose.caddy.yml` (networks: per-stack, `external: true`);
  deploy driver deliberately does NOT reship Caddy (`recognition-service.sh:320-322`). Caddyfile:
  `dev.api.altcontext.com { reverse_proxy dev-api:8000 }`.
...
- ingress: `fir.api.altcontext.com → dev-fir-api:8000`
...
- `recognition-service.sh`: add `dev-fir)` arm to all 6 env-mapping case blocks — tag=`dev`,
  unit=`acx-dev-fir`, **remote dir=`/opt/acx-backend/dev-fir`** (BR-04), health/ready=`https://fir.api.altcontext.com/health|/ready`,
...
- `db-reset-remote.sh`: add `dev-fir)` arm with PG_CONTAINER=`acx-dev-fir-postgres-1`,
  ...
  HEALTH_URL=`https://fir.api.altcontext.com/health` (BR-06).
- `mk/deploy.mk`: `deploy-dev-fir`, `deploy-rollback-dev-fir` (`.PHONY`; `deploy-verify ENV=dev-fir` already generic).
...
1. Create `docker network create acx-dev-fir-net` (own network — BR-02).
2. Create `/opt/acx-backend/dev-fir/.env` from `.env.fir.example` (inline secrets, fir DB password).
3. `fetch_face_pipeline_models.py --dest /opt/acx-backend/data/dev-fir-models/face_pipeline`
   so ONNX land where `RECOGNITION_FACE_PIPELINE_MODELS_DIR=/data/cache/face_pipeline` reads them (BR-01).
4. `make deploy-dev-fir`; verify: container Up, boot migration at `vector(128)`, three-way guard passes,
   `active_embedding_model_id()==opencv-sface@128d/l2/cosine`, **local** `/health` green.
5. **Operator adds DNS A-record `fir.api.altcontext.com → VM`; CONFIRM it resolves** (BR-07) BEFORE step 6.
6. Add `acx-dev-fir-net` to `docker-compose.caddy.yml` networks + `fir.api.altcontext.com` vhost to Caddyfile;
   `docker compose -f docker-compose.caddy.yml up -d` to recreate Caddy — AFTER fir up + DNS resolves (BR-03, BR-07).
7. Verify public `https://fir.api.altcontext.com/health` green + TLS issued.
...
- Set `acx_recognition_url=https://fir.api.altcontext.com`; **auth disabled ⇒ no tenant key** (mirror dev) (BR-05).
- Scan the SAME image set on both stacks → compare curated clusters in the two Workbenches.

## Open threads

- Caddy network model confirmed per-stack; Slice-2 step 6 attaches Caddy to `acx-dev-fir-net`.
