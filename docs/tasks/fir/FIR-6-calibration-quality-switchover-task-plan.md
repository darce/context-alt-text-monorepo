...
> **Metadata**
>
> - **Date**: 2026-07-22 (rev 4 — post plan-analyze + three adversarial /planning-review rounds; findings FIR-6-PA-01..06, FIR-6-GR-01..13, FIR-6-LC-01..12, FIR-6-GR2-01..05, FIR-6-LC2-01..05, FIR-6-LC3-01..04 in MCP)
...
S1 lands the knob surface + the profile-resolution helper; **S2 deliverable 5 rebinds every consumer site through the helper** (the enumerated list lives there — that rebinding is what makes S4's apply-commit govern the runtime); S4 fills in calibrated values.
...
3. **Plumbing — full round trip** (resolves LC-03/GR2-03/LC2-03): `FaceDetection` gains the three factor fields; scan persist path (`scan/service.py` identity-write sites `:472-474,504-506` vicinity) stores them to the three **new nullable `media_identities` columns** (001 greenfield edit + sync/verify parity); the ORM model (`db/models/identity.py` media_identities mapping) gains the columns; **every row→`MediaIdentity` hydration mapper copies them**: `orchestrator._build_domain_identities` (`orchestrator.py:321-338`) and the `cluster_repository.py` mappers (`:541`, `:590`, `:1001`, `:1413` — the sites that copy `pose_*` field-by-field today). `MediaIdentity` (`domain/identity.py:14`) gains the optional fields. **Round-trip test (mandatory)**: persist a face_pipeline `FaceDetection`, load via the clustering repository path, assert all three factors arrive non-`None` on `MediaIdentity` — the silent-hydration-gap killer. **No NEW DB surface beyond these three columns; pose proxies flow through the existing nullable `pose_*` columns by design** (resolves LC-10). Insightface path: fields stay `None`/NULL everywhere.
4. **OACT dark scaffold — with its data path** (resolves LC-04/GR-02/GR2-03/LC2-04): `compute_identity_quality` gains `occlusion_severity: float | None = None` (threaded through to `compute_quality_adjustment`; `None` ⇒ zero term); the additive occlusion term is `-(oact_coefficient × occlusion_severity)`-shaped (monotone, bounded; exact form in-slice), active only under the face_pipeline profile with `oact_coefficient != 0.0` (default `0.0` ⇒ bitwise-identical adjustments). **Caller threading enumerated**: `checks/confidence.py:130-155` (the live gate-time consumer of `threshold_adjustment` — passes the candidate `MediaIdentity.occlusion_severity`), `cluster_repository.py:681`, `representative_selector.py:63`, `detector.py:117` (detection-time: passes `FaceDetection.occlusion_severity`). `FacePipelineSettings.oact_coefficient` is bridged into the quality-settings object `quality.py` consumes via the profile-resolution helper. **Discrimination test (mandatory)**: a non-zero coefficient with non-`None` severity moves `threshold_adjustment` **through the gate-time path in `checks/confidence.py`**, not just `quality.py` in isolation. **OACT coefficient is a single global scalar** (resolves GR2-05, [ARCH-08] boring-first): per-stratum coefficients are explicitly deferred; the S5 report discloses the FNMR tax of the global-coefficient compromise per stratum ([CAL-05]).
5. **Settings surface** from the table above (knobs + profile-resolution helper), rg-008 fail-closed validation. Consumer **rebinding** of the five threshold knobs is S2's deliverable 5 (below) — S1 only lands the surface + helper.
...
3. **Wiring (resolves LC-02/GR-04/GR2-01)**: in `orchestrator.py` between `evaluate_chunk_candidates` and `_persist_accepted_assignments` (`:535-546`), face_pipeline profile only. `resolve_photo_conflicts` returns the **post-joint accepted set + loser identity ids**, and the orchestrator **rebinds `accepted_decisions`, `accepted_ids`, and the chunk outcome counts from the solver output before BOTH the persist call and `partition_unclustered`** — conflict losers must flow into the new-cluster/unknown partition, never orphan (GR2-01). Plus two invariant guards:
   - **Photo-atomic chunking**: `IdentityChunker.iter_chunks` (`chunked_processor.py:79-90`) never splits one `media_id` across chunks (keyed grouping under face_pipeline only; insightface path bit-identical).
   - **Persistent same-photo uniqueness guard** (resolves GR2-02/LC-2 residue; return contract per GR3-01/02): implemented inside `AssignmentWriter.persist_assignments_chunk` (`assignment_writer.py:321` — the ONLY path the orchestrator's bulk persist uses, via `orchestrator.py:624`) or a shared helper both persist paths call. **Gating channel**: `persist_assignments_chunk` gains an explicit `joint_uniqueness_enabled: bool` parameter, passed by the orchestrator from the profile-resolved settings (the writer itself stays profile-ignorant; an unconditional guard would change insightface cross-batch behavior — dark break). **Return contract**: the method returns the set of guard-rejected identity ids alongside its existing counts; the orchestrator **rebinds `accepted_ids` and the partition inputs after persist** so rejected duplicates flow to the unknown/new-cluster partition, never orphan (same rebinding discipline as GR2-01). Test exercises the **bulk path through the orchestrator**, including a rejected duplicate landing in the partition.
...
5. **Threshold-knob consumer rebinding** (resolves GR2-04/LC2-01 — the calibration apply path becomes real here): route the consumers of the threshold overrides through the **pre-committed** profile-resolution helper (wave-1 precondition — the helper + knobs already exist on the base branch): `AssignmentGate` construction (`interface_adapters/http/deps/services.py:153,282`), complete-link/graph discovery threshold reads (`discovery/centroid.py:49`, `representative.py:111`, `graph/discovery.py`, `graph/selection.py:45`), `ClusteringLimitsSettings.similarity_threshold` readers, `IdentityDetectionSettings.default_threshold` readers. (`checks/confidence.py:130-155` rebinding is **S1-owned** — hunk-ownership rule in Lane Decomposition.) **Per-knob discrimination test (mandatory)**: mutate the override → the effective threshold moves under face_pipeline and does NOT move under insightface.

**Tests** (`recognition/tests/unit/test_joint_assignment.py`, new): duplicate-identity photo resolves one-to-one with loser→unknown ([TEST-15] — fails on the pre-S2 path); **conflict loser reaches the new-cluster/unknown partition, never orphans** (GR2-01, orchestrator-level); all-below-threshold photo does not crash and routes to unknown (LC-11); knob-off parity; single-face photo unchanged; photo-atomic chunking property; **orchestrator-level test: duplicate-identity photo whose faces would previously split across chunks ends one-to-one**; persistent-guard test **through the bulk `persist_assignments_chunk` path**: second-batch duplicate rejected under face_pipeline, allowed under insightface (dark parity); **per-knob rebinding discrimination tests** (mutate each face_* override → effective threshold moves under face_pipeline only). Plus the characterization golden.

**Verify**: `cd apps/prototype-description-service && uv run --extra dev pytest recognition/tests/unit/test_joint_assignment.py recognition/tests/unit/test_insightface_characterization_golden.py recognition/tests/unit/test_representative_discovery.py recognition/tests/unit/test_assignment_gate_defaults.py recognition/tests/unit/test_assignment_writer_interface.py recognition/tests/unit/test_diverse_representative_selection.py -q` (existing suites pin the rebinding-touched modules — LC3-02)
...
`scripts/eval_harness/calibrate_face_thresholds.py`, consuming the **pinned face_bakeoff report schema v1** (extracted from `feature/fir-5` `report.py:1028-1052`, committed as fixture `scripts/eval_harness/tests/fixtures/face_bakeoff_report.v1.json` — resolves LC-06/GR-08). Contract with the **real key names** (resolves LC2-05): top-level `report_kind == "face_bakeoff"`, `tau: {tau_k: [float; K], tau_op: float}`, `slices: {stratum: {metrics...}}` (aggregate), `decisions: [{media_id, box_index, true_name, predicted_name, decision, s_max, fold, name_star, enrolled, tau_k}]`, `counts` — pin from the actual fir-5 serialization when extracting the fixture, not from this prose. **Decision→stratum mapping**: per-decision stratum membership is NOT in the report; the CLI takes the **golden manifest as a second, schema-validated input** and joins strata by `media_id` tags. The CLI: validates both inputs fail-fast (missing keys ⇒ non-zero exit), applies the pair-level K-fold protocol from Constraints, emits a per-stratum proposed threshold set + FNMR-tax table (incl. the global-OACT-coefficient tax) + protocol disclosure. Deterministic; writes no settings. FIR-5 side owes a schema-conformance test before S4 (coordination note on the FIR-5 task, not here).
...
| --- | --- |
| S1 | `recognition/infrastructure/face_pipeline/_common.py` (embed_batch → EmbedBatchResult), `ort_adapters.py`, `opencv_ref.py`, `recognition/infrastructure/embeddings/face_pipeline_adapter.py` (`:640-666`), `recognition/application/embedding/detector.py` (FaceDetection fields + `:117` quality call), `recognition/domain/identity.py` (MediaIdentity fields), `recognition/application/scan/service.py` (persist sites), `db/models/identity.py` (ORM columns), `recognition/application/orchestration/clustering/orchestrator.py` (`_build_domain_identities` hydration `:321-338`), `recognition/infrastructure/repositories/cluster_repository.py` (mappers `:541,:590,:1001,:1413` + `:681` OACT call), `db/migrations/versions/001_identity_schema.py` (+3 nullable REAL columns), `db/sync_identity_schema.py`/`db/verify_identity_schema.py`, `recognition/application/assignment/quality.py` (OACT signature + channel), `recognition/application/assignment/checks/confidence.py` (`:130-155` OACT threading), `recognition/application/persistence/representative_selector.py` (`:63` quality call), `recognition/config/settings.py` (knob surface + profile-resolution helper), FIR-4 observability emit site, new tests |
| S2 | `pyproject.toml` (scipy), new `recognition/application/assignment/joint.py`, `recognition/application/orchestration/clustering/orchestrator.py` (wiring + rebinding), `chunked_processor.py` (photo-atomic), `recognition/application/persistence/assignment_writer.py` (`persist_assignments_chunk:321` uniqueness guard), `recognition/interface_adapters/http/deps/services.py` (`:153,:282` gate construction rebinding), `recognition/application/discovery/centroid.py`/`representative.py`/`graph/discovery.py`/`graph/selection.py` (threshold rebinding), new tests (knob surface + `joint_assignment_enabled` are pre-committed wave-1 infrastructure, not S2 files) |
...
**Wave-1 preconditions (resolves LC2-02 + LC3-01) — coordinator commits BOTH on `feature/fir-6` before dispatch, as read-only shared infrastructure for every lane**:
1. The insightface characterization golden (fixture + `test_insightface_characterization_golden.py`) — the baseline must predate every slice; one owner, no divergent goldens.
2. The **knob surface + profile-resolution helper** (`FacePipelineSettings` additions from the table above, **including `joint_assignment_enabled`**, with their rg-008 fail-closed validation tests) — S1 and S2 both consume these symbols in parallel; single ownership up front removes the double-definition and absent-symbol failure modes. S1's remaining settings work is only the OACT coefficient bridge; S2's file row no longer includes `config/settings.py`.

**Hunk-ownership rule (resolves the LC3-01 same-hunk collision)**: `checks/confidence.py:130-155` is owned by **S1** (which threads OACT there) — the face-threshold rebinding of that site moves from S2 to S1's caller-threading deliverable. S2 rebinds only the disjoint sites: gate construction (`services.py:153,282`), discovery threshold reads, limits + detection readers.
