...
def _review_guide_path(workspace_root: Path, review_kind: ReviewKind) -> str:
    guide_name = (
        ...
        if review_kind == ReviewKind.PLANNING
...
def _build_packet_for_decision(
    ...
    *,
    ...
    return {
        "slice_label": str(decision_row["decision"])[len(SLICE_DECISION_PREFIX):],
        "task_ref": task_ref,
        "lane_id": decision_row.get("lane_id"),
...
def get_latest_slice_review_packet_data(
    ...
    *,
    ...
    params: list[Any] = [task_ref, f"{SLICE_DECISION_PREFIX}%"]
    where_sql = "task_ref = ? AND decision LIKE ?"
    if lane_id is not None:
        where_sql += " AND lane_id = ?"
        params.append(lane_id)
