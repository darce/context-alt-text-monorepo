...
Work is structured as four slices (0 is prerequisite infrastructure uncovered during branch setup):

0. **Prerequisite bugfix — `check-task-context.py` silent exit.** Added mid-session: `scripts/check-task-context.py` exits 1 with zero stdout/stderr when `get_handoff_state(sections='identity')` returns an error envelope (e.g. `{ok: false, data: {error: "Ambiguous active task for workspace path; matching task_refs: ..."}}`). The discovery slice depends on `make context` being loud, so the silent failure path is fixed before the Codex probe runs. Scope: surface the error in `_load_active_state` at `scripts/check-task-context.py:166-171`, add a regression test covering the error-envelope branch.
