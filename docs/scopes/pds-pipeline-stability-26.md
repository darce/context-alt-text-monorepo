...
| **F-6** Magic-string status values | Centralize domain status; satisfy sr-007. | New `JobStatus(StrEnum)` in `recognition/domain/`. Replace assignments and comparisons in `recognition/application/` and `recognition/interface_adapters/http/` (clustering + scan paths only). DB column stays `text`; SQLAlchemy `Enum(JobStatus, native_enum=False)` validates at the boundary. |
| **F-7** Long parameter list at orchestration entry | Satisfy sr-008's 8-parameter ceiling. | Group `cluster_unclustered_identities` and `IncrementalClusteringRunner.__init__` ([orchestrator.py:54-113](../../apps/prototype-description-service/recognition/application/orchestration/clustering/orchestrator.py#L54)) into 3 frozen dataclasses: `ClusteringDependencies`, `ClusteringRuntimeConfig`, `ClusteringContext`. Same surface, three values instead of fourteen. Apply the same move to `_persist_and_cache_new_clusters` ([assignment_writer.py:374](../../apps/prototype-description-service/recognition/application/persistence/assignment_writer.py#L374)). |
...
2. **F-6 next** (`JobStatus` enum): unblocks F-2's phase rename ("running" → "reserved" → "completed" or similar) and F-3's breaker-open status without re-touching call sites later.
...
3. `JobStatus` values can be enumerated exhaustively from current code: `pending`, `running`, `completed`, `failed`. If grep turns up a fifth (`retrying`, `awaiting_projection`, etc.), expand the enum and call it out in the slice note rather than silently dropping it.
