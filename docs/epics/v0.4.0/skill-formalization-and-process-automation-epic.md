...
- All 19 skills in `.claude/skills/` now carry the full anatomy frontmatter (`mode`, `context_budget`, `makefile_target`, `mcp_tools`). The initial Phase 1–2 retrofit covered 10 skills; the remaining 9 legacy skills (including `commit2git`, `document-sync`, `investigate`, `refactor`, `rescue-lane`, `review`, `security-audit`, `subfeature-committer`, `worktree-worker`) were retrofitted under the E17-6 core retrofit work. Phase 3 still owes the complementary integration deliverables: `make check-skills` structural validation, planning-pipeline exit-gate wiring, and the routing-table + reference-appendix cleanup.
...
>
> **Phasing note**: Phase 3 and Phase 4 are independent tracks and may execute concurrently. Phase 3 covers skill-anatomy retrofit, `make check-skills`, planning-pipeline gate wiring, and routing-table cleanup; Phase 4 covers workflow-integrity and dashboard work. The Phase 1 → 4 numbering reflects planning order, not a hard sequential dependency between tracks 3 and 4.
...
Exit criteria (mapped to task plans — each bullet names the task plan that owns it so Phase 3 scope is explicit, not implicit):

- [E17-6] All skills pass `make check-skills` anatomy validation (structural validation target)
- [E17-6] Planning pipeline exit gates include automated constitution-alignment check (plan-analyze + planning-review gate wiring)
- [E17-6] `CLAUDE.md` and `instructions.md` reference skills as the primary workflow entry points (documentation routing update)
- [E17-6] `branch-review-guide.md` and `planning-review-guide.md` are explicitly labelled as reference appendices, not primary execution surfaces (guide-to-reference reclassification)
- [E17-6] End-to-end test: cold-start agent -> `make context` -> `make plan-analyze` on a sample plan -> findings recorded in MCP -> `make plan-review` -> verdict recorded -> `make task-start` -> implementation -> `make task-finish` -> clean main
- [E17-7] Concurrent active-task support, raw test-trace storage, MCP tool-surface compression, and manifest-driven portable workflow normalization land after E17-6 exits
