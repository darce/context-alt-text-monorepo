...
- **Weights are approximated by duplication:** the MV duplicates rows to approximate quality-tier weights even though pgvector 0.8.1 supports scalar multiplication; switch to explicit multipliers to reduce compute cost and keep averages exact (`apps/recognition-service/db/migrations/versions/20251102_2019_ec68f4f6d1d2_baseline_postgresql_schema.py:89`). Switching to embedding \* 0.8-style weighting will keep the averages precise and cheaper to compute.
