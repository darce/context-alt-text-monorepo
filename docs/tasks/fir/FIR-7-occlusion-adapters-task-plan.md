...
> **Metadata**
>
> - **Date**: 2026-07-23 (v4 — aligns with `fir-embeddings-dims-detectors-qa-20260723` v2 conclusions; v3 mechanics settled: paired McNemar gate, sealed eval, K=3, PEFT musts, occluder-pack disjunction)
...
- **[C-INDEP-SIGNAL] At least one genuinely independent occlusion signal before any robustness claim (FIR7PR-01 / FIR7PLR-G01).** The claim-bearing gate leg MUST include **at least one occlusion signal that does not share FIR-7's compositing code path**: either (a) an **independently-authored occluder render path** (a second compositor — different placement/blend implementation, ideally different author, not a re-parameterization of `synthetic_occlusion.py`), OR (b) a **small consented real-occlusion holdout** (real masked/sunglasses faces from the consented corpus). The alternate-renderer stress leg satisfies (a) only if it is a distinct code path, not merely a distinct pack fed to the same compositor.
  ...
  **Preferred path for (a) — SAM real-occluder compositing (named, preferred [C-INDEP-SIGNAL] path; QA v2 §SAM).** A SAM-family cutter (SAM / SAM2 / MobileSAM / EfficientSAM — all Apache-2.0, verified in the QA doc §SAM) segments **real** occluder assets (real masks / sunglasses / hands from license-clean photos) and a *separate, independently-written* compositor places them onto clean faces (MLDATA-10: match training degradation to target stats). This is **offline tooling only** — SAM never enters the runtime path. The SAM cutter + compositor code path (`scripts/train/occlusion/sam_occluder_factory.py`) shares **nothing** with `synthetic_occlusion.py` (distinct placement/blend implementation), so it satisfies [C-INDEP-SIGNAL](a). Real-asset occluders also shrink the renderer-signature leak (FIR7PLR-G01). **CPU cost:** MobileSAM / EfficientSAM are CPU-feasible offline; this factory runs on the OCI CPU box or the rented GPU, **never the M1**.

  **Model-card hard cap:** an artifact whose only occlusion evidence is synthetic twins from FIR-7's own compositor is stamped exactly `synthetic-twin-recovery under renderer R only — NO robustness claim`; it may ship as a candidate but no card field, demo, or report may attach robustness language to it. A robustness claim requires a CONFIRMATORY cell from an independent signal (real-occlusion or independent-code-path renderer) with reported N and CI.
...
- **SAM occluder factory**: offline tooling (`scripts/train/occlusion/sam_occluder_factory.py`) that uses a SAM-family model (Apache-2.0) to segment real occluder assets and a *separately written* compositor to place them. Satisfies [C-INDEP-SIGNAL](a); never runtime.
...
- **Detector A/B lever**: drop-in swap of the face detector (candidates: MediaPipe BlazeFace, Paddle BlazeFace-FPN-SSH — license verified at ingest via `license_policy.py`) measured on the same Golden-150 sweep table. Not a training run.
...
- **Slice 0 (unconditional)** exhausts non-training levers, **stage-separated** (detection levers vs re-detect miss; recognition levers on the already-detected subset only). Detection ledger gains **Detector A/B** (MediaPipe BlazeFace / Paddle BlazeFace-FPN-SSH drop-ins) and **person→face cascade** (RT-DETR / D-FINE / PP-PicoDet; ban Ultralytics AGPL) rungs on the FM-05 ladder. Publishes a fixed sweep table + greedy stacking rule, and emits a go/no-go `TRAINING_DECISION` artifact against a pre-registered descope threshold (FIR7PR-10). If levers close the gap → task PASSES here.
- **Slice 1 (unconditional)** builds the clean occlusion training-data pipeline with **disjoint train/eval occluder packs** ([C-EVAL-DISJOINT]), the preferred **SAM occluder factory** independent-signal path ([C-INDEP-SIGNAL](a)), a legal-provenance manifest (buffalo-output ban via `derived_from_model`), a strict train/eval identity split, and the full leakage matrix + synthetic-identity linkage audit (FIR7PR-13). This slice also commits the **hard gate spec** ([C-GATE]) and the sealed-eval registration ([C-SEALED]) before any training.
...
| eval tooling | `scripts/eval_harness/config_levers.py` (new) | Slice 0 lever sweeps, **stage-separated** (detection vs recognition); detection ledger includes **Detector A/B** (MediaPipe BlazeFace, Paddle BlazeFace-FPN-SSH) + **person→face cascade** (RT-DETR / D-FINE / PP-PicoDet; route around Ultralytics AGPL) arms; fixed sweep table; greedy stacking; `TRAINING_DECISION` emitter; δ/FP/IoU gating applies to swapped detectors exactly as to fine-tuned ones |
| eval tooling | `scripts/eval_harness/regate.py` (new) | full-corpus re-embed ([C-REEMBED]); **paired per-floor non-inferiority** (cluster-robust McNemar + Newcombe paired-diff CI for binary floors; bootstrap-over-identities paired CI for non-binary floors; deff/n_eff per AUDIT-11); fixed-denominator `a_s`; cascade metric; mixed-space assertion; program-budget/α-spend accounting; tier-label report lint; FIR7PR-14 claim-lint (blocks public accuracy claims while model-card `intersectional: PENDING-STRATUM`) |
| training (new) | `scripts/train/occlusion/data_pipeline.py` (new) | (clean, occluded) pair generation with **disjoint train/eval occluder packs**; synthetic-identity ingest; dataset-provenance manifest with required `derived_from_model`; leakage matrix + linkage audit |
...
| training (new) | `scripts/train/occlusion/sam_occluder_factory.py` (new) | **SAM occluder factory** (preferred [C-INDEP-SIGNAL](a)): SAM-family cutter (SAM/SAM2/MobileSAM/EfficientSAM) segments real license-clean occluder assets; *separate* independently-written compositor places them (MLDATA-10); shares nothing with `synthetic_occlusion.py`; offline only (OCI CPU / rented GPU, never M1) |
...
| training (new) | `scripts/train/occlusion/license_policy.py` (new) | allow/denylist code constants + SPDX ids + verification metadata + fixtures; denylist covers buffalo **weights and output-derived provenance tags** (`insightface/*` via `derived_from_model`) |
...
| File | Note |
| --- | --- |
| `scripts/eval_harness/synthetic_occlusion.py` | twin renderer — reused as training-data generator; enforce train/eval occluder-pack + identity split here; **must share nothing** with SAM factory compositor |
| `scripts/eval_harness/face_bakeoff.py` | `build_occlusion_twin_pairs` (FIR5GL-01) — the pair-generation seam |
| `scripts/train/occlusion/sam_occluder_factory.py` (new) | preferred [C-INDEP-SIGNAL](a) path — real-asset SAM cut + independent compositor |
...
| `benchmarks/reports/fir-embeddings-dims-detectors-qa-20260723.html` | QA v2 conclusions grounding this v4 alignment |
...
- `config_levers.py` with a **stage-separated lever ledger** (FM-05 ladder gains rungs; QA v2: detection recall 0.504 is attackable from the context side with clean-license models **today**, cheaper than fine-tuning):
  - **Detection levers** (measured against masked re-detect miss / detection recall):
    - YuNet `det_threshold` sweep, input-resolution / tiling, alignment crop-margin (0/20/40% context).
    - **Detector A/B lever** (pure config/model-swap, NOT a training run): swap-in candidates **MediaPipe BlazeFace** and **Paddle BlazeFace-FPN-SSH** (license status per QA doc §detectors; verify at ingest via `license_policy.py`) measured as drop-in detector alternatives on the same Golden-150 sweep table. Highest expected-value-of-information step before any training.
    ...
    - **δ/FP/IoU gating on swapped detectors:** a lever that recovers recall by minting false boxes fails the same paired rule as a fine-tuned detector — FP-rate / IoU thresholds apply exactly as to a trained `yunet-occ`.
...
- **Independent occlusion signal ([C-INDEP-SIGNAL], FIR7PLR-G01)** — provision at least one of: (a) an **independently-authored compositor code path** (distinct placement/blend implementation, not a re-parameterization of `synthetic_occlusion.py`) exposed as `--leg alt-renderer`; OR (b) a **small consented real-occlusion holdout** curated from the corpus. A third pack fed to the *same* compositor does **not** satisfy this. This signal, not synthetic `a_s`, carries any robustness claim.
  - **Preferred (a) work item — SAM occluder factory** (`scripts/train/occlusion/sam_occluder_factory.py`, new; owned by `fir7-data` lane): SAM-family cutter (SAM/SAM2/MobileSAM/EfficientSAM — Apache-2.0) segments **real** occluder assets (masks/sunglasses/hands from license-clean photos); a *separate, independently-written* compositor places them (MLDATA-10). Code path shares **nothing** with `synthetic_occlusion.py` → satisfies [C-INDEP-SIGNAL](a) and shrinks renderer-signature leak (FIR7PLR-G01). Offline only: OCI CPU box or rented GPU, never the M1; SAM never enters the runtime path.
- `data_pipeline.py`: generate training pairs over **train-split identities + train-pack occluders only**; ingest license-checked synthetic-identity faces (Vec2Face/DCFace — license verified via `license_policy.py` before ingest); emit `occlusion-training-data-provenance.json` with required **`derived_from_model`** field (any `insightface/*` value FAILS the audit — buffalo output ban).
...
- **Failure-mode dispositions (RLSE-08)**:
  ...
  - **sealed gate says adapters cannot close the gap within budget K** → disposition **`escalate to the retrain campaign task`** (future FIR task, own plan: SCRFD-architecture detector + AdaFace-style 512-D embedder on synthetic+consented data, weights server-side). **NOT** deeper adapter work and **NOT** raising K. FIR-7 stays 128-D; dim changes belong to that campaign.

Proof:

- Boot smoke + health readiness green per id; **rollback unit test** green; `make check-remote` green; final tier-labeled delta table in `benchmarks/results/golden150-fir-adapted-*/`; handoff decision records each produced artifact + sealed evidence for FIR-6's gate (or the retrain-campaign escalation disposition).
...
| --- | --- | --- | --- |
| `fir7-levers` | `scripts/eval_harness/config_levers.py` (Detector A/B + person→face cascade arms), `regate.py` | none | `pytest scene/tests -k eval_harness`; lever-ledger fixtures for A/B + cascade |
| `fir7-data` | `scripts/train/occlusion/{data_pipeline,occluder_packs,sam_occluder_factory,license_policy,model_card}.py`, provenance manifest (`derived_from_model`), gate spec | none | pack-disjunction + SAM-factory independence smoke + provenance-schema + buffalo-output denylist fixture + license-fixture + model-card-schema (incl. `intersectional`) tests |
| `fir7-yunet` | `scripts/train/occlusion/train_yunet_occ.py` | `fir7-data`, `TRAINING_DECISION=go` | sealed Golden-150 detector re-gate |
| `fir7-sface` | `scripts/train/occlusion/train_sface_adapter.py` (UMAP diagnostics) | **`fir7-yunet`** (crops from updated detector), `TRAINING_DECISION=go` | frozen-base assertion + sealed recognizer re-gate |
...
- [ ] Main merged into `feature/fir-7` (FIR-5 SATISFIED-by-main; Slice-0 preflight on-branch assertion passes).
- [ ] Lever sweeps implemented as reproducible harness options, separated into detection vs recognition ledgers.
- [ ] Detection ledger includes **Detector A/B** (MediaPipe BlazeFace, Paddle BlazeFace-FPN-SSH; license via `license_policy.py`) and **person→face cascade** (RT-DETR / D-FINE / PP-PicoDet; Ultralytics AGPL banned); δ/FP/IoU gating applies to swapped detectors.
...
- [ ] Train/eval occluder packs authored, content-hashed, pack-disjunction test green; **independent occlusion signal** ([C-INDEP-SIGNAL]) provisioned — preferred: **SAM occluder factory** (`sam_occluder_factory.py`, distinct from `synthetic_occlusion.py`) OR a consented real-occlusion holdout (not merely a third pack fed to the same compositor).
...
- [ ] Final tier-labeled cascade head-to-head (adapter-alone / detector-alone / joint) + failure-mode dispositions wired (incl. **escalate to retrain campaign** if sealed gate cannot close gap within K — not deeper adapters, not raise K); handoff decision for FIR-6's gate.
