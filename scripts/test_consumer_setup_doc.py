...
REQUIRED_SNIPPETS = (
    'git+ssh://git@github.com/darce/mcp-agent-handoff.git@v0.4.1',
    'git+ssh://git@github.com/darce/mcp-agent-orchestrator.git@v0.1.1',
...
def test_consumer_setup_doc_exists_and_is_standalone() -> None:
    ...
    assert "--upgrade \"git+ssh://git@github.com/darce/mcp-agent-handoff.git@v0.4.1\"" not in update_section
    assert "--upgrade \"git+ssh://git@github.com/darce/mcp-agent-orchestrator.git@v0.1.1\"" not in update_section
