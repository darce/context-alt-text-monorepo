...
lane-run: lane-guard
	...
	set -- $(MCP_PYTHON) -m workstate_orchestrator_mcp.orchestration.lane_exec \
		--orchestrator-root "$(ORCHESTRATOR_ROOT)" \
		--task-ref "$(TASK)" \
		--lane-id "$(LANE)" \
		--session "$(SESSION)" \
	...
	if PYTHONPATH="$(MCP_PYTHONPATH)" "$$@"; then \
		$(MCP_PYTHON) -m workstate_orchestrator_mcp.orchestration.lane_result handoff \
			--orchestrator-root "$(ORCHESTRATOR_ROOT)" \
			--task-ref "$(TASK)" \
			--lane-id "$(LANE)" \
			--session "$(SESSION)" \
...
dashboard-live:
	@PYTHONPATH="$(MCP_PYTHONPATH)" $(MCP_PYTHON) -m workstate_orchestrator_mcp.orchestration.dashboard_live \
		--orchestrator-root "$(ORCHESTRATOR_ROOT)" \
		--task-ref "$(TASK)" \
...
dashboard-tui:
	@PYTHONPATH="$(MCP_PYTHONPATH)" $(MCP_PYTHON) -m workstate_orchestrator_mcp.orchestration.dashboard_tui \
		--orchestrator-root "$(ORCHESTRATOR_ROOT)" \
		--task-ref "$(TASK)" \
		$(if $(LANES),$(addprefix --lanes ,$(LANES)),) \
		--interval "$(or $(INTERVAL),10)" \
