...
|---|---|---|---|---|
| **OB-8a** Notifications topic + routing | An OCI **Notifications topic** with an email subscription; the Cost **Budget alert** (whatever threshold set `E15-5a` defines — the `$50` vs `$1/$5/$10` reconciliation is **E15-5a's**, not OB-8's), **Cloud Guard** findings, and **scheduled-maintenance events** all route to it. Thresholds live in OCI config, not app code. | a test publish reaches the confirmed email; the budget alert and a Cloud Guard finding both land in the topic | `[OBS-05]` (expose state, externalize policy), biz `[SEC-08]` (budget alerts vs denial-of-wallet) | — |
| **OB-8b** symptom alarms | OCI **alarms** page on *symptoms* routed to the OB-8a topic, **each with a named metric source**: `/health` failure → the OCI **compute-agent health check**; TLS-cert expiry → cert-monitor metric; **5xx-rate → an OCI Monitoring custom metric (`PostMetricData`) bridging the app's `/metrics` 5xx counter — that bridge does not exist yet** (no repo publisher found), so OB-8b **stands it up** or defers the 5xx alarm until it does. A raw CPU/memory spike **alone does not page**. | forced `/health` failure fires an alarm to the topic; a synthetic CPU spike with healthy `/health` does **not** page | `[OBS-07]` (page on symptoms not causes), `[OBS-04]` (alarm = operator action) | OB-8a |
...
| **OB-8d** stream freshness gate | A **freshness gate** on the critical **host/infra** telemetry streams only (host metrics, VSS scan cadence): alert when an *expected* stream goes quiet, distinguishing "no events" from "capture broken." **Product-funnel freshness (PostHog flywheel/funnel silence) stays an OB-2/OB-3 concern** — the host plane does not watch the product stream (plane separation). | stop a host metric emitter → the gate fires within the window; a genuinely idle-but-healthy stream does not false-page | `[OBS-08]` (silence is not success), `[REF-05]` (plane separation) | OB-8a |

> **Verifications are operational probes, not `pytest`:** each row's check is a console/CLI/probe run once and observed — a test publish to the topic (OB-8a), a `curl` that trips `/health` (OB-8b), an `oci` scan/limits query (OB-8c) — not a fast unit suite. This is the correct shape for an infra slice; the §15 "one falsifiable end-state + one check" litmus is met by an observed probe, not a `TEST_CMD`.
