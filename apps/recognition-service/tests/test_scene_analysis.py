...
from recognition.domain.entities import EmbeddingEntry, FaceDetection, FaceEmbedding, MatchResult
from recognition.domain.interfaces import EmbeddingRouterPort, RecognitionModelPort
...
class StubRecognitionModel(RecognitionModelPort):
    ...
    async def analyze(self, image: Image.Image):  # type: ignore[override]
        ...
        embedding = FaceEmbedding(embedding=np.array([1.0, 0.0], dtype=np.float32), detection=detection)
        return [embedding]
