...
- **Monorepo** at task branch `feature/e17-10-followon-publish-and-validate`, HEAD `b5a89201` (plan rewrite, decision `#2224`) carries the v0.4.1 source for `agent-handoff-mcp` and unmodified source for the other three packages. Slice 1a fix landed at `a912d91e`; Slice 1b cleanup at `2f835a61` (which also deleted the ephemeral `scripts/test_e17_10_p{1,2,3}_*.py` smoke runners — proof now runs inline via scratch-venv install + import smoke).
...
- **`darce/mcp-agent-orchestrator`** — v0.1.0 (e21aaf96) on `main`. 2-file drift vs current monorepo source: `orchestration/slice_review_packet.py` and `tests/test_lanes_and_handoff_state.py`. Slice 2 below decides ship-or-skip; proof uses an inline scratch-venv install smoke (the earlier `scripts/test_e17_10_p2_orchestrator_remote.py` runner was deleted in Slice 1b cleanup).
- **`darce/agentic-system`** — v0.1.0 (ac7a77c4) on `main`. Material drift: 11 NEW skills (`commit2git`, `daemon-lifecycle`, `document-sync`, `investigate`, `refactor`, `rescue-lane`, `review`, `security-audit`, `subfeature-committer`, `worktree-orchestrator`, `worktree-worker`); 2 modified (`plan-analyze`, `planning-review`); 2 modified hooks (`filter-test-output.py` + tests); 1 modified contract (`security.md`). All `.claude/commands/*` and `.github/prompts/*` byte-identical. Slice 3 below cuts v0.2.0 with the contract split.
- **`darce/agentic-bootstrap`** — v0.2.0 published; clone at `~/Development/agentic-bootstrap/` on branch `feature/e17-10-bootstrap-cli`. `consumer-setup.md` and the doc-lock test in monorepo HEAD already pin `agentic-bootstrap@v0.2.0` and (as of this rewrite) `mcp-agent-handoff@v0.4.1`. Orchestrator pin remains `@v0.1.0` pending Slice 2 disposition.
