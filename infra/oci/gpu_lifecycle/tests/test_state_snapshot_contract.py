...
DEPLOYED_COMPOSE = REPO_ROOT / "apps" / "prototype-description-service" / "docker-compose.env.yml"


def test_previous_snapshot_freshness_defaults_match_real_reader() -> None:
    assert DEFAULT_PREVIOUS_GPU_STATE_MAX_AGE_SECONDS == DEFAULT_GPU_STATE_STALE_SECONDS == 180.0
    assert DEFAULT_PREVIOUS_GPU_STATE_MAX_FUTURE_SKEW_SECONDS == GPU_STATE_FUTURE_SKEW_SECONDS == 5.0
...
def test_real_reader_fails_closed_when_snapshot_is_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_reader_blank_path_env_uses_default(monkeypatch: pytest.MonkeyPatch, configured_path: str) -> None:
...
def test_snapshot_directories_enforce_distinct_writer_ownership() -> None:
    ...
    assert (
        ...
        is None
    ), "lifecycle services must be rendered into the private staged generation"
    ...
    state_chown_match = re.search(r"^sudo chown ([^:\s]+):([^\s]+) /run/acx$", script, flags=re.MULTILINE)
    state_tmpfiles_match = re.search(r"^d /run/acx (\d+) ([^\s]+) ([^\s]+) -$", script, flags=re.MULTILINE)
    ...
    state_chmod_match = re.search(r"^sudo chmod (\d+) /run/acx$", script, flags=re.MULTILINE)
    load_chmod_match = re.search(r"^sudo chmod (\d+) /run/acx-write$", script, flags=re.MULTILINE)
    ...
    environment_load_mode, environment_load_owner, environment_load_group = environment_load_tmpfiles_match.groups()
    ...
    assert int(environment_load_mode[-2]) & 0o2, "API gid 10001 must be able to write per-environment load dumps"
...
def test_deployed_compose_keeps_load_writable_and_gpu_state_read_only() -> None:
    ...
    load_path_match = re.search(r"^\s*- ACX_DESCRIBE_LOAD_PATH=([^\s]+)$", api, flags=re.MULTILINE)
    ...
    mount_matches = re.findall(r"^\s*- ([^:\s]+):([^:\s]+)(?::([^\s]+))?$", api, flags=re.MULTILINE)
    ...
    assert not any(
        source == "/run/acx-write" or target == "/run/acx-write" for source, target, _options in mount_matches
    )
...
def test_reader_accepts_snapshot_at_future_skew_boundary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_reader_rejects_snapshot_beyond_future_skew_boundary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tolerance = gpu_state.GPU_STATE_FUTURE_SKEW_SECONDS
    path = tmp_path / "gpu-state.json"
    path.write_text(
