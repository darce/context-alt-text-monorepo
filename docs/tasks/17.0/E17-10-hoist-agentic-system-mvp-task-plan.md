...
- **The `mcp-` prefix is reserved for repos that ship an MCP server binary**; shared-surface and CLI repos do not carry it. The canonical four-repo family is therefore `darce/mcp-agent-handoff` and `darce/mcp-agent-orchestrator` (MCP servers), plus `darce/agentic-system` (passive symlink surface for skills/hooks/contracts/prompts; no MCP tools) and `darce/agentic-bootstrap` (pip console-script CLI; no MCP tools). E17-10 originally assumed the handoff repo already existed from AHMCP-5, but remote verification on 2026-04-19 showed that assumption was stale. Slice 0 must therefore create or recreate `darce/mcp-agent-handoff` alongside the three newly introduced repos. The bootstrap pip package is named `agentic-bootstrap` and matches its repo name. This rule is mirrored by the canonical paragraph in `docs/agentic/rules/development-workflow.md § Shared Agentic Surface` and the doc-lock test at `scripts/test_consumer_setup_doc.py`; do not reintroduce the deprecated `mcp-agentic-*` names.
...
- [x] Slice 0: `darce/mcp-agent-handoff` private repo exists remotely again via `gh`; if absent, it is recreated before Slice 1 release tagging and install-path proof. (commit `2ba563f1` provisions repo; populated + tagged in P1 commit `7c839726`.)
- [x] Slice 0: `darce/agentic-system` private repo created via `gh`; initial commit mirrors shared surface from monorepo `main` via the `rsync --archive --delete` + `git init` + commit recipe (no `git subtree split`); `v0.1.0` tag pushed. (commit `2ba563f1` provisions; commit `7a1cbcf2` extracts content + tags v0.1.0.)
- [x] Slice 0: `darce/mcp-agent-orchestrator` private repo created via `gh`; initial commit mirrors `packages/agent-orchestrator-mcp/` from monorepo `main`; release metadata and `v0.1.0` tag land in Slice 1. (commit `2ba563f1` provisions; commit `841b8fb2` extracts content + tags v0.1.0.)
- [x] Slice 0: `darce/agentic-bootstrap` private repo created via `gh`; README-only initial commit seeds the standalone repo; CLI implementation and first package tag `v0.1.0` land in Slice 3. (commit `2ba563f1` provisions; CLI implementation pending in standalone repo.)
...
- [x] Slice 1: `agent-handoff-mcp` and `agent-orchestrator-mcp` version `0.1.0` tagged on their respective **standalone** `darce/mcp-agent-handoff` + `darce/mcp-agent-orchestrator` repos; `CHANGELOG.md` entries written in each repo. (commits `7c839726`, `841b8fb2`; remote smoke proofs tightened in `1ffb9fec`.)
- [x] Slice 1: orchestrator `pyproject.toml` retargets its `agent-handoff-mcp` dependency from the editable sibling path to `git+ssh://git@github.com/darce/mcp-agent-handoff.git@v0.1.0`; transitive-install smoke test green. (Pinned in `packages/agent-orchestrator-mcp/pyproject.toml`; transitive-install proven by tightened P2 smoke in commit `1ffb9fec`.)
- [x] Slice 1: each standalone MCP repo hosts its own `scripts/release_mcp_package.sh` that emits `git+ssh://git@github.com/darce/<repo>.git@v<tag>` URLs (no `#subdirectory=` fragment, no `context-alt-text-monorepo` URL); `[tool.hoisted]` table in each `pyproject.toml` documents the standalone install URL.
- [x] Slice 2: `scripts/overlay_resolver.py` returns tagged `ResolvedPath` entries across skills/hooks/commands/prompts/contracts, with the hook surface covering `.github/hooks/`, `scripts/hooks/`, and `scripts/hooks/git/`. (commits `b2d5f532`, `96af23d6`.)
- [x] Slice 2: `check-skills` and `check-harness-sync` consume the overlay; unit tests cover shared-only, local-only, overlapping, `BrokenOverlayError` on broken symlink, and YAML replace-semantics. (commits `8d89e721`, `c7d0f10b`, `5ce1d2f9`.)
- [x] Slice 2: `.agentic-overlay.json` schema documented in `docs/agentic/contracts/overlay-manifest.yaml` (or equivalent); round-trip test green. (commit `e4aedeee`.)
- [x] Slice 2: `scripts/lint_hoisted_paths.py` + `make lint-hoisted-paths` added to statically catch monorepo-only path leakage in hoisted surfaces. (commit `44e2fffd`.)
- [x] Slice 3: `agentic-bootstrap` (developed in the standalone `darce/agentic-bootstrap` repo created in Slice 0) with `install`, `update`, `status`, `doctor`, `repair` commands and a standalone package tag pushed from that repo; zero runtime deps beyond stdlib + `tomllib` (reader) + `tomlkit` (round-trip writer). `tomli_w` is explicitly rejected because it cannot preserve comments. (Shipped at `v0.2.0` on `mcp-agentic-bootstrap@d5f43300`; decisions #2198, #2200–#2207, #2210.)
- [x] Slice 3: four config writers implemented (`claude_mcp_config`, `vscode_mcp_config`, `codex_mcp_config`, `git_hook_config`) with per-writer unit tests for merge semantics, idempotency/value stability, and default symbolic console-script commands. The `codex_mcp_config` writer uses `tomlkit` so comments and key order in existing `.codex/config.toml` files are preserved across re-runs. (Shipped in `mcp-agentic-bootstrap@7739ffcc`, decision #2202; `--mcp-servers` flag added in #2207.)
- [x] Slice 3: `agentic-bootstrap repair` unit tests cover missing-remote, corrupt-clone, SHA-mismatch, healthy-noop, **and dirty-clone guard (rg-017): abort-without-`--force-dirty`** (no destructive filesystem op; error names the dirty files and the `--force-dirty` flag) and **proceed-with-`--force-dirty`**. (Shipped in `mcp-agentic-bootstrap@efd2f8e0`, decision #2206.)
- [x] Slice 3: `BrokenOverlayError` raised consistently by `doctor`, `repair`, `check-skills`, and `check-harness-sync` when overlay is broken; error message names `agentic-bootstrap repair` as the fix. (Monorepo half via `scripts/overlay_resolver.py` since commit `8d89e721`; bootstrap CLI half via decisions #2204 (`doctor`) + #2206 (`repair`).)
- [x] Slice 3: `docs/agentic/consumer-setup.md` written and self-contained. (commit `fa92bf9a`; doc-lock guard `scripts/test_consumer_setup_doc.py` added.)
- [x] Slice 3: consumer overlay includes `.github/hooks/`, `scripts/hooks/`, and `scripts/hooks/git/`; generated config never references monorepo-local `scripts/mcp/mcp-server.sh`, and bootstrap wiring sets `core.hooksPath` to the overlaid git-hook subtree. (Monorepo half via `scripts/overlay_resolver.py` `_iter_hook_entries` since commit `96af23d6`; bootstrap CLI `git_hook_config` writer shipped in decision #2202.)
