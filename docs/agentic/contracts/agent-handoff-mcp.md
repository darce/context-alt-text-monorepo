...
- `set_handoff_state` and `record_event(event_kind="decision")` now emit non-fatal `context_drift` warnings in the response envelope when the resolved actor branch differs from the active task's `target_branch`, or when the current process working directory differs from the active task's `target_worktree_path`. The write still succeeds; the warning surfaces drift so cross-agent handoffs do not silently land work in the wrong context.
