...
- **Concrete resilience tunables with fail-safe validation** (rg-008). Defaults: backoff `base=60s`, per-attempt `cap=3600s` (1h), `retry_window=24h` (primary terminal), `hard_cap=30` attempts (backstop); conflict-storm threshold fires when curated divergence is `>= 20` entities (absolute) **OR** `> 50%` of the tenant's curated clusters. **Window-first by construction:** with these values the exponential ramp is `60,120,240,480,960,1920` then `3600` per attempt, so cumulative wall-clock reaches the 24h window at ~attempt 24 — *before* `hard_cap=30` (~30h) — making the window the terminal that actually fires and the count a true backstop for a wedged clock. (`hard_cap=12`, cumulative ~6h, would make count primary and the window dead config — do not use it.) All are `apply_filters`-overridable, but each filtered value is validated at read time (finite, numeric, `> 0`; `hard_cap >= 1`) and falls back to the default when a filter returns an invalid value — a bad filter can never disable the terminal ceiling or the storm cap.
...
| backend | `src/sovereign/sync/class-snapshot-projector.php` | Pre-count curated **cluster AND member** divergence before the member merge (`:490/:516`, `:131`); above threshold enter degraded mode: record one aggregate conflict (counts + `backend_version` + affected `entity_key` set) + suppress the per-entity cluster storm; thread a suppression flag into the member merge |
| backend | `src/sovereign/repositories/class-identity-member-snapshot-merger.php` | Accept + honor the storm-suppression flag at `:100` so the member merge skips per-entity recording in a suppressed cycle |
| backend | `src/sovereign/repositories/class-member-conflict-recorder.php` | Honor the suppression flag: skip `curated_member_deleted` (`:78`) + `member_cluster_reassignment` (`:48`) per-entity rows while the projector suppresses; expose the divergence entity set to the projector's aggregate payload |
...
| backend | `src/sovereign/sync/class-conflict-resolution-service.php` | Add a `backend_roster_regressed` branch to `resolve_projection_acceptance()` (`:327`) iterating the persisted `entity_key` set, supporting **`restore_local`** (default; re-enqueue local curation) and **`accept_backend`** (apply per-entity deletes/reassignments). Without it the code falls through to `return false` → `entity_mutation_failed` (outbox_id=0 path), so the conflict can never clear |
...
- Concrete defaults (all `apply_filters`-overridable, each **validated fail-safe at read time**, rg-008): `base=60s`, `cap=3600s`, `retry_window=24h` (`86400s`, **primary terminal**), `hard_cap=30` (backstop; see Constraints for why `12` would make the window dead config). A filter returning a non-finite/non-positive value falls back to the default — the terminal ceiling cannot be disabled.
...
- **Record one aggregate `backend_roster_regressed` conflict** carrying `backend_version`, per-code counts, **and the affected `entity_key` set** (PR-03) — the resolver acts per `entity_key`, so counts alone cannot drive a data-applying resolution; the aggregate `machine_payload` must persist the divergence entity list (or a re-derivable cursor) or the resolution can only be a no-op.
...
- **Wire two resolution directions (PR-04, PR-07):** `ConflictResolutionService::resolve_projection_acceptance()` (`class-conflict-resolution-service.php:327`) is a per-`conflict_code` whitelist; an unrecognized code falls through to `return false` → `entity_mutation_failed` (**not** `resolution_not_allowed`, which only fires for an invalid resolution string or an outbox-tied non-whitelisted op — a projector aggregate has `outbox_id=0`). Add a `backend_roster_regressed` branch that iterates the persisted `entity_key` set and supports **both** directions:
  - **`restore_local` (primary/recommended default):** the signal is "backend rolled back," so local curation is the good copy — re-assert local curation (re-enqueue the affected local deltas to the outbox for re-push) and clear the aggregate conflict. No local data loss.
  - **`accept_backend` (secondary):** accept the regressed backend roster as truth — apply the per-entity deletes/reassignments across the persisted entity set (the same mutations the per-code branches use) and clear the conflict.
  - Reuse `ConflictResolutionStatus` — no new magic strings.
...
- PHP resolution test — `restore_local`: `resolve()` returns `ok`, the row → resolved, **and the affected local curated entities are preserved / re-enqueued** (assert the post-resolution local state kept curation and outbox has the re-push ops) — not a status-only no-op.
...
- [ ] Projector pre-counts curated cluster **and** member divergence; threshold (`>=20` OR `>50%`) + aggregate conflict (counts + `backend_version` + entity_key set) implemented.
...
- [ ] `backend_roster_regressed` resolution wired into `ConflictResolutionService::resolve_projection_acceptance()` (both `restore_local` default + `accept_backend`; current-state failure is `entity_mutation_failed`, not `resolution_not_allowed`).
