...
> **Metadata**
>
> - **Date**: 2026-07-17 (r2 — revised against planning-review run `planning-review-uxp3-20260717-r1`, verdict fail, findings `UXP-3-PR-01..15` in handoff)
...
- **Behavior changes are named, tested, and recorded** ([API-09]) — never smuggled into a refactor slice.
...
- Heuristics: `heuristics-canon` `interaction-ux.md` ([COG-02], [PERC-02], [PERC-05], [FORM-05], [HAI-02]), `accessibility.md` ([A11Y-04], [A11Y-06], [A11Y-21], [A11Y-24]), `engineering.md` ([API-09], [API-10], [TEST-03], [TEST-06]); repo constitution sr-004/005/007, rg-015. Every ID verified against the lexicon row before citation; canon clone `~/Development/heuristics-canon` @`bcc23fa`+.
...
| Dropdown eligibility | frontend | shows any truthy-label target | **deliberate behavior change**: auto-label targets (`cluster-*`) stop rendering on identity-keyed surfaces ([API-09] reasoning in D2) | dropdown + inline tests with `cluster-*` fixture rows |
...
2. **0b-1 — inline leg** (post-UXP-2 merge): `useInlineSuggestionBatch` fetches at `PROJECTION_TOP_K`, filters via predicate, keys under `projection.identityBatch`, surfaces top-1. The `cluster-*` visibility change lands and is tested here.
3. **0b-2 — dropdown leg**: `useClusterSuggestionsLoader` swaps `fetchIdentitySuggestions` for a single-id batch call through the projection at the same depth; `useClusterSuggestions` consumes `ProjectedSuggestion[]`; `identityFor` key deleted. Inline and dropdown now share transport, depth, predicate, and order by construction — the PR-01 divergence is structurally closed, tested on the window-depth fixture row.
4. **0b-3 — review-queue leg**: `useSuggestionReviewQueries.assignmentQuery` → `projection.reviewPage(0)`; `buildSuggestionReviewItems` consumes `ProjectedSuggestion` (predicate applied idempotently — server already filters stricter) keeping its **per-cluster** grouping (PR-14) with `compareSuggestions` for presentation order; **optimistic paths re-keyed**: `setQueryData`/`cancelQueries`/rollback in `useSuggestionReviewMutations` move from `pending()` to `projection.reviewPage(0)` with a rollback-after-failed-accept test (PR-08); `pending` key deleted.
5. **0b-4 — invalidation sweep**: every site in the D4 inventory routes assignment-family invalidation through `invalidateSuggestionProjection` while keeping the enumerated cross-family calls; per-event tests assert the D4 table row exactly (assignment) and presence (cross-family).
6. **0b-5 — proxy cleanup**: drop the dead `threshold` from `class-suggestions-controller.php` per-identity forward, with a PHP test on the forwarded query args.
...
- **Shared option builder** `buildNamingOptions(rosterEntries, labelMatches)` (new, in `suggestionProjection.ts`'s sibling module or beside the panel): value, display label, `source: 'person' | 'cluster'`, deduped case-insensitively **preferring the person entry** (its casing is canonical), persons listed before clusters ([COG-02] — recognition over recall; [PERC-02] — badge only encodes the real behavioral difference: person = names an entity, cluster = merges into an existing cluster). Filter-before-slice: the union is prefix-filtered, then truncated.
- **Panel** (`ClusterLabelingPanel.tsx`): Combobox options = union (today roster-only). Selecting a cluster entry routes to the existing merge path; selecting a person keeps the person's canonical casing.
...
- **Duplicate guard** (PR-11): the backend never emits a duplicate-label 409 (`clusters_topology.py` has no such check), so the panel's 409-driven `duplicateMatch` "Is this X?" branch is dead code — **deleted** (greenfield). Replaced by a **pre-save** check in the save path: exact case-insensitive match in the union (excluding the editable cluster) blocks with an inline warning at the field ([FORM-05]) offering explicit *merge into existing* / *rename anyway* / *cancel*. No silent create, no silent merge.
...
4. **Feedback**: successful case-only rename shows saved state inline at the field ([PERC-05], [FORM-05]) and is announced ([A11Y-21]).
...
| 1 | `identity-clusters/buildNamingOptions.ts` (**new**) ; `ClusterLabelingPanel.tsx` ; `useClusterSuggestionsLoader.ts` | union source, badges, pre-save duplicate guard, delete dead 409 `duplicateMatch` branch |
...
Proof: `buildNamingOptions` unit tests (union, person-preferred case-insensitive dedupe, persons-first ordering, filter-before-slice with a match beyond the slice boundary); panel and loader tests consuming the same builder; duplicate-guard grid (existing person / existing cluster / self-case-only → no block); dead-409-branch deletion compiles with no `duplicateMatch` references; pin control and plumbing absent (`IdentityClusterItem` renders without pin props); a11y assertions ([A11Y-04]/[A11Y-21]/[A11Y-24] states).
...
- [ ] 0b-1 inline: depth 5 + predicate + projection key; `cluster-*` change pinned by test.
- [ ] 0b-2 dropdown: single-id batch at same depth; `identityFor` deleted; no consumer left on `fetchIdentitySuggestions`; route-retirement follow-up recorded for UXP-2 absorption, not implemented.
- [ ] 0b-3 review: projection key + `ProjectedSuggestion`; per-cluster grouping kept; **optimistic setQueryData/cancelQueries/rollback re-keyed** with rollback-after-failed-accept test; `pending` deleted.
...
- [ ] `buildNamingOptions` single definition consumed by panel and loader; persons first; person-preferred case-insensitive dedupe; filter-before-slice.
- [ ] Pre-save duplicate guard with explicit merge/rename/cancel; dead 409 `duplicateMatch` branch deleted.
