...
class TestDiscoverArtifactRefs:
    ...
    def test_brief_messages_payload_artifacts_found(self) -> None:
        ...
        discover = self._subject()
        activity = {'messages': [{'subject': 'brief:context for backend lane', 'message': 'See attached artifact.', 'payload': {'source_lane': 'frontend', 'reason': 'context', 'artifacts': ['42']}}]}
...
class TestBuildArtifactQueries:
    ...
    def test_brief_payload_summary_used_as_query(self) -> None:
        ...
        bq = self._subject()
        activity = {'messages': [{'subject': 'brief:implement retention', 'message': 'Summary: retention policy.', 'payload': {'source_lane': 'orch', 'reason': 'implement retention', 'summary': 'Retention export endpoint'}}]}
        queries = bq(activity)
        assert any(('Retention export endpoint' in q for q in queries))
    ...
    def test_brief_body_excluded_from_message_pass(self) -> None:
        ...
        bq = self._subject()
        activity = {'messages': [{'subject': 'brief:do something', 'message': 'do something', 'payload': {'source_lane': 'orch', 'reason': 'do something', 'summary': 'Specific brief summary'}}, {'subject': None, 'message': 'regular assignment'}]}
        queries = bq(activity)
    ...
    def test_cap_at_four_queries(self) -> None:
        ...
        bq = self._subject()
        activity = {'messages': [{'message': f'task {i}'} for i in range(10)], 'blockers': [{'description': f'blocker {i}'} for i in range(3)]}
...
class TestArtifactContextSection:
    ...
    def test_returns_empty_when_search_unavailable(self) -> None:
        ...
        try:
            ...
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(), budget_chars=10000)
            assert lines == []
        finally:
    ...
    def test_pinned_ref_rendered_first(self) -> None:
        lp = _load_lane_prompt()
        mock_source_response = json.dumps({'ok': True, 'data': {'source': {'id': 7, 'source_label': 'pytest-output', 'summary': 'Test summary of the output', 'chunks': [{'chunk_order': 1, 'title': 'Run', 'body': 'Test body text'}]}}})
        mock_search_response = json.dumps({'ok': True, 'data': {'hits': []}})
        with patch.object(lp, '_mcp_get_artifact', return_value=mock_source_response), patch.object(lp, '_mcp_search_artifacts', return_value=mock_search_response), patch.object(lp, '_ARTIFACT_SEARCH_AVAILABLE', True):
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(artifact_ids=[7]), budget_chars=10000)
        assert len(lines) == 1
    ...
    def test_fts_fallback_when_no_pinned_refs(self) -> None:
        lp = _load_lane_prompt()
        mock_search_response = json.dumps({'ok': True, 'data': {'hits': [{'source_id': 3, 'source_label': 'migration-log', 'title': 'DB migration', 'snippet': 'Applied 3 migrations'}]}})
    ...
    def test_fts_results_skip_already_rendered_pinned_ids(self) -> None:
        ...
        lp = _load_lane_prompt()
        mock_source_response = json.dumps({'ok': True, 'data': {'source': {'id': 5, 'source_label': 'output', 'summary': 'Pinned summary', 'chunks': []}}})
        mock_search_response = json.dumps({'ok': True, 'data': {'hits': [{'source_id': 5, 'source_label': 'output', 'title': 'FTS hit title', 'snippet': 'FTS snippet'}]}})
        with patch.object(lp, '_mcp_get_artifact', return_value=mock_source_response), patch.object(lp, '_mcp_search_artifacts', return_value=mock_search_response), patch.object(lp, '_ARTIFACT_SEARCH_AVAILABLE', True):
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(artifact_ids=[5]), budget_chars=10000)
        assert len(lines) == 1
    ...
    def test_budget_limits_total_rendered_chars(self) -> None:
        lp = _load_lane_prompt()
        long_summary = 'A' * 500
        mock_source_response = json.dumps({'ok': True, 'data': {'source': {'id': 1, 'source_label': 'big-log', 'summary': long_summary, 'chunks': []}}})
        mock_search_response = json.dumps({'ok': True, 'data': {'hits': []}})
        with patch.object(lp, '_mcp_get_artifact', return_value=mock_source_response), patch.object(lp, '_mcp_search_artifacts', return_value=mock_search_response), patch.object(lp, '_ARTIFACT_SEARCH_AVAILABLE', True):
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(artifact_ids=[1]), budget_chars=50)
        assert lines == []
    ...
    def test_mcp_exception_does_not_raise(self) -> None:
        ...
        with patch.object(lp, '_mcp_get_artifact', side_effect=_raise), patch.object(lp, '_mcp_search_artifacts', side_effect=_raise), patch.object(lp, '_ARTIFACT_SEARCH_AVAILABLE', True):
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(artifact_ids=[1]), budget_chars=10000)
        assert lines == []

    def test_pinned_ref_uses_first_chunk_body_when_no_summary(self) -> None:
        lp = _load_lane_prompt()
        mock_source_response = json.dumps({'ok': True, 'data': {'source': {'id': 2, 'source_label': 'test-log', 'summary': '', 'chunks': [{'chunk_order': 1, 'title': 'Head', 'body': 'Chunk body fallback'}]}}})
        mock_search_response = json.dumps({'ok': True, 'data': {'hits': []}})
        with patch.object(lp, '_mcp_get_artifact', return_value=mock_source_response), patch.object(lp, '_mcp_search_artifacts', return_value=mock_search_response), patch.object(lp, '_ARTIFACT_SEARCH_AVAILABLE', True):
            lines = lp._artifact_context_section(task_ref='t', lane_id='l', activity=self._make_activity(artifact_ids=[2]), budget_chars=10000)
        assert len(lines) == 1
