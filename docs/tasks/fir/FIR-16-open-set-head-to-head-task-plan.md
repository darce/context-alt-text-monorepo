...
- **`GateSummary`** (DD-05): the per-stack return shape `score_open_set` produces — `points: tuple[IETPoint, ...]` (the full tau sweep, not just the selected point), `selected: IETPoint | None`, `tau: float | None`, `probe_outcomes: dict[str, bool]` keyed `f"{media_id}:{probe_index}"`. This is a new dataclass in `scripts/bench/score.py`, NOT an extension of `fir_bakeoff_run.RunReport` (DD-05 — `RunReport` stays FIR-12's own shape; the bench layer wraps it rather than mutating it).
- **`match_cluster_id`** (DD-03): the cluster id the best-centroid match resolved to, captured in `CentroidDiscovery.discover` (`recognition/application/discovery/centroid.py:30-60`) BEFORE the `similarity_threshold` filter — i.e. for every face with at least one candidate cluster, whether or not the face was ultimately assigned. Persisted alongside `match_score` on `MediaIdentity`; distinct from `cluster_label` (the assigned/roster label after the gate), which may be `None` even when `match_cluster_id` is set.
...
- **Export path correction (DD-03)**: `_serialize_identity` alone does not reach `export_map.py` — the bench's actual read path is `MediaIdentityService.list_by_media_ids` (`recognition/interface_adapters/http/deps/stores.py:165–284`) → `RemoteSceneClient.media_identities` (`scripts/eval_harness/remote_client.py:212–218`) → `export_map.py`. S1a must add `match_score`/`match_cluster_id` to the row shape at `list_by_media_ids`, not merely to `_serialize_identity`'s dict, or the fields never reach the bench.
- **`_find_best_centroid_match` bug (DD-03)**: `CentroidDiscovery._find_best_centroid_match` (`recognition/application/discovery/centroid.py:62–87`) initializes `best_cluster: str | None = None` and `best_similarity = 0.0`, then only updates on `similarity > best_similarity` — an all-negative-similarity gallery silently returns `(None, 0.0)` (a false "no match" rather than the true best-of-a-bad-lot negative score), and an empty `centroids_by_cluster` also returns `(None, 0.0)` rather than a distinguishable "no gallery" signal. `discover()` (30–60) calls this BEFORE filtering on `similarity_threshold` (49), so the raw score is available pre-filter — this is where `match_cluster_id`/`match_score` capture must hook in (S1a).
- FIR-13 (`docs/tasks/fir/FIR-13-open-set-gate-contract-task-plan.md`, drafted 2026-09-11) specifies `scripts/eval_harness/gate_contract.py` — **not yet implemented** (`check_index_coverage` on that path reports `freshness: missing` per FIR-13's own Current State Analysis): `GateContract` (`metric`, `max_fpi: int | None`, `n_nonmated_declared: int | None`, `rubric_version`, `ratified_by_decision_id: str | None`, `t14_thresholds_declared: dict[str, float] | None`, `t14_thresholds_sha256: str | None` — all seven keys required, `null` until ratified), `GateContractError`, `load_gate_contract(path: str | Path) -> GateContract`, `select_gate_point(points: Sequence[IETPoint], *, max_fpi: int) -> IETPoint | None`. `GateSummary` is **this** plan's dataclass in `scripts/bench/score.py` (DD-05), not FIR-13's. Every reference to `gate_contract.*` in this plan is **new, per FIR-13** until that task lands.
...
| --- | --- | --- | --- | --- | --- |
| Bench ↔ recognition public export | `scripts/bench/` (client) / recognition service (server) | FIR-8 consumption table: `/ready`, `/health/detailed`, tenant export routes; `media_identities` rows `{media_id, identity_id, bbox, cluster_label, is_auto_label}` | S1a adds two additive fields, `match_score: float | None` and `match_cluster_id: str | None` (DD-03), to the row returned by `MediaIdentityService.list_by_media_ids` and carried through `RemoteSceneClient.media_identities` to the exported identity row | Yes — additive-only; existing FIR-8 consumers (`to_face_metric_inputs` etc.) must not break on the new keys | `recognition/tests/api/test_media_identities_match_score.py::test_list_by_media_ids_round_trips_match_score`, `::test_rejected_face_keeps_best_centroid_score`; `scripts/bench/tests/test_export_map_match_score.py::test_export_rows_carry_match_score_and_cluster` |
...
S1a adds the two missing fields — a persisted per-face match score and its resolved cluster id, captured pre-filter in `CentroidDiscovery.discover` (DD-03) — to the production export so an open-set score exists to sweep. S1b wires `scripts/bench/score.py::score_open_set` from those fields through `fir_bakeoff_run.score_run`, once per stack (never pooling the two embedding spaces), returning a `GateSummary` per stack that carries the full tau sweep rather than extending `RunReport` (DD-05). S2 renders the result as an `open_set` report section carrying the NI verdict and both banners. S3 gives the operator a `propose-gate` CLI and the exact `record_event` decision template. S4 is the live runbook, hard-gated on FIR23-STACK existing and on FIR-6 S4 calibration having run, and builds an explicit enrollment-only gallery reproducing the frozen `RunPlan` G1 so no probe leaks into the gallery during the run (DD-18).

## Files and Surfaces to Change

| Surface | File | Change |
| --- | --- | --- |
| recognition (production) | `apps/prototype-description-service/db/models/identity.py` | `MediaIdentity` (41–105) gains `match_score: Mapped[float \| None] = mapped_column(Float, nullable=True)` and `match_cluster_id: Mapped[str \| None] = mapped_column(String, nullable=True)` (DD-03) — the best-centroid similarity and cluster id `CentroidDiscovery._find_best_centroid_match` (`recognition/application/discovery/centroid.py` 62–87) returns for the face, captured in `discover()` (30–60) BEFORE the `similarity_threshold` filter (49), persisted for EVERY detected face whether or not the confidence gate accepted it (an open-set sweep needs the score below the production threshold too, or the IET curve is truncated at the deployed tau); both `NULL` only when no centroid existed to compare against (empty gallery). Greenfield: edit `001_identity_schema.py` directly, no migration chain. |
| recognition (production) | `apps/prototype-description-service/recognition/application/discovery/centroid.py` | `_find_best_centroid_match` (62–87) fixed: `best_similarity` initialized to `float("-inf")` (not `0.0`) so an all-negative-similarity gallery still returns its true best match instead of a false `(None, 0.0)` "no match"; empty `centroids_by_cluster` explicitly returns `(None, None)` rather than `(None, 0.0)` so "no gallery" is distinguishable from "gallery present, worst-possible match" (DD-03). |
| recognition (production) | `apps/prototype-description-service/recognition/application/assignment/checks/confidence.py` | `ConfidenceCheck.evaluate` (91–242) is NOT the capture point (DD-03 — the raw score is already available earlier, in `discover()` pre-filter); this file is unchanged by S1a. |
| recognition (production) | `apps/prototype-description-service/recognition/interface_adapters/http/deps/stores.py` | `MediaIdentityService.list_by_media_ids` (165–284) adds `match_score`/`match_cluster_id` to the returned row shape (DD-03 — this, not `_serialize_identity` alone, is the path the bench actually reads through). |
| recognition (production) | `apps/prototype-description-service/recognition/application/services/export_service.py` | `_serialize_identity` (295–317) adds `"match_score": identity.match_score, "match_cluster_id": identity.match_cluster_id` to the emitted dict (kept in sync with `list_by_media_ids`, but not itself the bench's read path). |
| eval-harness | `apps/prototype-description-service/scripts/eval_harness/remote_client.py` | `RemoteSceneClient.media_identities` (212–218) passes the two new fields through unchanged (no reshaping needed; verified it already forwards whatever `list_by_media_ids` returns). |
| eval-harness | `apps/prototype-description-service/scripts/bench/export_map.py` | New `to_open_set_searches(export: LegExport, manifest, join: dict[int, dict], *, roster: Sequence[str], gallery: GalleryName) -> list[SearchResult]` (see Slice 1); `top1_name` derived from `match_cluster_id`'s enrollment-gallery name, never a GT-derived optimistic label (DD-03). |
| eval-harness | `apps/prototype-description-service/scripts/bench/score.py` | New `GateSummary` dataclass (`points: tuple[IETPoint, ...]`, `selected: IETPoint \| None`, `tau: float \| None`, `probe_outcomes: dict[str, bool]`) and `score_open_set(searches: Mapping[str, list[SearchResult]], *, plan_by_stack: Mapping[str, RunPlan], tau_grid: Sequence[float]) -> dict[str, GateSummary]` (DD-05 — wraps `RunReport`, does not extend it). |
| eval-harness | `apps/prototype-description-service/scripts/bench/score_report.py` | New `score_open_set_report(run_dir: Path \| str) -> Path`, `render_open_set_section(summaries: dict[str, GateSummary], *, contract: GateContract) -> dict[str, Any]`, `PowerCeilingBanner` dataclass + `format_power_ceiling_banner(...)`, reuse of the existing licence banner text. |
...
| tests | `apps/prototype-description-service/scripts/bench/tests/test_score_open_set.py` | New — covers `GateSummary` shape and per-stack sweep. |
...
| tests | `apps/prototype-description-service/recognition/tests/api/test_media_identities_match_score.py` | New (replaces `scene/tests/test_export_service_match_score.py`) — `test_list_by_media_ids_round_trips_match_score`, `test_rejected_face_keeps_best_centroid_score` (DD-03). |
| tests | `apps/prototype-description-service/recognition/tests/unit/test_centroid_discovery.py` | New/extended cases: all-negative-similarity gallery still returns the true best match; empty gallery returns `(None, None)` (DD-03). |
| tests | `apps/prototype-description-service/scripts/bench/tests/test_export_map_match_score.py` | New — `test_export_rows_carry_match_score_and_cluster` (DD-03). |
...
| `apps/prototype-description-service/scripts/eval_harness/fir_bakeoff_run.py` | `score_run` (410–536), `RunReport` (183–260), `MatedSearchUnit` (152–163), `GALLERY_STRATUM`/`PROBE_STRATA`, `build_run_plan` (263–288), `_validate_search_against_entry` (715–740). |
...
- Deterministic tests:
  - `uv run --extra dev pytest scripts/bench/tests/test_score_open_set.py scripts/bench/tests/test_score_report_open_set.py scripts/bench/tests/test_propose_gate.py scripts/bench/tests/test_export_map_match_score.py scripts/bench/tests/test_gallery_leakage.py -q` (run from `apps/prototype-description-service`)
  - `uv run --extra dev pytest recognition/tests/api/test_media_identities_match_score.py recognition/tests/unit/test_centroid_discovery.py -q`
...
- Contract/fixture verification: a `score_open_set` fixture asserts a `top1_score=None` search never becomes FPI (mirrors `open_set_identification._is_fpi`'s own invariant); a `propose-gate` fixture asserts non-zero exit when `contract.ratified_by_decision_id is None`; a `test_gallery_leakage` fixture asserts a probe search never mutates the frozen G1 gallery's centroid set (DD-18).
...
Implements: FIRG-059 (S1a), FIRG-005 and FIRG-060..061 (S1b).
**Goal**: Both stacks can be scored for FNIR@FPIR from their exports, each at its own swept tau.

Changes:

- **S1a — persisted match score and cluster id (prerequisite, DD-03)**: `db/models/identity.py` `MediaIdentity` gains `match_score: Mapped[float | None]` and `match_cluster_id: Mapped[str | None]` (both nullable, no check constraint); `001_identity_schema.py` edited directly (greenfield, no migration chain). Capture point is `CentroidDiscovery.discover` (`recognition/application/discovery/centroid.py:30–60`), BEFORE the `similarity_threshold` filter at line 49 — every detected face with at least one candidate centroid gets `match_score`/`match_cluster_id` written from `_find_best_centroid_match`'s return, regardless of the accept decision (an open-set sweep needs the score below the production threshold too, or the IET curve is truncated at the deployed tau); `NULL`/`NULL` only when `centroids_by_cluster` was empty. `ConfidenceCheck.evaluate` is NOT touched — it runs after discovery and never sees the pre-filter score. `_find_best_centroid_match` itself is fixed in the same slice: `best_similarity` starts at `float("-inf")` (was `0.0`, which silently masked an all-negative-similarity gallery as "no match"); an empty gallery returns `(None, None)` (was `(None, 0.0)`, indistinguishable from a real worst-case match). `list_by_media_ids` (`stores.py:165–284`) and `_serialize_identity` (`export_service.py:295–317`) both surface the two new fields.
- **S1b — field-by-field export → `SearchResult` mapping**, implemented as new `export_map.to_open_set_searches`:
  ...
  | `match_cluster_id` (new, S1a) resolved against the run's frozen enrollment gallery | `top1_name: str | None` | **DD-03**: `top1_name` is the enrollment-gallery name bound to `match_cluster_id` — never `cluster_label`/`map_cluster_labels_optimistic`'s GT-derived optimistic label, which this gate metric explicitly prohibits (an optimistic label leaks ground truth into what must be a blind identification decision); `None` when `match_cluster_id` is `None` or does not resolve to an enrolled name. |
  ...
  `score.py::score_open_set(searches: Mapping[str, list[SearchResult]], *, plan_by_stack: Mapping[str, RunPlan], tau_grid: Sequence[float]) -> dict[str, GateSummary]` (DD-05) builds one `RunPlan` **per stack** (never pooled — 512-D and 128-D are separate embedding spaces, EMB-01/IDX-02) by calling `fir_bakeoff_run.score_run(plan=plan_by_stack[stack_id], searches=..., tau=tau, ...)` once per candidate `tau` in `tau_grid`, collecting every resulting `IETPoint` into that stack's `GateSummary.points` (the full sweep, not just one selected tau), leaving `GateSummary.selected`/`.tau` as `None` until S2's `gate_contract.select_gate_point` call fills them in, and populating `GateSummary.probe_outcomes` (keyed `f"{media_id}:{probe_index}"`, value = mated-search pass/fail) from `score_run`'s per-search results. `RunReport` itself is unchanged — `GateSummary` wraps it, it does not extend it (DD-05).

Proof:

- `scripts/bench/tests/test_score_open_set.py` — fixtures: (a) a mated search with `top1_score=None` never registers FPI/FNIR-pass (mirrors `open_set_identification._is_fpi`); (b) two stacks scored from the same manifest never share a `tau`; (c) `to_open_set_searches` round-trips a synthetic export with `probe_only_identities` correctly producing `true_name=None` rows; (d) `to_open_set_searches` never derives `top1_name` from an optimistic/GT-based label; (e) `GateSummary.points` carries every swept tau, `probe_outcomes` keys match `f"{media_id}:{probe_index}"`.
- `uv run --extra dev pytest scripts/bench/tests/test_score_open_set.py -q`
- `uv run --extra dev pytest recognition/tests/api/test_media_identities_match_score.py -q` — `test_list_by_media_ids_round_trips_match_score`, `test_rejected_face_keeps_best_centroid_score`.
- `uv run --extra dev pytest recognition/tests/unit/test_centroid_discovery.py -q` — all-negative-similarity gallery and empty-gallery cases.
- `uv run --extra dev pytest scripts/bench/tests/test_export_map_match_score.py -q` — `test_export_rows_carry_match_score_and_cluster`.

### Slice 2: Head-to-head report (Phase C)

Implements: FIRG-062, FIRG-065.
**Goal**: One report renders both stacks' D3 numbers, a paired non-inferiority verdict, and the mandatory banners.

Changes:

- `score_report.py::score_open_set_report(run_dir: Path | str) -> Path` reads both stacks' `GateSummary`s (S1, DD-05) and the ratified `GateContract` (FIR-13), calls `gate_contract.select_gate_point(summary.points, max_fpi=contract.max_fpi)` per stack to fill in `GateSummary.selected`/`.tau`, and writes `benchmarks/results/crossbench-<stamp>/open_set.json` + `.md`.
- `render_open_set_section(summaries: dict[str, GateSummary], *, contract: GateContract) -> dict[str, Any]` emits, per stack: `{fnir_at_gate, tau, fpi, n_mated, n_nonmated, coverage_gaps, toolchain, contract, measured}`, reading `fnir_at_gate`/`fpi` off `summary.selected` (an `IETPoint`) and `tau` off `summary.tau` — `summary.selected is None` cells render `fnir_at_gate: null`, never `0.0` [AUDIT-04].
- Paired NI verdict: candidate vs incumbent FNIR at their respective gate points, via FIR-11 rev 7's Nam/Tango paired score test (`paired_ni_score_test`, `H0: Δ ≤ −δ`, one-sided α=0.025) — **not implemented by this task**; this section calls it as a dependency once FIR-11's `scripts/eval_harness/face_metrics.py` symbol lands (`[UNVERIFIED — confirm via codemap]`: `paired_ni_score_test` is listed "new" in FIR-11 rev 7's Files table but is not yet on disk under `scripts/eval_harness/`; verified absent via `search_graph`). Pairing keys off `GateSummary.probe_outcomes` (DD-05) — both stacks' outcome dicts are joined on the shared `f"{media_id}:{probe_index}"` key so the paired test compares the same probe across stacks, never a positional/unpaired join.
...
Implements: FIRG-063, FIRG-064; FIRG-006 jointly with FIR-13 S2 (operator ratification decision `firplan_d3_operating_point_<date>` — not D-08, which is the QA v8 decision to adopt D3 as the metric at all).
...
Supports: FIRG-060..065 (Phase D execution runbook; no new gate ids).
...
- New `docs/runbooks/fir-16-open-set-head-to-head-acx-dev-fir.md`:
  ...
  4. **Enrollment-only gallery, built explicitly (DD-18)**: before any probe runs, reproduce the frozen `RunPlan` G1 gallery (`fir_bakeoff_run.build_run_plan`, 263–288) against the live stack by enrolling exactly the plan's gallery entries — never letting probes contribute to enrollment. During the run, disable assignment auto-accept so no probe search updates a centroid mid-run (a probe that silently joined the gallery would leak into later probes' matches). Matedness for each probe is derived by replaying `fir_bakeoff_run._validate_search_against_entry` (715–740) against this frozen gallery, not by re-deriving it from whatever the live service happened to auto-assign.
  5. `cross_stack_bench.py run` → ingest → analyze → cluster → export on both stacks (`driver.run_pair`).
  6. `cross_stack_bench.py score` (existing detection/cluster legs) + the new open-set leg (S1/S2 of this task).
  7. `cross_stack_bench.py propose-gate` (S3) → operator reviews → records `fir_gate_decision_<date>`.
  8. `render_handoff(kind='dashboard')` after the decision is recorded.
  9. Failure routing: preflight failure → do not run; cluster-gate failure → do not score; gallery-leakage check failure (any probe found in the enrollment set post-run) → discard the run, do not score; NI section under-powered → tier capped at DIRECTIONAL, proposal still permitted but flagged.
...
- New `scripts/bench/tests/test_gallery_leakage.py::test_probe_never_joins_enrollment_gallery` (deterministic, offline fixture) — asserts that replaying a probe search against the frozen G1 gallery via `_validate_search_against_entry` never mutates the gallery's centroid set, i.e. the harness-side leakage guard is exercised even though the live S4 run itself is infrastructure-gated (DD-18).
...
- [ ] `MediaIdentity.match_score`/`match_cluster_id` added; captured in `CentroidDiscovery.discover` before the `similarity_threshold` filter (DD-03); `list_by_media_ids` and `export_service.py` both emit the two fields.
- [ ] `_find_best_centroid_match` fixed: `best_similarity` starts at `float("-inf")`; empty gallery returns `(None, None)` (DD-03).
- [ ] `export_map.to_open_set_searches` implemented per the field-by-field mapping table; `top1_name` derived from `match_cluster_id`'s enrollment name, never an optimistic/GT-derived label.
- [ ] `score.py.score_open_set` returns one `GateSummary` per stack (`points`, `selected`, `tau`, `probe_outcomes`); scores each stack independently, never pooling embedding spaces; `RunReport` itself is unmodified (DD-05).
...
- [ ] Runbook builds an explicit enrollment-only gallery reproducing frozen `RunPlan` G1 before any probe runs, disables auto-accept during the run, and derives matedness via `_validate_search_against_entry` against that gallery (DD-18).
- [ ] Failure routing table covers preflight, cluster-gate, gallery-leakage, and under-powered NI cases.
...
- [ ] `score_open_set` (returning `GateSummary` per stack, DD-05), `to_open_set_searches`, `score_open_set_report`, and `propose-gate` exist, are imported by the bench CLI, and pass their test suites.
- [ ] No probe ever joins the frozen enrollment gallery during the S4 live run (DD-18, leakage test green).
