...
- **Wire two resolution directions (PR-04, PR-07; revised for PR2-01/PR2-02):** the two directions take **structurally different paths** through `ConflictResolutionService::resolve()`:
  - **Reachability first (PR2-01):** `resolve()` whitelists the resolution *string* at `:72` (`accepted|dismissed|accept_backend|merge`) **before** any `conflict_code` logic, and `resolve_projection_acceptance()` only runs under the `accepted|accept_backend` branch (`:138`). `restore_local` must therefore be (a) added to the `:72` whitelist and (b) dispatched as its **own top-level branch** parallel to `merge`/`accept_backend` — a `restore_local` case inside `resolve_projection_acceptance()` is dead code by construction.
  - **`restore_local` (primary/recommended default; PR2-02):** the signal is "backend rolled back," so local curation is the good copy. There is **no existing re-enqueue primitive for this**: `re_enqueue_with_current_base()` (`class-outbox-maintenance-service.php:284`) requires an existing `outbox_id`, and a projector aggregate has `outbox_id=0`. The branch must **synthesize new outbox ops** via `OutboxWriter::enqueue()` (`class-outbox-writer.php:44`), injected as `OutboxWriterInterface` (fifth optional-defaulted ctor dep, `:45-55` — object seam for tests, [TEST-04]/[REF-15]). Per `entity_key` in the persisted aggregate set: read the *current* curated state (curated cluster label via `ClustersRepositoryInterface`; member assignment via `IdentityMembersRepositoryInterface` — both already injected), rebuild `operation_type` + payload in the same shape `ClusterMutationsController` enqueues for that entity type, and derive the idempotency key with the same derivation contract as `derive_curation_idempotency_key()` (`class-cluster-mutations-controller.php:352`) — extract that derivation into a shared helper rather than duplicating it ([NAME-05]) — so a replayed resolution cannot double-apply ([RES-01], [DATA-13], [API-02]). An `entity_key` whose local curated state no longer exists is **skipped, counted, and reported** in the result — a partially-restorable set must not fail the whole resolution ([REF-20]). This makes `restore_local` materially heavier than `accept_backend`; the weight is the cost of the no-data-loss default.
...
- [ ] `restore_local` reachable end-to-end: `:72` whitelist entry + top-level dispatch branch + `OutboxWriterInterface` ctor injection + per-entity op synthesis with shared idempotency-key derivation; missing local state skipped-and-reported, not fatal.
