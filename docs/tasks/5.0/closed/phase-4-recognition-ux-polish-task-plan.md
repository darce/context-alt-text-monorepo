...
| `js/admin/pages/RosterPage.tsx`                | L117-135 | Replace `window.confirm` in `handleBulkMerge` with `ConfirmDialog` state. Use direct `confirmAction` state (`'merge' \| 'dismiss' \| null`) with local `handleConfirm` callback (async, closes on success). |
...
| `js/admin/pages/workbench/identity-clusters/useClusterConfirmDialog.ts` | Existing confirm dialog state hook. Roster uses a simpler direct-state approach instead (no promise-based flow needed for two-action dialogs). |
...
- [x] **Implement**: replace `window.confirm` in `RosterPage.handleBulkMerge` with `ConfirmDialog` state. Use direct `confirmAction` state (`'merge' | 'dismiss' | null`) with local `handleConfirm` callback that dispatches the correct mutation and closes on success.
