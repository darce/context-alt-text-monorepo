...
def _fetch_mcp_lane_params(orchestrator_root: Path, task_ref: str, lane_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        res = list_worktree_lanes(task_ref=task_ref)
        data = json.loads(res)
        if not data.get("ok"):
            return {}
...
def _record_token_usage_to_handoff(
    *,
    ...
    from agent_handoff_mcp.enums import normalize_model_label  # noqa: PLC0415
    ...
    try:
        ...
        config = RuntimeConfig.for_workspace(orchestrator_root)
        api.configure_runtime(config)
        from agent_orchestrator_mcp.lanes import record_turn_metric  # noqa: PLC0415
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--task-ref", required=True, help="MCP task reference (e.g. phase-5-retention-export-and-audit-controls)."
    )
    parser.add_argument("--lane-id", required=True, help="Lane identifier (e.g. backend-domain).")
