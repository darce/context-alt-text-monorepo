...
> - **Revision**: 4 — revision 2 answered astra/high plan review run `gpuflow2-plan-review-02-p1` (findings `GPUFLOW-2-PLAN-R-01`…`R-11` in handoff; verdict fail); revision 3 answered the local Claude canon review `gpuflow2-plan-review-03-local` (findings `GPUFLOW-2-PLAN-L-01`…`L-06`); revision 4 (user directive 2026-09-16) makes end-to-end GPU engagement from the demo workbench a guaranteed deliverable of this wave and adds the always-visible "Description Service is off" indication, after live evidence classified the cause (see Problem Statement §1). The VLM model is never hardcoded: `ACX_DESCRIPTION_ADAPTER` on the API host is the single source of truth and the release gate checks the returned `model_id` against it. No second astra pass; freeze follows this revision.
...
1. **The describe operation is not durable on the client, and the service cannot say why the GPU is absent.**
   ...
   - **Cause classified from live evidence (2026-09-16 23:01 UTC, DIAG-01/DIAG-02).** The lifecycle side is healthy: `acx-gpu-start.timer` fires every 30 s and `acx-gpu-reap.timer` every 2 min on acx-backend, both units read `--load-dir /run/acx-write` (globbing `*/describe-load.json` across `dev`/`prod`/`staging`), `/run/acx/gpu-state.json` is fresh and says `stopped` with the A10 instance OCID present, and no intent file exists. `GpuLifecycleController.start_needed_instances()` (`controller.py:71-97`) starts only when a load file shows `queue_depth>0`, `in_flight>0` or `batch_in_progress`. **Both public API hosts (`api.altcontext.com`, `staging.api.altcontext.com`) report `commit_sha 51ff7022` (2026-09-14, image variant `recognition`), 316 commits behind `main` and older than every GPUFLOW-1 merge of 2026-09-15.** At that commit the single-image describe route has no `_ensure_gpu_ready` lease path and `load_snapshot` counts only `RUNNING` single-run items; the live load files carry exactly the legacy keys `{queue_depth,in_flight,batch_in_progress,written_at}` and no `lease_demand`/`revision`. So a Suggest against a stopped GPU returns a 503 that publishes **no demand**, the start cycle sees zero work, and the GPU never starts. The plugin on the demo is 0.0.23 (current `main`), so its GPUFLOW-1 warming loop retries for 120 s against a service that cannot ask for the GPU. This is a stale-deploy cause, not a controller or provisioning cause; the A10 quota value and the API-host adapter env remain operator evidence (Slice 0) but no longer gate Slice A.
   - **Two further defects survive a redeploy of `main`.** (a) Cold boot is budgeted at `ACX_GPU_WARMUP_TIMEOUT_SECONDS` (default 510 s, `scene/config/settings.py:21`) while `useDescribeMedia.ts:32` abandons a Suggest after `SUGGEST_WARMING_CEILING_MS = 120 s` with "Still starting — try again"; the 180 s demand lease then expires unless the operator keeps retrying, so a cold Suggest cannot complete and the started GPU idles out after 300 s (OBS-12: the actuator regulates a stock the client stops observing). (b) `GpuTierStatus` (`MediaSelection.tsx:564,760-817`) returns `null` unless a run id exists, so an idle workbench never says the Description Service is stopped or that describing will start it (OBS-08, RLSE-05, HAI-18).
...
- **Keep the per-response ceiling; the warming budget comes from the service.** The `_PUBLIC_RETRY_AFTER_CEILING` (120 s, matched by the PHP validator range `[1,120]`) is unchanged and bounds each advertised `Retry-After`. The *total* client warming budget is no longer a client constant: the `starting` envelope carries `startup_budget_seconds` (the service's own `gpu_warmup_timeout_seconds`, the same value the bulk worker waits) and the SPA retries, with short polls, until that budget elapses from the persisted start. Resume must never hold a PHP request open.
- **Model is a setting, never a literal.** `ACX_DESCRIPTION_ADAPTER` on the API host (`florence_small | gpu_qwen30b | gpu_qwen30b_ensemble`) is the single source of truth; no lane, test or smoke names a model string. Proofs compare the returned `model_id`/`model_version` with `/health/detailed.description_adapter.profile` of the same deployment.
- **Engagement is a deliverable, not a diagnosis.** The wave is not done until a cold "Describe selected"/Suggest from demo.altcontext.com starts the burst GPU within one start-timer tick and returns a description from the configured adapter (Slice A9, Gate R). Deployment of the integrated service image is part of the release gate (operator-run), because the classified cause is a stale deploy.
...
- **Evidence before lanes, and evidence again after (CARD-06, DIAG-02).** The live cause is recorded (stale deploy); Slice 0 completes the operator evidence without gating Slice A. Every GPU claim is proven twice: in-repo with a fake clock (service → load file → controller START) and live on the deployed build (Gate R). A provisioning or env blocker raises an operator gate with a named re-check; it never closes the wave (RLSE-02).
- **Sensors touch the stock (OBS-12).** The client keeps observing the operation for the whole service-advertised startup budget; the idle workbench shows the Description Service state at all times, so the operator can predict what "Describe" will do before pressing it (INT-10).
...
- **Representative quality**: service-computed scalar in [0,1] per representative identity with its components (`confidence`, `bbox_area`, `sharpness`, `occlusion_severity`) — the composite always ships with its parts (UXR-15). Storage is the **existing** `identity_cluster_representatives.quality_score` column (Float NOT NULL, 0..1 check, `db/models/identity.py:244-280`, written by `AssignmentWriter._create_and_add_representative`); C0 adds only `quality_components` beside it — no parallel `representative_quality` column (REF-09). On the wire the value is named `representative_quality` (snapshot, projection, roster); the rename happens once, at the service export boundary (API-10). No tier recomputes it.
...
- **Two undo stacks, never conflated** (INT-09): the roster "Merged from N groups · Undo" (B3) is the existing WP-side person merge undo (`PersonMergeService` `undo_token`, `UNDO_TOKEN_TTL_SECONDS`, `POST /roster/persons/merge/undo`) and reverts a WP person merge; the workbench "Merged automatically · Undo" (C3) is the recognition-side cluster merge receipt and reverts a cluster merge. Neither surface reads the other's token, and no lane bridges them.
- **Startup budget**: `detail.startup_budget_seconds` on the `description_service_starting` envelope — the service's `gpu_warmup_timeout_seconds` (settings SSOT), never a client constant. The SPA's total warming window is `startup_budget_seconds` measured from the persisted `warming_started_at`; each poll still honours `Retry-After ≤ 120 s`.
- **Idle service status**: the workbench chip state when no run or Suggest is pending, read from the existing `GET acx/v1/recognition/gpu/status` proxy (`class-gpu-control-controller.php:30-38`, service `GET /gpu/status`, `require_auth`). Copy per state: `stopped` → "Description Service is off — it starts when you describe (about N min)"; `starting`/`warming` → "Description Service is starting…"; `ready` → "Description Service is ready"; `degraded`/`unknown` → reason via the A7 copy map; fetch failure → "Description Service status unavailable — Retry". N is derived from `warmup_eta_seconds`/`startup_budget_seconds` advertised by the service, never hardcoded.
...
| --- | --- | --- |
| Deployed description service | `commit_sha 51ff7022` on prod and staging API hosts; pre-GPUFLOW-1; single describe publishes no demand on a stopped GPU | `curl https://api.altcontext.com/health`; `git log 51ff7022..main` (316 commits); live `/run/acx-write/*/describe-load.json` legacy keys |
| Lifecycle start rule | START only when a load file shows work; 30 s timer; reads all env dirs | `infra/oci/gpu_lifecycle/controller.py:71-97`, `reaper.py:1746`, `/etc/acx/gpu-lifecycle.env` (`IDLE_SECONDS=300`, `MAX_LEASE_SECONDS=3600`) |
| Single Suggest lease | In-memory refs; cleared on unmount; 120 s client ceiling vs 510 s service warmup budget | `js/admin/hooks/useDescribeMedia.ts:32,98-104,185-191`; `scene/config/settings.py:21`; `describe_run_worker.py:161-205` |
...
| GPU chip | `data-gpu-state` from `progress.gpuState`; hidden when `runId===null`; no idle read of `/gpu/status` (Settings page only, `useGpuControl.ts:68-129`) | `js/admin/pages/workbench/MediaSelection.tsx:560-564,761-818,864`; `js/admin/api/gpuApi.ts` (`fetchGpuStatus`) |
...
- A cold "Describe selected" or Suggest from the demo workbench publishes demand, the start cycle issues START within one 30 s tick, the client keeps polling for the service-advertised startup budget, and the description arrives from the adapter named by `ACX_DESCRIPTION_ADAPTER` on the deployed host. Proven in-repo with a fake clock and live on the deployed build.
- The idle workbench always shows the Description Service state ("off — starts when you describe", "starting…", "ready", or a named reason) so the operator can predict the wait before acting.
...
- `scene-describe-multipart.schema.json`: `description_service_unavailable` gains required `detail.reason` (enum above) and optional `detail.lifecycle_reason` (verbatim from the snapshot). `description_service_starting` gains required `detail.startup_budget_seconds` (number > 0, the service warmup budget). Additive for both.
- `GET /gpu/status` (service) and `GET acx/v1/recognition/gpu/status` (WP) are unchanged; the workbench idle chip is a new read-only consumer. Permission stays `can_manage_recognition`, the same callback every describe route already uses (`class-describe-controller.php:161-324`).
...
- `recognition-cluster-snapshot.schema.json`: `clusters[]` gains `representative_quality` (number|null), `quality_components` (object|null), `representative_media_id` (integer|null, WP attachment id), `undoable_merge_receipt_id` (string|null). This is the only path by which quality and undo availability reach WordPress; the WP projection stores exactly these four fields and nothing derived.
...
- New `roster-merge-candidates-response.schema.json`: for a loser person, ranked survivors `{person_id, name, similarity, band, shared_media_count}`. The recognition service has no person entity (persons are WP integers bound to cluster UUIDs by `PersonMergeService`/`bind`), so the ranking is produced in two tiers: the service ranks **per cluster** (`recognition-cluster-merge-candidates-response.schema.json`, new: `{cluster_id, candidates[]: {cluster_id, name, similarity, band}}`, similarity from `centroid_utils.compute_similarity` merged with pending `ClusterMergeSuggestion` rows, `band` from the calibration policy block, same shape family as the existing `GET /recognition/clusters/{cluster_id}/roster-candidates`); PHP resolves the loser person → its bound cluster UUIDs, calls the service once per bound cluster, maps candidate clusters → bound persons through the projection, and aggregates per person (`similarity` = max over pairs, `band` = band of that max, `shared_media_count` from the projection's media rows). Every emitted number traces to a service value or a projection count; PHP invents none (rg-015).
...
- Recognition HTTP: `GET /recognition/clusters/{cluster_id}/merge-candidates`, `POST /recognition/clusters/{id}/revert-merge` (body `{receipt_id}`), both mounted through `router.py` by an owning lane, both taking `_tenant_id = Depends(get_tenant_id)` like `get_roster_candidates` (`suggestions.py:156-195`).
- WP REST: `GET acx/v1/roster/persons/{id}/media` (paged media via `IdentityMembersReadRepository`/`list_projected_cluster_rows_by_person`), `GET acx/v1/roster/persons/{id}/merge-candidates` (person → bound clusters → service per cluster → aggregate per person, above), `POST acx/v1/workbench/clusters/{id}/revert-merge` (proxy); all three registered in `class-api.php` by an owning lane with the shared `$permission` callable and asserted through the real `rest_api_init` bootstrap.
...
| Area | Files |
| --- | --- |
| Contracts | `packages/shared-contracts/schemas/scene-describe-multipart.schema.json`, `scene-health-detailed.schema.json` (new), `recognition-cluster-snapshot.schema.json`, `roster-entry.schema.json`, `roster-merge-candidates-response.schema.json` (new), `recognition-cluster-merge-candidates-response.schema.json` (new), `docs/workbay/contracts/gpu-lifecycle.md`, `docs/workbay/contracts/identity-merge.md` (new) |
...
| PHP | `src/api/class-abstract-recognition-proxy-controller.php`, `src/support/class-life-cycle-manager.php`, `src/sovereign/repositories/interface-clusters-repository.php`, `class-cluster-projection-writer.php`, `class-clusters-repository.php`, `src/sovereign/repositories/class-cluster-snapshot-merger.php`, `src/sovereign/mappers/class-cluster-response-mapper.php`, `src/sovereign/repositories/class-roster-entry-projection-repository.php`, `src/api/class-person-media-controller.php` (new), `src/api/class-cluster-revert-controller.php` (new), `src/api/class-person-merge-controller.php`, `src/api/class-api.php`, `src/sovereign/sync/class-outbox-maintenance-service.php` |
...
- Service: pytest per lane (exact commands in the lane table), run in the lane's VM sandbox `.venv` for DB-backed suites; contract example documents validated by `test_shared_schema_documents.py`. Route registration is proven through the real app: `TestClient(create_app())` route table contains the new paths (not a per-file `include_router` in the test), and, following the E15-31B structural-test precedent, the test iterates the two new routes and asserts each declares the `get_tenant_id` dependency.
...
- PHP: `vendor/bin/phpunit` filtered suites per lane; runtime autoload parity check (rg-016) for each new controller; route registration proven through `rest_api_init` → `rest_get_server()->get_routes()` containing the three new routes, each with the shared `$permission` callback as `permission_callback` (extends `ApiBootstrapSyncTest` pattern).
- Projection propagation: a PHPUnit test feeds a snapshot fixture carrying `representative_quality`/`representative_media_id`/`undoable_merge_receipt_id` through `ClusterSnapshotMerger` → `ClustersRepository` → the projection table → `RosterEntryProjectionRepository`, asserting the roster row carries the same values and that a pre-export snapshot (fields absent) yields nulls, never fabricated values. A second case projects a pre-export snapshot at version N, then the same version N with the fields present (must be skipped by the COR-1 monotonicity guard, columns stay null), then version N+1 (columns filled).
...
- Recovery merge: fixtures replay the C1 ground truth plus a mixed-unnamed residual (two people in one residual) and a borderline chain; assertions are pair-level — every moved member individually passes against ≥ k distinct-media destination exemplars, the residual is internally coherent, and the mixed residual is abstained (suggestion), never attached. Two successive merges into one survivor undo LIFO; reverting a non-top receipt is refused; expired receipts are purged by the housekeeping step. After a revert, the survivor's and the restored cluster's centroid, representatives and `identity_count` equal a fresh recompute, and a second recovery run over the same fixture does not re-attach the restored cluster.
- GPU engagement (in-repo, fake clock, Slice A9): one test module drives the FastAPI describe route with a `stopped` `gpu-state.json` fixture and a temp `ACX_DESCRIBE_LOAD_PATH`, then runs `GpuLifecycleController.start_needed_instances` over `JsonFileJobLoadSource` on that directory with a recording actuator. Asserts: single multipart Suggest → `in_flight ≥ 1` (`lease_demand`) → exactly one START; bulk run enqueue → `batch_in_progress: true` → START; lease expiry after `DEMAND_LEASE_SECONDS` with no client retry → no work; `starting` envelope carries `startup_budget_seconds == settings.gpu_warmup_timeout_seconds`. Scene DB tests hang inside the codex sandbox; the lane's `test_cmd` runs over ssh in the VM sandbox `.venv` (known gotcha).
- Live engagement proof (Gate R, operator-run on acx-backend as `ubuntu`): (1) deploy the integrated service image to staging and prod API hosts and confirm `/health.commit_sha` equals the integrated HEAD; (2) `ACX_GPU_SMOKE_CONFIRM=RUN python3 scripts/gpu_burst_smoke.py --live --wp-base-url https://demo.altcontext.com --media-ids <ids> --service-base-url http://<burst-ip>:8000 --service-api-key-env <var> --instance-id <ocid> --max-seconds 1200` (always STOPs in `finally`); (3) one manual cold Suggest from the demo workbench: chip reads "off" before the click, "starting…" after, description arrives, and the response `model_id` matches `/health/detailed.description_adapter.profile`; (4) `describe-load.json` before/after excerpts and the start-cycle journal lines are attached to the `test_result` for the HEAD SHA. Measured cold-boot time is recorded and compared with the advertised `startup_budget_seconds` (A5 has no client constant to tune).
...
### Slice 0: Live evidence capture (operator + read-only agent)

**Goal**: The classified cause (stale service deploy, Problem Statement §1) is recorded with the remaining operator evidence, and the A3 gate is decided. Slice A lanes do not wait for this slice.

Changes:

- Already recorded by the planning session (2026-09-16 23:01 UTC): both API hosts at `commit_sha 51ff7022`; lifecycle timers active; `gpu-state.json` `stopped` with instance OCID; all env load files zero work with legacy keys; no intent file; plugin 0.0.23. Slice 0 copies these into the assessment verbatim.
- Operator captures the rest: the A10 service-limit value and instance list for `acx_gpu_burst`; the API-host `ACX_DESCRIPTION_ADAPTER`, `ACX_GPU_ENDPOINT_URL` and `preflight-gpu-env.sh --check-reaper` exit code (the `gate` account cannot read the docker socket, journal or compose env); the deployed `gpu_lifecycle` version and whether `reaper.py` on the host contains the `readiness_timeout` fallback path; the last two start-cycle journal excerpts; `/health/detailed` body; the WP transient circuit key state; the outbox `failed` row count by age.
- Recommended, not gating: the operator deploys current `main` to the staging API host now and runs one Suggest against it, recording `describe-load.json` before and 30 s after. This turns the stale-deploy classification into a measurement with the GPUFLOW-1 lease code present (DIAG-02) and gives A5/A9 a first cold-boot timing.
- Agent lane (luna max, read-only against evidence) writes `docs/assessments/GPUFLOW-2-gpu-online-diagnosis-20260916.md`: the evidence table, the trace of the deployed timeout → fallback → `DEGRADED` → snapshot path with the journal lines that prove or refute it, the Katy Perry representative row (identity id, `representative_quality`, occlusion score, media id), the Emma Watson triple (identity ids, embeddings' pairwise cosine, refinement decision), and the Perry/Trudeau mixed group membership.
- The assessment sets `a3_required: true|false`. A3 dispatches only when true.
- A provisioning or env blocker (quota 0, adapter not `gpu_*`, endpoint unset/non-private) becomes an **operator gate with a named re-check**: the assessment lists the exact command and expected value, Gate R blocks until the re-check passes, and the wave is not closed around it (RLSE-02).

Proof: assessment committed; handoff decision references it; `a3_required` and any operator gate recorded.
...
Changes: `responses.py` adds `UnavailableReason` StrEnum and the field; `routers/describe.py::_ensure_gpu_ready` maps missing/stale snapshot, DEGRADED, operator STOP and unconfigured endpoint to reasons and forwards the snapshot's own `reason` as `detail.lifecycle_reason` verbatim; the `starting` envelope adds `startup_budget_seconds = settings.gpu_warmup_timeout_seconds` (SSOT with the bulk worker's wait); `GpuRemoteAdapterError` after ready keeps `502 description_service_error`. Contract example updated. Lint deferral `SVCCOL-M-04` fixed here.

Proof: route tests assert reason per state, `lifecycle_reason` passthrough for a `DEGRADED`/`readiness_timeout` snapshot, and `startup_budget_seconds` on every `starting` response; multipart schema example round-trips.
...
Changes: `useDescribeMedia.ts` persists the full versioned command `{version, kind: 'suggest', media_id, operation_id, startup_id, warming_started_at, startup_budget_seconds, request: {writeAlt, force}}` through the store on the first `starting` response, taking `writeAlt`/`force` from the same arguments `describeMedia` sends (`useDescribeMedia.ts:15-43,65-79`); on mount with a matching media id, resumes the retry loop honouring `Retry-After` and the remaining budget from the persisted start, and replays `request` verbatim on the POST. `SUGGEST_WARMING_CEILING_MS` is deleted: the total window is the persisted `startup_budget_seconds` (fallback when absent: the last advertised `Retry-After`, one poll, then the timed-out state — never a silent success). Warming copy shows elapsed/expected ("Starting the Description Service · 2:10 of about 8:30") and the timed-out copy names the next step. `clearLease()` also clears storage. An unknown `version` is discarded. Resolves `GPUFLOW1-FIN-01` correlation by reusing the persisted `operation_id` on manual retry.

Proof: fake-timer unmount/remount test table over `{writeAlt, force} ∈ {false,true}²` asserts the resumed POST carries the persisted `operation_id` and the same flags, and that `writeAlt=true` ends in the alt-persistence mutation; the loop keeps polling past 120 s while `startup_budget_seconds` remains and stops exactly at the budget computed from the persisted start, not remount time; a response without the field ends after one bounded poll.
...
### Slice A8: Idle Description Service status in the workbench (SPA)

**Goal**: The workbench always says whether the Description Service is off, starting, ready or unavailable, and what "Describe" will do next (INT-10, HAI-18, OBS-08, RLSE-04).

Changes: `GpuTierStatus` moves out of `MediaSelection.tsx` into a new `js/admin/pages/workbench/GpuTierStatus.tsx` that renders in every state. New hook `js/admin/hooks/useGpuServiceStatus.ts` polls the existing `fetchGpuStatus` (`gpuApi.ts`) on the Settings cadence while no run/Suggest is pending and pauses while one is (the run's own `gpu_state` wins). Copy per Terminology "Idle service status"; the expected wait is rendered from `warmup_eta_seconds`/`startup_budget_seconds` when the service supplies them and omitted otherwise; `data-gpu-state` keeps its enum; icon + colour (sr-004); polite announcement only on state change (A11Y-21). The `stopped` copy names the deferred effect of pressing Describe. No new PHP route or permission: the status proxy already uses `can_manage_recognition`, the same callback as the describe routes.

Proof: `GpuTierStatus.test.tsx` renders all six states from `GPU_STATE` plus fetch-failure; `useGpuServiceStatus.test.ts` proves polling pauses during a run and resumes after terminal status; `MediaSelection` tests still pass with the extracted component.

### Slice A9: GPU engagement proof (backend test, no production code)

**Goal**: A committed test proves service → load file → controller START for both describe paths, with the lease and budget semantics the SPA relies on.

Changes: new `apps/prototype-description-service/scene/tests/test_gpu_engagement_end_to_end.py` plus fixture `scene/tests/fixtures/gpuflow2-gpu-state-stopped.json`, per Verification Strategy "GPU engagement". It imports `infra.oci.gpu_lifecycle` (`GpuLifecycleController`, `JsonFileJobLoadSource`, a recording actuator) against the real `dump_load_snapshot` writer. No production file changes; a failing assertion routes a finding to `svc-unavailable-reason` or, with evidence, to `infra-ready-degrade`.

Proof: the module passes over ssh in the VM sandbox `.venv` and is listed in the lane's `test_result`.
...
- **B2a service export** (`svc-rep-export`): `clusters_snapshot.py::_build_cluster_responses` emits `representative_quality` (from the representative row's existing `quality_score`), `quality_components` (column landed by C0), `representative_media_id` from the persisted representative row, and `undoable_merge_receipt_id` from the newest unreverted, unexpired receipt; `recognition-cluster-snapshot.schema.json` example updated by `contracts-roster`.
- **B2b WP storage** (`php-projection-schema`): `class-life-cycle-manager.php` adds the four columns to the projection table via `dbDelta`; `interface-clusters-repository.php` + `class-cluster-projection-writer.php::upsert_projection_cluster` accept and persist them (null when absent from the snapshot). Rows written before the export lands stay null until the service publishes a **newer** `snapshot_version`: `SnapshotProjector::project` (COR-1, `class-snapshot-projector.php:104-108`) skips any snapshot whose version is not greater than the stored one, so re-running the bootstrap/targeted sync on the same version is a no-op by design (FLOW-06: heal by reprocessing into a fresh version). For the demo this is the next clustering run or the greenfield `make reset-local`; no backfill and no version bump in PHP is invented.
- **B2c WP ingest + read** (`php-projection-ingest`, then `php-roster-projection`): `src/sovereign/repositories/class-cluster-snapshot-merger.php` and `class-clusters-repository.php` pass the fields from the snapshot payload to the writer; `class-cluster-response-mapper.php` exposes them to the workbench; `class-roster-entry-projection-repository.php` reads the columns into `roster-entry.schema.json` shape; `roster-entry.ts` regenerated.

Proof: snapshot schema example validates; projection-propagation PHPUnit test (Verification Strategy) passes for present and absent fields and for the same-version/next-version replay case; `RosterEntryProjectionRepositoryTest` covers null and non-null rows; generated type diff reviewed.
...
Changes: recognition route file `routers/cluster_merge_candidates.py` `GET /recognition/clusters/{cluster_id}/merge-candidates` ranks other clusters by centroid cosine (`centroid_utils.compute_similarity`) merged with pending `ClusterMergeSuggestion` similarity, returns `{cluster_id, name, similarity, band}` with `band` cut from the accepted calibration policy block in `settings/clustering.py` (no literal thresholds in the route), reusing the `list_roster_candidates` ranking path (`suggestions.py:156-195`) rather than a second ranker; `svc-route-registry` mounts it in `router.py`. `class-person-merge-controller.php` implements `GET acx/v1/roster/persons/{id}/merge-candidates` as person → bound cluster UUIDs → one service call per cluster → candidate cluster → bound person → per-person aggregate (max similarity, its band, `shared_media_count` from the projection), and also accepts `operator_initial_choice` on the merge POST and stores it on the merge record. `PersonMergeDialog.tsx` is two-step (HAI-15): **step 1** shows `#merge-survivor` as an unranked, score-free list in `name ASC`, filtered by `#merge-search` typeahead, and requires a pick; the pick is committed to dialog state and sent as `operator_initial_choice`. **Step 2** (only after the pick) fetches candidates and re-renders `#merge-survivor` sorted by similarity desc (name asc within band) with labels "Name — 87% match (likely)" from `similarityCopy.ts`, highlighting the operator's pick and, when it differs from the top candidate, showing both; the operator confirms or changes, and the merge request carries both `operator_initial_choice` and the final survivor.

Proof: service unit test on ranking and band cuts read from the policy block; PHPUnit for the person→cluster→person aggregation (a person bound to two clusters, a candidate cluster with no bound person is dropped, max-similarity selection) and `operator_initial_choice` persistence; vitest asserts no score, band, or similarity ordering is rendered or fetched before a pick, that step 2 ordering is by similarity, and that both choices are in the submitted payload.
...
Changes: `db/models/identity.py` + `001_identity_schema.py` add the `cluster_merge_receipts` table (Terminology) and `quality_components` (JSONB, nullable) on `identity_cluster_representatives`; the composite stays in the existing `quality_score` column (no parallel `representative_quality` column, REF-09); ORM relationships `IdentityCluster.merge_receipts` (ordered by `created_at` desc). New `recognition/tests/unit/test_identity_schema_receipts.py` asserts the columns, the ordering, that `IdentityClusterRepresentative` has exactly one quality scalar, and that reverting a non-top receipt is impossible at the repository layer. Greenfield: no migration.
...
Changes: `cluster_merge.py` gains `revert_merge(receipt_id)`: refuses with `receipt_not_top` unless the receipt is the survivor's newest unreverted one, refuses with `receipt_expired` past `expires_at`, otherwise restores `moved_identity_ids` to a cluster labelled `source_label` or "<name> (restored)" (4.2.3 semantics) and sets `reverted_at`. The restore mirrors `merge_cluster` (`cluster_merge.py:286-440`) step for step: move members, clear `moved_by_merge_id` on them, `recompute_representatives` + `recompute_centroid` for both clusters, best-effort `refresh_centroids_view` (warn, never roll back — MAINT-descsvc-deadcode finding 1550 precedent), delete stale `ClusterMergeSuggestion` rows for both clusters, set `identity_count` from a member count, broadcast `cluster_merge_reverted`. Refusals are `409` problem-details with `code` (API-05). New route file `routers/cluster_revert.py` `POST /recognition/clusters/{id}/revert-merge` (mounted by `svc-route-registry`); new `class-cluster-revert-controller.php` proxies it (registered by `php-route-registry`); `IdentityClusterItem` shows "Merged automatically · Undo" only while the projected `undoable_merge_receipt_id` is non-null (arrives through B2's snapshot → projection path) and calls the proxy with that id.

Proof: revert test restores the exact prior partition and both clusters' centroid/representatives/`identity_count` equal a fresh recompute; an MV-refresh stub that raises leaves the revert committed; a second recovery run does not re-attach the restored cluster; two successive merges undo in LIFO order and the non-top revert is refused; UI test toggles on `undoable_merge_receipt_id` and submits the receipt id.
...
Ordered, each step recorded on the integrated HEAD SHA:

1. Integrated branch `make check-all`; A9 module green over ssh; zero open highs.
2. Operator deploys the integrated service image to staging and prod API hosts (`scripts/deploy/recognition-service.sh` via `mk/deploy.mk`) and the plugin zip via `make deploy-demo`; `/health.commit_sha` on both hosts equals HEAD and `/health/detailed.description_adapter.profile` equals the host's `ACX_DESCRIPTION_ADAPTER`. Any Slice 0 operator gate re-check passes here.
3. Live engagement proof per Verification Strategy (smoke script + one manual cold Suggest from the demo workbench; `model_id` matches the profile; load-file and journal excerpts attached).
4. Operator smoke per Verification Strategy; local codex harmonizing review against the canon; `handoff_close_check(enforce=True)`; slice-complete decision.

A failure at step 3 reopens the wave; it is never deferred (RLSE-02, CARD-13: the proof is the outcome, not a proxy).
...
| `contracts-roster` | B2/B6/C3 | `packages/shared-contracts/schemas/roster-entry.schema.json`, `packages/shared-contracts/schemas/recognition-cluster-snapshot.schema.json`, `packages/shared-contracts/schemas/recognition-cluster-merge-candidates-response.schema.json` (new), `packages/shared-contracts/schemas/roster-merge-candidates-response.schema.json` (new), `docs/workbay/contracts/identity-merge.md` (new) | diagnosis: assessment | `python3 -m pytest apps/prototype-description-service/scene/tests/test_shared_schema_documents.py -q -p no:cacheprovider` |
...
| `spa-gpu-idle-status` | A8 | `js/admin/pages/workbench/GpuTierStatus.tsx` (new), `js/admin/hooks/useGpuServiceStatus.ts` (new); `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/GpuTierStatus.test.tsx` (new test) | contracts-service: `scene-describe-multipart.schema.json` (`startup_budget_seconds` name, read-only); `js/admin/api/gpuApi.ts` read-only (existing `fetchGpuStatus`) | npx vitest run js/admin/pages/workbench/__tests__/GpuTierStatus.test.tsx js/admin/hooks/__tests__/useGpuServiceStatus.test.ts |
| `spa-run-resume-chip` | A4 | `js/admin/pages/workbench/MediaSelection.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/fixtures/gpuflow2-resume.json` (new test fixture) | spa-operation-store: `js/admin/hooks/describeOperationStore.ts`; spa-gpu-idle-status: `js/admin/pages/workbench/GpuTierStatus.tsx` (extracted component, imported) | npx vitest run js/admin/pages/workbench/__tests__/MediaSelection.statusAnnouncement.test.tsx js/admin/pages/workbench/__tests__/MediaSelection.resume.test.tsx |
| `spa-suggest-resume` | A5 | `js/admin/hooks/useDescribeMedia.ts` | spa-operation-store: `js/admin/hooks/describeOperationStore.ts`; spa-error-copy: `js/admin/api/describeApi.ts`; svc-unavailable-reason: `scene/tests/fixtures/gpuflow2-unavailable.json` (`startup_budget_seconds` wire shape) | npx vitest run js/admin/hooks/__tests__/useDescribeMedia.test.tsx |
| `svc-engage-proof` | A9 | `apps/prototype-description-service/scene/tests/test_gpu_engagement_end_to_end.py` (new test), `apps/prototype-description-service/scene/tests/fixtures/gpuflow2-gpu-state-stopped.json` (new test fixture) | svc-unavailable-reason: `scene/interface_adapters/http/routers/describe.py` (committed `starting` envelope) | `python3 -m pytest apps/prototype-description-service/scene/tests/test_gpu_engagement_end_to_end.py -q -p no:cacheprovider` (run over ssh in the VM sandbox `.venv`) |
...
| `svc-merge-candidates` | B6 | `recognition/interface_adapters/http/routers/cluster_merge_candidates.py` (new), `recognition/application/suggestions/merge_candidates.py` (new); `apps/prototype-description-service/recognition/tests/unit/test_merge_candidates.py` (new test) | contracts-roster: `recognition-cluster-merge-candidates-response.schema.json`; calibration: policy block (band cuts) | `python3 -m pytest apps/prototype-description-service/recognition/tests/unit/test_merge_candidates.py -q -p no:cacheprovider` |
| `svc-route-registry` | B6/C3 | `recognition/interface_adapters/http/router.py`; `apps/prototype-description-service/recognition/tests/unit/test_recognition_route_table.py` (new test) | svc-merge-candidates: `routers/cluster_merge_candidates.py`; svc-merge-revert: `routers/cluster_revert.py` | `python3 -m pytest apps/prototype-description-service/recognition/tests/unit/test_recognition_route_table.py -q -p no:cacheprovider` |
| `php-merge-candidates` | B6 | `src/api/class-person-merge-controller.php`; `apps/prototype-wp-alt-context/tests/Unit/PersonMergeCandidatesAggregationTest.php` (new test) | contracts-roster: `roster-merge-candidates-response.schema.json`; svc-merge-candidates: `recognition/tests/unit/test_merge_candidates.py` (wire shape); svc-route-registry: `test_recognition_route_table.py` (route mounted) | vendor/bin/phpunit --filter PersonMergeController |
...
| `ux-describe` | E | `apps/prototype-wp-alt-context/docs/ux-maps/describe-gpu-tier.uxmap.json`, `apps/prototype-wp-alt-context/docs/ux-maps/describe-gpu-tier.md` | spa-run-resume-chip: `gpuflow2-resume.json`; spa-gpu-idle-status: `GpuTierStatus.tsx` (idle states and copy) | render_ux_maps.py --check |
...
- `MediaSelection.tsx` is owned only by `spa-run-resume-chip`; `useDescribeMedia.ts` only by `spa-suggest-resume`; both consume the committed store file read-only. `GpuTierStatus` leaves `MediaSelection.tsx` in `spa-gpu-idle-status` (new file) *before* `spa-run-resume-chip` dispatches; the chip lane only imports it. `gpuApi.ts` and `useGpuControl.ts` are read-only for every lane.
- `svc-engage-proof` owns only its test module and fixture; it never edits `describe.py`, `describe_load.py` or `infra/oci/gpu_lifecycle/*`.
...
- WP projection storage and ingest are split by layer: `class-life-cycle-manager.php`, `interface-clusters-repository.php`, `class-cluster-projection-writer.php` → `php-projection-schema`; `src/sovereign/repositories/class-cluster-snapshot-merger.php`, `class-clusters-repository.php`, `class-cluster-response-mapper.php` → `php-projection-ingest`; `class-roster-entry-projection-repository.php` → `php-roster-projection`. `SnapshotProjector` (`src/sovereign/sync/`) is not touched by any lane.
...
1. Gate 0: astra high plan review and the local Claude canon review recorded; plan frozen. The GPU cause is already classified (stale deploy); `diagnosis` lands in Wave 1 alongside the contracts lanes and decides only `infra-ready-degrade` (`a3_required`) and any operator gate for Gate R. Slice A lanes never wait on it.
2. Wave 1 (parallel, GRPH-31 longest chains first): `contracts-service`, `contracts-roster`, `diagnosis`, `calibration`, `spa-roster-headings`, `spa-cluster-ungrouped`, `php-outbox-reclaimer`, `spa-decorative-control` (after the operator decision).
3. Wave 2: `svc-health-adapter`, `svc-unavailable-reason`, `spa-operation-store`, `spa-gpu-idle-status` after `contracts-service`; `infra-ready-degrade` only when gated in; `svc-identity-schema`, `php-projection-schema`, `php-person-media` after `contracts-roster`; `svc-merge-candidates` after `contracts-roster` and accepted `calibration`; `svc-rep-quality` after accepted `calibration` and `svc-identity-schema`; `spa-deadletter` after the reclaimer.
4. Wave 3: `php-circuit-reason` and `svc-engage-proof` after `svc-unavailable-reason`; `spa-run-resume-chip` after the store and `spa-gpu-idle-status`; `svc-rep-export`, `svc-recovery-merge`, `svc-merge-revert` after `svc-identity-schema`; `php-merge-candidates` after `svc-merge-candidates`; `svc-rep-recompute` after `svc-rep-quality`.
...
6. Wave 5: `spa-service-status` and `spa-suggest-resume` (also after `svc-unavailable-reason`'s fixture) after `spa-error-copy`; `php-roster-projection` after `php-projection-ingest`; `php-route-registry` after `php-person-media`, `php-merge-candidates`, `php-cluster-revert`.
...
Critical path: `contracts-roster → svc-identity-schema → svc-merge-revert → svc-route-registry → php-cluster-revert → php-route-registry → spa-cluster-undo → ux-identity`. GPU engagement chain (must finish before Gate R step 2): `contracts-service → svc-unavailable-reason → {svc-engage-proof, php-circuit-reason → spa-error-copy → spa-suggest-resume}` and `contracts-service → spa-gpu-idle-status → spa-run-resume-chip → ux-describe`. Integrate continuously into `feature/gpuflow-2`; retire each worktree once its commit and evidence are preserved; `main` receives one final merge.

### Manifest

```bash
make lane-manifest-init TASK=GPUFLOW-2 LANE_IDS='diagnosis contracts-service contracts-roster svc-health-adapter svc-unavailable-reason infra-ready-degrade svc-engage-proof php-circuit-reason spa-error-copy spa-service-status spa-operation-store spa-gpu-idle-status spa-run-resume-chip spa-suggest-resume spa-roster-headings svc-identity-schema svc-rep-export php-projection-schema php-projection-ingest php-roster-projection spa-roster-table php-person-media spa-person-media svc-merge-candidates svc-route-registry php-merge-candidates php-cluster-revert php-route-registry spa-merge-dialog spa-decorative-control calibration svc-recovery-merge svc-merge-revert svc-rep-quality svc-rep-recompute spa-cluster-ungrouped spa-cluster-undo php-outbox-reclaimer spa-deadletter ux-roster ux-describe ux-service ux-identity' TASK_PLAN=docs/tasks/v0.5.0/GPUFLOW-2-durable-describe-roster-identity-and-cluster-recovery-task-plan.md
```
...
- [ ] `startup_budget_seconds` on the `starting` envelope; Suggest polls to the service budget, no client ceiling constant
- [ ] Idle workbench chip shows off/starting/ready/reason with the deferred effect named; polling pauses during a run
- [ ] Engagement proof module green over ssh: Suggest → lease → START; bulk → `batch_in_progress` → START; lease expiry → no work
...
- [ ] Service image and plugin deployed; `/health.commit_sha` == HEAD on both API hosts; adapter profile == host `ACX_DESCRIPTION_ADAPTER`
- [ ] Live engagement proof recorded as `test_result` on HEAD: smoke script pass + manual cold Suggest from the demo workbench, `model_id` matches the profile, load-file and journal excerpts attached
...
- Revision 4 canon validation (GPU engagement): DIAG-01/DIAG-02 — cause classified from live `/health`, unit config and load files, not from hypothesis; OBS-12 — the client observes the operation for the service-advertised budget instead of a shorter proxy window; OBS-08/RLSE-05/RLSE-04 — the idle chip and the timed-out state are designed, never silent or blank; HAI-18/INT-10 — the `stopped` copy names what Describe will do and how long it takes; RES-02 — every poll bounded by `Retry-After ≤ 120 s`, the whole loop by `startup_budget_seconds`; DIAG-07 — the lease expiry test names where a stopped client's fault stops (no START, no cost); RLSE-02/CARD-13/CARD-06 — the live proof on the deployed build is the release gate, not a proxy test; sr-007 — model identity comes from the adapter setting and `model_id`, no literal in code, test or smoke.
...
- On the deployed build, a cold "Describe selected" or Suggest from demo.altcontext.com publishes demand, START is issued within one 30 s tick, and the description arrives from the adapter configured on the host (`model_id` matches the profile). Proven live at Gate R and in-repo with a fake clock.
- With nothing pending, the workbench chip says the Description Service is off and that describing will start it; it never renders nothing.
