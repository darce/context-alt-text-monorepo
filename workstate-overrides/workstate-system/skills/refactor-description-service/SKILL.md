...
- **Breaking API/DB schema change** — removing/renaming a field the WP plugin consumes; reusing a column/tag. → additive-only, new fields nullable/defaulted, never repurpose; backward-compat requests + forward-compat responses (Kleppmann Ch4; `docs/workstate/contracts/{recognition-clustering,curation-sync-api}.md`).
