...
This document classifies the non-handoff MCP tools that still exist in the legacy [`unified_server.py`](/Users/daniel/Development/context-alt-text-monorepo/scripts/mcp/unified_server.py) implementation. The portable [`agent-handoff-mcp`](/Users/daniel/Development/context-alt-text-monorepo/docs/agentic/contracts/agent-handoff-mcp.md) package does not expose these tools.
...
| `find_wp_action` | wordpress/php | drop | This is a specialized grep over PHP hook strings. Generic search is already the right primitive. |
| `find_wp_rest_route` | wordpress/php | drop | Native grep/read tools already expose route registration sites without requiring MCP mediation. |
| `find_php_class` | wordpress/php | drop | Generic code navigation is sufficient. |
...
Do not create separate WordPress, React, or docs MCP servers from the current helper set. The existing helpers in those families are convenience wrappers, not durable contracts.
