...
Each loaded MCP server costs prompt tokens for the entire session. Most sessions need only one or two of the three servers (`workbay-handoff-mcp`, `workbay-orchestrator-mcp`, `computer-use`). This protocol uses **declarative routing**: rules in a YAML file, applied mechanically at session start, reproducible across harnesses.

## Where the rules live

- **Routing data**: [`docs/workbay/maps/mcp-tool-routing.yaml`](../maps/mcp-tool-routing.yaml) — the always/on-demand/exclude lists keyed by triggers.
...
1. Edit [`docs/workbay/maps/mcp-tool-routing.yaml`](../maps/mcp-tool-routing.yaml).
