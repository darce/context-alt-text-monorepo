...
class SqlAlchemyClusterRepository(ClusterRepository):
    ...
    def _to_domain_identity(self, model: MediaIdentity, *, cluster_id: str | None = None) -> DomainIdentity:
        ...
        return DomainIdentity(
            ...
            sharpness=float(model.sharpness) if getattr(model, "sharpness", None) is not None else None,
            embedding_norm=(
                float(model.embedding_norm) if getattr(model, "embedding_norm", None) is not None else None
            ),
            occlusion_severity=(
                float(model.occlusion_severity)
                if getattr(model, "occlusion_severity", None) is not None
                else None
            ),
