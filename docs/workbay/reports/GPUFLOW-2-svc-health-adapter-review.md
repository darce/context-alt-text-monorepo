...
## Re-review r8 (12f6021e6..df3c7a9b9)

VERIFIED: {"GPUFLOW-2-SVCHEALTHADAPTER-R-13":"fixed","SVCHEA-0feee8f56b32d713-H-e773a7622485644b567bfd6f":"not_fixed","SVCHEA-0feee8f56b32d713-H-1ab3ae3ba4b23b879dba88f2":"fixed","GPUFLOW-2-F7BI-01 (?)":"not_fixed","SVCHEA-0feee8f56b32d713-H-4707ac0d3ee4fb55fed109ac":"fixed","SVCHEA-2963cea0f6407fb9-H-5ffb24aeb0647109050048fb":"fixed","SVCHEA-2963cea0f6407fb9-H-09650fb82186ac780a5e42e6":"fixed"}
FINDINGS: [{"id":"GPUFLOW-2-SVCHEALTHADAPTER-R-14","severity":"high","file_path":"infra/oci/demo/lib/describe-gate.sh","line":165,"summary":"The demo gate trusts a complete but unusable adapter readiness object.","evidence":"The validator returns only profile after shape/type checks (infra/oci/demo/lib/describe-gate.sh:121-165), and classify_describe_gate trusts any allowlisted profile without inspecting usable, fresh, endpoint_private, endpoint_configured, endpoint_allowlisted, or reason (infra/oci/demo/lib/describe-gate.sh:301-311). The new fixture explicitly accepts status=degraded with endpoint_private=null, fresh=false, and usable=false (infra/oci/demo/tests/test-describe-gate.sh:198); the extracted trusted profile then classifies RUN for partial coverage, so bootstrap can publish through an adapter with no fresh private-endpoint evidence [rg-015][CARD-09][TEST-15]."}]

| finding | verdict | evidence |
| --- | --- | --- |
| GPUFLOW-2-SVCHEALTHADAPTER-R-13 (high) | fixed | The extractor now requires the exact eleven-key `descriptionAdapterReadiness` shape (`infra/oci/demo/lib/describe-gate.sh:121-135`) and validates the enum, boolean, nullable-private, timestamp, reason, and identity types (`:137-158`); the tests cover missing/extra keys and invalid field types (`infra/oci/demo/tests/test-describe-gate.sh:192-197`). |
| SVCHEA-0feee8f56b32d713-H-e773a7622485644b567bfd6f (high) | not_fixed | This delta only hardens the already-object extractor (`infra/oci/demo/lib/describe-gate.sh:112-166`) and its fixtures (`infra/oci/demo/tests/test-describe-gate.sh:174-211`); it contains no producer/consumer integration hunk for the object/string compatibility break, so the listed cross-consumer release issue is not fixed by this diff. |
| SVCHEA-0feee8f56b32d713-H-1ab3ae3ba4b23b879dba88f2 (high) | fixed | Exact-key validation rejects omitted identities, and the trusted-profile guard requires both `model_id` and `model_version` to be non-null, non-empty strings (`infra/oci/demo/lib/describe-gate.sh:156-164`); the new tests cover both trusted null cases and the non-trusted null case (`infra/oci/demo/tests/test-describe-gate.sh:185-191`). |
| GPUFLOW-2-F7BI-01 (?) | not_fixed | The supplied brief gives this placeholder no finding text or behavior-to-hunk mapping; the delta therefore provides no evidence that it is fixed, so it remains conservatively unresolved. |
| SVCHEA-0feee8f56b32d713-H-4707ac0d3ee4fb55fed109ac (high) | fixed | The nested validator now enforces all required readiness keys with exact-key rejection and type/range checks (`infra/oci/demo/lib/describe-gate.sh:121-158`), and the tests reject missing kind, extra keys, and invalid field types (`infra/oci/demo/tests/test-describe-gate.sh:192-197`). |
| SVCHEA-2963cea0f6407fb9-H-5ffb24aeb0647109050048fb (high) | fixed | Trusted profiles now require non-null, non-empty `model_id` and `model_version` (`infra/oci/demo/lib/describe-gate.sh:159-164`); the added null-identity cases expect an empty extraction (`infra/oci/demo/tests/test-describe-gate.sh:188-189`). |
| SVCHEA-2963cea0f6407fb9-H-09650fb82186ac780a5e42e6 (high) | fixed | The extractor now rejects missing or non-`ok`/`degraded` top-level status (`infra/oci/demo/lib/describe-gate.sh:113-116`), with explicit missing-status and unhealthy tests (`infra/oci/demo/tests/test-describe-gate.sh:174-175`). |

### FINDINGS

#### GPUFLOW-2-SVCHEALTHADAPTER-R-14 — high

The new validator proves only that the nested object has the right keys and scalar types, then emits its profile (`infra/oci/demo/lib/describe-gate.sh:121-165`). The downstream `classify_describe_gate` decision still checks only whether that profile is allowlisted (`infra/oci/demo/lib/describe-gate.sh:301-311`); it never requires `usable`, fresh private-endpoint evidence, or consistent endpoint flags. The new regression fixture intentionally supplies a full trusted GPU object with `status: degraded`, `endpoint_private: null`, `fresh: false`, and `usable: false` and expects extraction to succeed (`infra/oci/demo/tests/test-describe-gate.sh:198`). That profile reaches `RUN` when media has missing alt text, allowing bootstrap to publish through an unavailable GPU adapter. This is a release fail-open under `[rg-015]`, `[CARD-09]`, and `[TEST-15]`; the gate must reject unusable readiness or pass the readiness state into classification.
