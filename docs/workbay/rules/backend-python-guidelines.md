...
> **Library reference**: Review current docs for FastAPI, SQLAlchemy, Pydantic,
...
For `workbay-handoff-mcp` write paths, resolve `task_ref` before calling `collect_target_context_warnings(...)`.
