...
- `GuidedPrototypePage.tsx` — `onUndo` no longer calls `focusGuidedSection`; page feedback pairs colour with `guided-page-feedback-icon`.
- `GuidedResetDialog.tsx` / `GuidedFacesPanel.tsx` — `DialogContent` `aria-modal="true"`.
- `GuidedDescriptionReview.tsx` — `demo-applied-image` `src` is distinct (`#demo-applied-preview`).
- `GuidedFaceMatchCard.tsx` — crop `alt` is `Detected {position} face`; keyboard-operable Enlarge comparison dialog.
...
- `_guided-prototype.scss` — feedback icon; dead unused selectors removed; live status modifiers match `GUIDED_LIVE_BRIEF_STATUS`.
...
| `GuidedPrototypePage.test.tsx`                 | 13 passed |
...
| T07 | run               | Replace-cancel/confirm; restore/cancel-reset focus.                                                               |
| T08 | run               | Stale preview UI.                                                                                                 |
| T09 | run               | omit/omit and include/include.                                                                                    |
| T10 | run               | Hash stays `#/guided-prototype`.                                                                                  |
| T11 | partially_covered | Tab order, no positive tabindex, dialog trap, focus stay/move. Manual toolbar NOT RUN.                            |
| T12 | run               | Keyboard comparison + honest counts; two Enlarge comparison buttons; crop alts in the tree.                       |
