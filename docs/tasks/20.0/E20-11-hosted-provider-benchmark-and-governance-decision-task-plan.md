...
| Eval CLI | `apps/prototype-description-service/scripts/eval_harness/cli.py` | Add `--provider <profile>` (and matrix iteration) via `_provider_flags` on the `fetch`/`run` subparsers only (`score` stays pure/offline); stamp provider/profile into the run-record provenance; reuse `fetch_run_record`, `build_reports`, bounded-failure path unchanged |
...
- Add `--provider` (matrix-capable) to `cli.py` via `_provider_flags` on `fetch`/`run` only; stamp provider into run-record provenance; reuse `fetch_run_record` + `build_reports` unchanged.
