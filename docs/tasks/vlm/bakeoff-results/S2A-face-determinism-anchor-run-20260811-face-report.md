...
- score manifest_sha256: `19861fede2ec99bc0a7ed8777bca25829bce0fdd65e948e8c0080407e84c18e4`
...
- **headline_identification**: status=`UNDER-FLOOR / DIRECTIONAL — awaiting operator demotion` directional=True precision=0.800 (4/5) recall=0.800 (4/5) n_recall_eligible=5/100 frame=`celebs01_named_matched_probes (provenance.source==CELEB); named_matched_probes_pooled_kfold_decisions: precision over accept/confusion; recall over enrolled (≥2 matched faces) only; detection misses excluded from FN (EVAL-16); error-item media excluded from scoring (listed in failures)`
...
- id-recall: 0.800 alongside detection-recall: 0.857 (coupling_flag=True, missed_gt=1, unmatched_det=0) — identification recall is computed only over faces this leg detected and §C-matched (enrolled); weak detection can inflate id-recall on the easy detected subset — report id-recall ALONGSIDE detection-recall
