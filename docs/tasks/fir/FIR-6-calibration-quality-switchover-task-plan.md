...
5. **Threshold-knob consumer rebinding** (resolves GR2-04/LC2-01 — the calibration apply path becomes real here): route every consumer of the five threshold overrides through the S1 profile-resolution helper: `AssignmentGate` construction (`interface_adapters/http/deps/services.py:153,282`), `checks/confidence.py:130-155`, complete-link/graph discovery threshold reads (`discovery/centroid.py:49`, `representative.py:111`, `graph/discovery.py`, `graph/selection.py:45`), `ClusteringLimitsSettings.similarity_threshold` readers, `IdentityDetectionSettings.default_threshold` readers. **Per-knob discrimination test (mandatory)**: mutate the override → the effective threshold moves under face_pipeline and does NOT move under insightface.
...
**Wave-1 precondition (resolves LC2-02)**: the **coordinator** captures and commits the insightface characterization golden (fixture + `test_insightface_characterization_golden.py`) directly on `feature/fir-6` **before** dispatching wave 1 — the baseline must predate every slice, and one owner avoids two lanes writing divergent goldens. Lane briefs treat the golden as read-only shared infrastructure.
