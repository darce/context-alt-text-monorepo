...
| tests | `apps/prototype-description-service/scene/tests/test_shared_schema_multipart.py` (new) | multipart success and error envelopes validated against the shared schema using the real response builder |
...
- **Contract checks:**
  - `apps/prototype-description-service/scene/tests/test_shared_schema_multipart.py` (new, contracts-owned) validates multipart success and error envelopes against the shared schema using the real response builder, including `timing` and error `code`. Contracts also runs `apps/prototype-description-service/scene/tests/test_describe_run_contract.py` against the real describe-run builder read-only; svc-run-timing retains ownership of that existing test.
...
Paths beginning `scene/`, `recognition/` or `db/` are under `apps/prototype-description-service/`; `src/` and `js/` are under `apps/prototype-wp-alt-context/`. Other paths are repository-relative. Each lane owns at most three source/artifact files plus its tests and test fixtures. New files are explicitly marked. The exact Python test files below are owned by their row's lane, except the explicitly read-only run-schema test in contracts (owned by svc-run-timing); no wildcard ownership or unresolved test placeholder may be dispatched.
