# Richer Caption Context — Fusion Architecture, OpenCV 5, Brand Detection, PG18/19 (PG18 historical)

> **Status:** Assessment / decision input. Not an epic or task plan.
> **Date:** 2026-07-05
> **Task:** `VLM-2` (branch `feature/vlm-2`)
> **PG upgrade:** Superseded 2026-09-16 by the PG17→19 evaluation. PG18 text below is historical context only — it is not current guidance. Canonical timing, gates, and triggers: [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) §11.
> **Question evaluated:** How to raise generated image-description quality by (a) adopting a context-fusion caption architecture (arXiv 2606.18553), (b) using OpenCV 5 for CPU-only scene analysis and dependency reduction, (c) adding user-defined brand/logo detection that auto-tags instances via the clustering engine, and (d) exploiting PostgreSQL 18/19 features (the PG18 hop was later skipped; the target is PG19 in one hop from 17).
> **Source inputs:** arXiv [2606.18553v1](https://arxiv.org/html/2606.18553v1) (Hierarchical Multi-Modal Retrieval for Knowledge-Grounded News Image Captioning, EVENTA 2025); [opencv.org/opencv-5](https://opencv.org/opencv-5/); [PostgreSQL 18 announcement](https://www.postgresql.org/about/news/postgresql-18-released-3142/) (historical); [PostgreSQL 19 documentation](https://www.postgresql.org/docs/19/index.html) · [Release notes 19](https://www.postgresql.org/docs/19/release-19.html); `literature/extracted/refactoring/distilled/`; code (`apps/prototype-description-service`); [identity-prose-merge-design-2026-06-15.md](./identity-prose-merge-design-2026-06-15.md); [segmentation-vlm-pipeline-feasibility-2026-06-15.md](./segmentation-vlm-pipeline-feasibility-2026-06-15.md); [context-aware-image-description-roadmap-2026-06-13.md](../../roadmaps/context-aware-image-description-roadmap-2026-06-13.md); [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) (canonical); [roadmap-pg18-upgrade.md](../../roadmaps/roadmap-pg18-upgrade.md) (superseded 2026-09-16 by the PG17→19 evaluation; historical); [e19-4a-identity-prose-merge-scope.md](../../scopes/e19-4a-identity-prose-merge-scope.md).
...
4. **PG19 one-hop; do not adopt PG18.** Superseded 2026-09-16 by the PG17→19 evaluation. Canonical policy: [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) §11. Stay on PG17 until the hard gates hold (PG19 minor ≥ 19.1 **and** a pinned arm64 pgvector image digest that passed dump/restore rehearsal) **and** the portfolio freeze permits a hop. Bloat, hot-path, video, and CVE-shaped triggers in that §11 do not bypass the freeze except a security fix unavailable on PG17, recorded as an explicit risk decision. Track PG19-specific interfaces for **after** those gates; build nothing against them now. Names/defaults below are **PG19 beta 3 — verify against final release notes at the GA gate**.
...
| YAGNI / Speculative Generality (Fowler) | Identity + brand + WP = three real providers → the provider port is justified *now*; but no plugin frameworks, no speculative Tier-B code, no PG19-dependent paths until the PG19 GA gates in [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) §11 |
...
> **Superseded 2026-09-16 by the PG17→19 evaluation.** Canonical timing, gates, and triggers live in [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) §11. The 2026-07-05 PG18 guidance in this section is historical context only — it is not current guidance, and this assessment does not say to adopt PG18.

**Historical (2026-07-05) — PG18 (GA 2025-09-25):** the original verdict was to adopt per [roadmap-pg18-upgrade.md](../../roadmaps/roadmap-pg18-upgrade.md) — uuidv7 PKs for `brand_templates` + new rows; AIO (up to 3× storage reads) for pgvector bitmap-heap scans used by filtered similarity queries; skip scan on `(tenant_id, …)` composite indexes; virtual generated columns for derived cache keys; pgvector 0.8.x PG18-compatible builds. That intermediate hop is skipped. uuidv7 PKs land on the PG19 hop (or via `recognition/shared/ids.py::generate_id` on PG17 today).

**Current — PG19 one hop from 17.** Hard gate: PG19 minor ≥ 19.1 **and** a pinned image digest that passed dump/restore rehearsal into a **new** named volume (not in-place `pg_upgrade`; `pg_upgrade --check` stays only as an early compatibility probe). Sequence: stop writers (maintenance mode), `pg_dumpall` from the PG17 container, start PG19 on the fresh volume, restore, `ALTER EXTENSION vector UPDATE`, smoke + revoked-key/tenant probes, switch the api to PG19. Rollback = point compose back at the PG17 image and the untouched PG17 volume (no data written to it after the dump; writes after cutover are lost on rollback — Release It! ch-18 expand/rollout/cleanup, rollback before rollout, bounded blast radius). Downtime is the measured dump+restore time from a rehearsal on a copy of the production dump; that rehearsal is an entry gate. The only freeze exception is a security fix unavailable on PG17, recorded as an explicit risk decision. pgvector entry gate (DDIA ch-4 encoding/schema evolution and durability of restored indexes): pin the pgvector PG19 image digest, run `ALTER EXTENSION vector UPDATE`, a vector-index smoke query on restored data, and record the extension version before/after.

**Now (PG17), independent of the hop:**

- Enable `pg_stat_statements` before any trigger or exit criterion uses it: `shared_preload_libraries` on the db service command, restart, `CREATE EXTENSION pg_stat_statements`, least-privilege read role, baseline capture.
- lz4 TOAST is **not** a PG19 benefit. It is independent PG17 config (`default_toast_compression=lz4`); it affects only newly written values; there is no recompression plan.

PG19-specific interfaces earmarked for the caption workload — **PG19 beta 3 — verify against final release notes at the GA gate** before any phase relies on the name or default:

- `INSERT … ON CONFLICT DO SELECT … RETURNING` — one-round-trip get-or-insert at `scene/application/description_repository.py::insert_or_get_existing` (`get_by_cache_key` + `begin_nested` + `IntegrityError` re-select today). `scene/interface_adapters/http/routers/describe.py` calls `DescribeOperationRepository.accept` (`scene/application/describe_operation_repository.py::accept`); that path has no conflict/get-or-insert and is **not** an `ON CONFLICT DO SELECT` site.
- `REPACK CONCURRENTLY` / `max_repack_replication_slots` — online bloat reclaim on churny `media_identities` / description-cache tables (today's answer is VACUUM FULL downtime). When a REPACK Make target is added (PG19 roadmap Phase 4), it performs its own free-space precheck (free space ≥ 2× the target table plus its indexes) and refuses otherwise; a compose disk healthcheck does not protect REPACK. If `REPACK CONCURRENTLY` needs `wal_level=logical`, set that restart-only cluster-wide setting persistently at the PG19 cutover restart; measure WAL volume overhead in the rehearsal; document replication-slot lifecycle (max slots, orphan-slot check and drop). Never toggle `wal_level` per make-target run.
- Also track under the same beta-3 label: `pg_stat_autovacuum_scores`, `pg_stat_lock`, adaptive `io_*`, `log_lock_waits` default, `RETURNING OLD/NEW`, autovacuum parallel workers. SQL/PGQ property-graph queries remain speculative for cluster/identity relationship queries.

GA-gate checklist item for this assessment (verify each name/default against final release notes before any caption-path phase relies on it): `ON CONFLICT DO SELECT`, `REPACK` / `max_repack_replication_slots`, `pg_stat_autovacuum_scores`, `pg_stat_lock`, adaptive `io_*`, `log_lock_waits` default, `RETURNING OLD/NEW`.
...
9. **PG19 upgrade** — not independently schedulable. Canonical policy: [roadmap-pg19-upgrade.md](../../roadmaps/roadmap-pg19-upgrade.md) §11. Frozen with the rest of the portfolio until production data exists (or a PG17-unavailable security fix, recorded as an explicit risk decision). PG18 is skipped. No PG19-dependent code until the hard gates.
