...
- Runtime-parity / environment checks:
  - `cd apps/prototype-description-service && ACX_EVAL_LIVE=1 uv run python -m scripts.eval_harness.cli seed-roster --entities "$GOLDEN_IMAGES_DIR/mock_entities"` — idempotent eval-tenant seeding; verify by fetching `/recognition/media/identities` for a seeded crop.
  - `cd apps/prototype-description-service && ACX_EVAL_LIVE=1 uv run python -m scripts.eval_harness.cli run --manifest scene/tests/seed/golden.json --limit 3` — live smoke: auth via existing `.env` key, analyze→poll→identities round trip, timeout handling, run-record shape. (The `cd` prefix is load-bearing: the repo root has a different `scripts/` package.)
- Contract/fixture verification:
  - **Broken as written (rg-006):** `score --run-record <committed baseline> --check-determinism` exits **1** on every committed baseline run-record (`VLM-2A-baseline-20260706-run-record.json`, `VLM-2C-seeded-stub-run-record-20260707.json`) with `ReportError: … identities[…] must be a dict identity row … got str`. Restoring a green baseline re-score needs a code/record lane (identity row shape and/or re-fetch), not a doc change. `--check-determinism` is declared on `score` / `run` / `score-face` only — not on `fetch`.
  - **Working determinism evidence today:** `cd apps/prototype-description-service && uv run --extra dev pytest scene/tests/test_eval_harness_cli.py -k score_check_determinism_runs_cross_process -q` (cross-process guard over a matched fixture). Operator gate/verdict/exit reference: `apps/prototype-description-service/scripts/eval_harness/README.md` § Score gates, verdict, and `--check-determinism`.
- Manual verification:
