...
- **Provisioned (new, 2026-07-12)**: an OCI A10 burst instance exists in the tenancy after `MAINT-oci-a10-quota-request-20260710`. That request unblocking **is not** machine-readable A10 quota confirmation: every live spike artifact still records `oci_capacity.a10_quota_confirmed=false` with `source=pending_operator_oci_console_or_cli_check`. Do not treat "instance exists" or "quota request unblocked" as Slice 7a quota proof. Operator must capture `oci limits` / console output on the live host before flipping that field.
...
- [ ] 7a: bench script `scripts/gpu_spike_bench.py` committed; spike artifact fully populated on the live A10 (cold-boot, warm-start p95 vs 90 s, model-load, s/img). Quota fields remain `a10_quota_confirmed=false` until operator `oci limits` / console evidence is captured; do not tick this on measurement completion alone.
