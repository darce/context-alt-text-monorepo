...
> Canon: heuristics-canon **v0.17.2** (bundle root `canon/lexicons/`, sibling of
> `repo/` — not under `repo/`). Historical plan pin `v0.17.0-11-ga238620` was a
> pre-tag `git describe` string for commit `a238620`, which resolves as
> **v0.17.1** (`git describe --tags a238620`); there is no
> `canon/PROVENANCE.txt` in this bundle. DEPICT-1 load-bearing IDs
> (`ATTRIB-07`, `ATTRIB-01`/`03`/`04`/`05`, `BOUND-03`, `API-09`/`10`,
> `TEST-15`, `REF-29`) re-verified against this bundle's lexicon definition
> anchors at v0.17.2.
...
DEPICT-0 section **Consumers:** (`:476-567`; re-verify after concurrent
...
- **File ownership (hard boundary).** This lane edits only the **closed**
  path set below (assessment `#### §6e ownership table` Lane B row at
  `:380` — under heading
  `### 6e. Dispatch shape — file ownership, not just task ownership` at
  `:355` — is the collision oracle for the wave and supersedes any open
  wording):
  - `scene/interface_adapters/http/schemas/requests.py` and co-located
    schema helpers under `scene/interface_adapters/http/schemas/`
    **including only** the new
    `scene/interface_adapters/http/schemas/context_contract.py` (no other
    new schema module without amending this bullet **and** the assessment
    Lane B row)
  - Domain `StrEnum` vocabularies in `scene/domain/description.py` (house
    pattern for `DescriptionAdapterKind` et al.; real path ownership of
    `scene/domain/description.py`)
  - **Exactly these four test files** under `scene/tests/`:
    - `test_context_pack.py` — **sole editor this wave** for DEPICT-1
      voice-related edits; "co-owned" in this plan means shared
      **pre-existing coverage**, not dual-lane concurrent editing
    - `test_context_voice_contract.py` (new, Lane B sole)
    - `test_description_register_contract.py` (new, Lane B sole)
    - `test_gravity_contract.py` (new, Lane B sole)
  - **Collision rule:** any fifth test module or extra schema file
    requires amending DEPICT-1 `## Constraints` **and** the assessment's
    `#### §6e ownership table` Lane B row before the edit.
  Do **not** edit
...
- **Prompt text is out of scope.** The weave-instruction half of F1 has nowhere
  ...
  `scene/prompts/caption_system.py` (DEPICT-0 `:452-455`: *"A future promotion
  replaces `PRODUCTION_PROMPT` (or rebinds the selected instance) with a new
  ...
  body export (DEPICT-0 section **Consumers:**, `:476-567`). The
...
- Production weave clause ("Weave the people's names and factual details…") is
  ...
  `:476-567`: selected-config consumers bind via the accessor functions; the
...
- Owned code: `scene/interface_adapters/http/schemas/requests.py`,
  ...
  `scene/domain/description.py`
- Characterization tests: the four test files named in `## Constraints` —
  `scene/tests/test_context_pack.py` (present in this checkout; dump-equality
  assert L139) plus the three new Lane B sole modules
  `test_context_voice_contract.py`, `test_description_register_contract.py`,
  `test_gravity_contract.py`
...
|---|---|---|---|---|---|
| `POST /scene/describe/multipart` request JSON (`context_pack`) | Lane B (this task) | `ContextPack` + nested models, `extra="forbid"` | Additive `voice` on speech-allowlist nested models (not on `IdentityContext` or `IdentityPolicyContext`); defaults applied server-side | Shape: omit → default; invalid enum → 422 ([API-09]). Observable default keys + `context_hash` change: Greenfield Policy | `test_context_pack.py` + `test_context_voice_contract.py` (param IN/OUT inventory; both in the four-file `## Constraints` set) |
| `DescribeImageEnvelope` | Lane B | tenant/media/context/context_pack/decorative/tier | Additive defaulted `description_register`, `gravity` | Shape: omit → defaults ([API-09]) | `test_description_register_contract.py` + `test_gravity_contract.py` (four-file `## Constraints` set) |
...
| `scene/infrastructure/vlm/gpu_remote_adapter.py` | Today: `_SYSTEM_PROMPT` L28–37 + weave L33–34; production serialization path `_render_context` L73–87 / `_user_text` L90–101 / `_describe` L176–182. Post-DEPICT-0: local constant deleted; body lives in `caption_system.py`; adapter **snapshots once** at `__init__` via `production_prompt()` and posts `self._production_prompt.body` (DEPICT-0 section **Consumers:** `:476-567` — not a describe-time `production_system_prompt()` re-read; not the accessor-definition block at DEPICT-0 `:413-455`, which itself reads `.body` inside the accessor). F1 (b) weave is OUT of this wave / unowned. |
| `scene/prompts/caption_system.py` | **Created by DEPICT-0.** Post-seam home of selected frozen `PRODUCTION_PROMPT` (`body` + `lineage_version`) and accessors `production_prompt()` / `production_system_prompt()` / `production_prompt_or_task_version()`. **Consumer bind rule** (DEPICT-0 section **Consumers:** `:476-567`): only permitted binding form for production consumers **of the selected config** is the accessor functions; do **not** read `PRODUCTION_PROMPT.body` / `.lineage_version` at the call site **via `from … import PRODUCTION_PROMPT`**, and do **not** introduce a dedicated frozen body export. The adapter posts `self._production_prompt.body` off its construction-time snapshot (required; not a call-site module bind). **Mutation rule:** REBIND-ONLY — replace `PRODUCTION_PROMPT` with a new `ProductionPrompt(body=…, lineage_version=…)`; never assign to `.body` on the frozen instance (`FrozenInstanceError`). A rebind is **not** retroactive on already-constructed adapters that bound body/stamp earlier. No separate `PRODUCTION_SYSTEM_PROMPT` string global. |
| `scripts/eval_harness/bakeoff.py` | Lane A / DEPICT-0 — harness prompt seam. **Pre-DEPICT-0 today:** inline registry at `bakeoff.py:138-141` constructs `PromptVariant("v1", …)` once; sole consumer read at `:595` is `variant = PROMPT_VARIANTS[self.prompt_variant]` then the stored `variant.system` / `variant.system_long`. **Post-DEPICT-0 (mandatory sole construction helper):** introduce `build_prompt_variants() -> dict[str, PromptVariant]` and set module-level `PROMPT_VARIANTS = build_prompt_variants()` (DEPICT-0 section **Consumers:** harness bullet; helper body is the sole construction site). Inside `build_prompt_variants`, v1 `system=` is resolved through `production_system_prompt()` (or `production_prompt().body`) **at the moment the helper runs** — **not** a live use-site accessor re-read at `:595`, **not** `from … import PRODUCTION_PROMPT` (DEPICT-0 `## Proposed Solution` → **Consumers:** `:476-567`, and **Parity temporal scope (committed option (a))** under that Consumers block + under `### Slice 1: Single prompt source + non-vacuous parity test`). A future F1 (b) rebind of `PRODUCTION_PROMPT` is visible to bakeoff v1 **only** under same-construction-epoch parity: a **post-rebind** `build_prompt_variants()` call (rebuilt registry) or a fresh process import that re-runs the helper — it does **not** rewrite an already-built / already-imported `PROMPT_VARIANTS["v1"].system` string held on a frozen `PromptVariant`. Full harness F1 voice-honouring is **OUT / unowned** (no harness pack `voice`). |
...
- **Do not** edit `scene/tests/test_response_schema_parity.py` /
  ...
  introduced.
- **Frozen contract set + exhaustiveness** (same sole new schema module
  `context_contract.py` from `## Constraints` — no adjacent new module). Do
  **not** use `set(DescriptionRegister)` as the permanent green —
...
- Add to `scene/domain/description.py`:
  ...
  ```
- Add request model (in `requests.py` or imported from `context_contract.py`):
  ```python
...
- [ ] Confirmed no edits under Lane A or Lane C paths; cross-lane deps recorded
      ...
      `:476-567`; never mutate frozen `.body`; rebind not retroactive on
...
- [ ] F1 production weave explicitly **not** claimed closed — F1 (b) is **OUT
      ...
      `PRODUCTION_PROMPT`; consumers of the selected config bind via accessors;
      adapter posts `self._production_prompt.body` from construction-time
      snapshot — DEPICT-0 section **Consumers:** `:476-567`; no
...
- Prompt / weave instruction edits — **F1 (b) production-weave**, **OUT of this
  ...
  `:476-567` forbids `from … import PRODUCTION_PROMPT` and a dedicated frozen
  body export, and forbids describe-time accessor re-read of the body; do
  **not** cite the accessor-definition block at DEPICT-0 `:413-455` as that
...
- Harness `ContextPack` voice mirror (`scripts/eval_harness/manifest.py`
  ...
  system wording **only for a post-rebind `build_prompt_variants()` rebuild or
  a fresh process import** (same-construction-epoch; DEPICT-0 parity option
  (a)) — never by live-tracking an already-built `PROMPT_VARIANTS["v1"].system`
  — and that wording lockstep still does **not** deliver harness pack-voice
  honour.
