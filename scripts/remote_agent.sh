...
# Bound hung `codex exec ... ping` probes (MAINT-ORCHW2-02). Codex ping children
# reparent to init, accumulate under user.slice, and starve bubblewrap forks
# once pids.current approaches pids.max. timeout(1) caps a live probe; the
# orphan reap is the backstop for probes whose parent already died.
PING_TIMEOUT_SEC="${WORKBAY_REMOTE_AGENT_PING_TIMEOUT_SEC:-30}"
PING_ORPHAN_AGE_SEC="${WORKBAY_REMOTE_AGENT_PING_ORPHAN_AGE_SEC:-600}"
case "$PING_TIMEOUT_SEC" in *[!0-9]*|"") echo "remote_agent: PING_TIMEOUT_SEC must be a non-negative integer" >&2; exit 2 ;; esac
case "$PING_ORPHAN_AGE_SEC" in *[!0-9]*|"") echo "remote_agent: PING_ORPHAN_AGE_SEC must be a non-negative integer" >&2; exit 2 ;; esac

# >>> orphan-ping-hygiene (MAINT-ORCHW2-02)
reap_orphan_codex_pings() {
    # Kill init-reparented `codex exec ... ping` processes older than $1 seconds.
    # Best-effort / fail-open: a ps miss must never wedge doctor or dispatch.
    _age="${1:-600}"
    command -v ps >/dev/null 2>&1 || return 0
    _killed=0
    # etimes is Linux/procps seconds-alive; the gate VM is Linux.
    while read -r _pid _ppid _etimes _args; do
        [ -n "${_pid:-}" ] || continue
        [ "${_ppid:-}" = 1 ] || continue
        case "${_etimes:-}" in *[!0-9]*|"") continue ;; esac
        [ "$_etimes" -gt "$_age" ] || continue
        case "$_args" in
    ...
    done < <(ps -eo pid=,ppid=,etimes=,args= 2>/dev/null || true)
    if [ "$_killed" -gt 0 ]; then
        echo "remote_agent: reaped ${_killed} orphan codex ping probe(s) older than ${_age}s" >&2
    fi
    return 0
}
# <<< orphan-ping-hygiene
...
case "$cmd" in
doctor)
    reap_orphan_codex_pings "$PING_ORPHAN_AGE_SEC" || true
    "${SSH[@]}" 'reap_orphan_codex_pings() {
    _age="${1:-600}"
    command -v ps >/dev/null 2>&1 || return 0
    _killed=0
    while read -r _pid _ppid _etimes _args; do
    ...
    [ "$_killed" -gt 0 ] && echo "remote_agent: reaped ${_killed} orphan codex ping probe(s) older than ${_age}s" >&2
    return 0
}
reap_orphan_codex_pings '"$PING_ORPHAN_AGE_SEC"' || true
...
reap)
    ...
    "${SSH[@]}" bash -s -- "$AGENT_ROOT" "$REAP_IDLE" "$REAP_DRY" \
        "$REAP_AUTHORIZED_ONLY:$REAP_ELIGIBLE" <<'REAP_EOF'
set -euo pipefail
agent_root="$1"
idle_limit="$2"
dry_run="$3"
...
if ! flock -n "$reap_lock_fd"; then
    ...
    printf '{"reaped":0,"would_reap":0,"skipped_live":0,"skipped_locked":0,"bytes_freed":0,"would_free_bytes":0,"sandbox_count_after":%s}\n' "$(_sandbox_count)"
    exit 75
fi
...
printf '{"reaped":%s,"would_reap":%s,"skipped_live":%s,"skipped_locked":%s,"bytes_freed":%s,"would_free_bytes":%s,"sandbox_count_after":%s}\n' \
...
# MAINT-ORCHW2-02: reap init-reparented `codex exec ... ping` probes before
# sandbox materialization so hung pings cannot accumulate across dispatches.
_ping_age='${PING_ORPHAN_AGE_SEC}'
if command -v ps >/dev/null 2>&1; then
    ...
    while read -r _pid _ppid _etimes _args; do
        ...
        [ "\${_ppid:-}" = 1 ] || continue
        case "\${_etimes:-}" in *[!0-9]*|"") continue ;; esac
        [ "\$_etimes" -gt "\$_ping_age" ] || continue
        case "\$_args" in
            *codex*" ping"|*codex*" ping "*) ;;
            *) continue ;;
        esac
        if kill -TERM "\$_pid" 2>/dev/null; then
            _ping_killed=\$((_ping_killed + 1))
        fi
    done < <(ps -eo pid=,ppid=,etimes=,args= 2>/dev/null || true)
    if [ "\$_ping_killed" -gt 0 ]; then
        echo "remote_agent: reaped \${_ping_killed} orphan codex ping probe(s) older than \${_ping_age}s" >&2
    fi
fi
...
if [ '${AGENT_SPEC_ENABLED}' = 1 ]; then
    ...
    if (
        ...
        # MAINT-ORCHW2-02: a ping probe must not inherit the multi-hour turn
        # budget. timeout(1) on the probe itself is what stops hung pings from
        # accumulating when the parent already died (parent=init).
        _ping_probe=0
        for _el in \${AGENT_ARGV[@]+"\${AGENT_ARGV[@]}"}; do
            [ "\$_el" = ping ] && _ping_probe=1
        done
        if [ "\$_ping_probe" = 1 ] && command -v timeout >/dev/null 2>&1; then
            TW="timeout -k 5 ${PING_TIMEOUT_SEC}"
        fi
