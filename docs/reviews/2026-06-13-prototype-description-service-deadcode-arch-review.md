...
| Slice 9 remaining — extract `RepresentativeSelector`/`CentroidMaintainer` from `AssignmentWriter` (1084 LOC, medium); `ClusterRepository` Protocol prefix-seam split (**high risk** — DOMAIN-5 refuted the broad split, ~20 direct consumers; needs explicit go-ahead) | ⏳ planned | — |
