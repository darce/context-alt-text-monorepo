...
Install dashboard extras for the full interactive TUI::
    ...
    pip install -e ".[dashboard]"   # from apps/prototype-description-service/
...
def _build_textual_app(
    ...
    class DashboardApp(App[None]):
        ...
        def on_mount(self) -> None:
            ...
            table.add_columns(
                "LANE", "STATE", "HEALTH", "PID", "TOKENS",
                "STK", "CYC", "PRES", "EFFORT", "MODEL", "SUMMARY",
            )
        ...
        def _populate_table(self) -> None:
            ...
            status_bar.update(
                f"task={task_ref}  lanes={len(lane_ids)}  updated={now}"
...
def _run_plain_text(
    ...
    from dashboard_live import _format_table, poll_lane_status as _live_poll
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Interactive TUI dashboard for lane worker health.  "
            "Uses Textual when installed; falls back to rich.live."
        )
    )
    parser.add_argument(
