...
- **Plaintext host surface (the problem):** `docker-compose.prod.yml` (postgres/api/worker all `env_file: .env`) and `docker-compose.env.yml` (same) resolved secrets through the `.env` symlink into the env's `secrets/` dir (now the regular `/opt/acx-backend/<env>/.env`); `.env.prod.example:30,38-39` shows `POSTGRES_PASSWORD`, `POSTGRES_DSN`, `POSTGRES_SYNC_DSN` (password embedded); `RECOGNITION_ADMIN_TOKEN` in the service env set (scope §Grounding). systemd `acx-env.service.template` just runs `docker compose … up` in `/opt/acx-backend/{{ENV}}`.
