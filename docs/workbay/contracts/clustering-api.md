...
- `total` counts qualifying clusters after invariant filtering on this request. `truncated` is true only when more qualifying rows exist beyond this page (`total_count > fetched_page`); mapper drops never set `truncated`.
