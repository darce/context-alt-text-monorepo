...
| DD-13 | Support-map asset `recognition/infrastructure/face_pipeline/assets/sface-support-map-v1.json`; integrity via the existing model-manifest sha256 scheme, not a self-hash field; loader `support_map.py::load_support_map`, fail-closed on bad sha256, out-of-range dim, or unknown region. An all-`False` support mask makes `masked_cosine` raise `ValueError`; `score_ladder` catches it per probe and records that probe `measured=False` for Rungs 1/2 rather than failing the run. |
