...
- `total` counts qualifying clusters after invariant filtering on this request. `truncated` is paging only (`fetched_page >= limit`); mapper drops never set `truncated`.
