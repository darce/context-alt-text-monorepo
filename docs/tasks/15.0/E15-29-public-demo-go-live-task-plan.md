...
- **Agent vs operator split**: the agent authors/dry-runs commands and records evidence; the **operator executes** any command that mutates a prod stack — minting the demo tenant/key (preferred: the tailnet `/admin` console per `docs/runbooks/admin-tenant-keys.md` (E15-31); fallback: `manage_api_keys` on the prod VM) and the prod CORS edit + `acx-prod.service` restart. The agent records minted-identity references (never the raw key) in a handoff decision.
...
- **Mint demo tenant + key (operator-executed)**: **preferred path is the tailnet `/admin` console** (`docs/runbooks/admin-tenant-keys.md`, E15-31) — the operator creates the tenant and mints the key in the browser and pastes it into `secrets/.env` `WORDPRESS_CONFIG_EXTRA`. **CLI fallback**: the agent prepares the exact `tenant-mint-runbook.md` commands (explicit UUID, `--site-url <chosen-url>`, `manage_api_keys --env prod` quirk); the **operator** runs the final `tenant create` + `create` against the **prod** stack on the VM and pastes the key into `secrets/.env` `WORDPRESS_CONFIG_EXTRA` only. Agent records a handoff decision noting the minted tenant UUID (not the key).
