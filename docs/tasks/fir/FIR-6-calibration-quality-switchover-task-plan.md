...
3. **Wiring (resolves LC-02/GR-04/GR2-01)**: in `orchestrator.py` between `evaluate_chunk_candidates` and `_persist_accepted_assignments` (`:535-546`), face_pipeline profile only. `resolve_photo_conflicts` returns the **post-joint accepted set + loser identity ids**, and the orchestrator **rebinds `accepted_decisions`, `accepted_ids`, and the chunk outcome counts from the solver output before BOTH the persist call and `partition_unclustered`** — conflict losers must flow into the new-cluster/unknown partition, never orphan (GR2-01). Plus two invariant guards:
   ...
   - **Persistent same-photo uniqueness guard** (resolves GR2-02/LC-2 residue): implemented inside `AssignmentWriter.persist_assignments_chunk` (`assignment_writer.py:321` — the ONLY path the orchestrator's bulk persist uses, via `orchestrator.py:624`) or a shared helper both persist paths call; **gated on the face_pipeline profile + `joint_assignment_enabled`** (an unconditional guard would change insightface cross-batch behavior — dark break). Rejected duplicates route to the unknown path. Test exercises the **bulk path through the orchestrator**, not `persist_assignment`.
