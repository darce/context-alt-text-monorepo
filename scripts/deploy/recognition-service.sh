...
sanitize_deploy_diagnostic() {
  LC_ALL=C tr -d '\000-\010\013-\037\177-\237' \
    | sed -E \
      ...
      -e 's/["'"'"'](token|access_token|refresh_token|password|passwd|secret|api[-_]?key|[A-Za-z0-9_]*_token|[A-Za-z0-9_]*_password|[A-Za-z0-9_]*_secret|[A-Za-z0-9_]*_key|pgpassword)["'"'"'][[:space:]]*:[[:space:]]*[^,}"'"'"'[:space:]]+/"\1": [REDACTED]/gI' \
