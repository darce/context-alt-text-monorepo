...
- New make target + script (`worktree-reap`) is repo-local (`scripts/worktree_reap.py`, `mk/lane-maintenance.mk`); it shells out to Git and reads authoritative ownership through the required `workbay_handoff_mcp` package, but never writes handoff state or findings. `--json` and `--check` need that backend too; `make worktree-reap*` is the supported entry point.
...
The reaper performs Git inspection/mutation plus a read-only ownership query through `workbay_handoff_mcp`; it never writes the handoff database. `--json` and `--check` require that registry backend too, so `make worktree-reap*` is the supported entry point. `land-1-hygiene` owns handoff-row closure and findings updates; for redundant rows it performs the conditional close after the successful pre-reap classification/cleanliness preflight and before the reaper apply, then reads back the row revision. If the later Git apply fails, hygiene compensates the row state from a fresh classification instead of claiming completion.
