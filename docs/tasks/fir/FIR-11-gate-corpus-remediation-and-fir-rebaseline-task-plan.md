...
> - **Author**: Claude Opus 5 (high) — rev 2 after `plan-analyze` (12 findings) + 5-lane remote grok-4.5 adversarial flock (21 findings)
...
Declared before any labeling or scoring work begins. Changes to this block require a new review pass.
...
| **Primary endpoint** | Paired per-subject identification correctness, buffalo_l vs candidate leg, on identical detected inputs. |
| **Secondary endpoints** | `false_split`, `false_merge`, non-mated reject rate **[EVAL-18]**, per-stratum FMR/FNMR **[CAL-01]**. |
...
| Occlusion measured only on synthetic twins | `synthetic_occlusion.py:724 filter_headline_probes` strips occluded faces from the headline set | live **[MLDATA-20]** exposure |
...
- Canon (**unpinned — canon versions are mutable, rule IDs are idempotent; consult current canon at implementation time**), `~/Development/heuristics-canon-research/lexicons/`, each rule verified present before citation:
  - *Circularity / design*: EXP-03, EXP-12, EXP-19, EXP-20, EXP-22 (`epistemics.md`); OBS-09 (`engineering.md:466`); MEAS-02 (`epistemics.md:199`)
  - *Audit / sampling*: AUDIT-04 (`ml-systems.md:268`), AUDIT-05, AUDIT-07 (`:271`), AUDIT-08 (`:272`), AUDIT-11 (`:275`)
  - *Data*: MLDATA-08 (`:113`), MLDATA-09, MLDATA-18, MLDATA-20, MLDATA-21, MLDATA-22; PROV-05 (`:394`)
  - *Eval / embeddings*: EVAL-16 (`:231`), EVAL-17 (`:232`), EVAL-18 (`:233`); EMB-02 (`:159`), EMB-07 (`:164`), EMB-10; CAL-01 (`:293`), CAL-09 (`:301`); FAIR-05 (`:371`)
  - *Human loop*: HITL-03 (`:423`), HITL-09 (`:429`)
  - *Engineering*: TEST-08 (`engineering.md:377`), TEST-15 (`:384`), DIAG-03 (`:435`); COST-04 (`ml-systems.md:508`)
...
| backend | `scripts/eval_harness/face_metrics.py` | `subject_level_rollup` **(new)**; `mcnemar_exact` **(new)**; `deff_adjust` **(new)**; `nonmated_reject_rate` **(new, [EVAL-18])**; `fuse_occasion_template` **(new, [EMB-07])**; guard `detection_pr` (`:151`) against `roster_only` |
...
- Manual: operator labels 10 queue items and confirms no cluster, name, box, or count hint is visible; the gold items are indistinguishable; the inconclusive channel is reachable.
...
| …after adjudicating the 36 scrape-pattern entries (worst case, all dropped) | **123** — see Slice 1 |
...
Shortfall: **~9× on the merged pool, ~39× on golden150 as-is, ~51× after scrape adjudication.** Re-labeling changes none of these numbers — it mints no new occasions, no new hard-condition captures, no new multi-shot identities, and no `similar_people` pairs (that stratum has zero members and no amount of re-boxing invents lookalikes).
...
- **Mis-attested provenance is the larger defect (measured).** Grepping the 144 provenanced entries for the Instagram CDN signature (`handle_<9+digits>_<9+digits>_<9+digits>_n.jpg`, `handle__<15+digits>.jpg`) returns **33 hits, all tagged `operator/mock_entity`**. With the 3 scraped entries among the unprovenanced 6, **36 of 150 entries (24%) are scraped third-party social media**, carrying **37 of 167 named probes (22%)** across 24 identities, 4 of which exist *only* on scraped entries. Two defects compound: `source=operator` is a false attestation for material the operator did not capture, and `license=mock_entity` is a **category error** — `mock_entity` denotes a fabricated identity, while these are real people's posts, so the tag asserts the opposite of the truth. A missing key fails closed; a wrong key passes. Per **[AUDIT-11]** / **[PROV-05]**, mis-attestation is more dangerous than absence.
- **Adjudicate the 36 before dropping anything.** Operator reviews each scrape-pattern entry individually (own repost vs third-party), re-tags survivors with a truthful `source`/`license` pair, drops the rest. `mock_entity` is never a valid tag for a real third-party subject.
- **Consequence: the drop is no longer free.** If all 36 go, the corpus is **104 entries / 123 named probes / 44 identities**, with 10 identities at ≤1 probe; strata become profile 36, clean 36, low_res 22, blur 22, occlusion_other 11, sunglasses 10, masked 3. The Slice 0 ceiling shortfall worsens from ~39× to **~51×** on golden150 alone. Slice 0's table is re-derived from post-adjudication counts, not from 160.
- Add a loader-level filename-heuristic warning so the pattern cannot silently re-enter, and carry the sampling-frame disclosure in the Slice 5 report **[AUDIT-07]**.
...
- Move the coverage check **to `GoldenManifest`** (not `GoldenEntry`) since the mode is manifest-level: `len(face_boxes) == face_count` under `exhaustive`, `len(face_boxes) <= face_count` under `roster_only`. Retire `_face_count_covers_labeled` (`manifest.py:386`).
- Adopt the `face_count` definition from Terminology and assert it in the loader docstring; the old ambiguity is what let corpus646 pass with 74 boxes on frames holding 189 faces.
...
- Keep per-test α = 0.05; raise per-stratum power to `0.80^(1/7) = 0.969` (1.86× N inflation). Do **not** Bonferroni-split α for an IU gate.
...
- **Dependency:** blocked on FIR-9 delivering blind mode in the curation atlas.
...
- [ ] Measurement Contract reviewed and locked before any labeling or scoring work.
...
- [ ] All 36 scrape-pattern entries (33 provenanced + 3 unprovenanced) adjudicated individually; survivors re-tagged with a truthful source/license; `mock_entity` removed from every real third-party subject.
