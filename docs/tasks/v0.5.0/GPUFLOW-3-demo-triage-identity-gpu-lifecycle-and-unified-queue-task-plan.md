...
Design decision (binding): _wait_for_gpu_ready TimeoutError must never mark the whole run FAILED with zero items and no reason. Order: (1) if a non-Unavailable CPU description adapter is resolvable, continue the run on it, keep each item on tier=provisional_cpu, and stamp the run fallback_reason=gpu_warmup_timeout plus provenance.fallback_reason per item (C2 as amended); (2) otherwise finish the run FAILED with a typed terminal detail {code: gpu_warmup_timeout, retryable: true, startup_budget_seconds}. Status/reason values come from the existing StrEnum surfaces (sr-007); add members there if they live in this file, else report a typed blocker naming the enum file. Scene DB fixtures may hang in-turn: prove collection with --co -q.
...
Merge-gate wave. App.test route boot expects #/description-history; spa-queue-drafts-mount redirects the history route into the filtered queue. Owned: `apps/prototype-wp-alt-context/js/admin/__tests__/App.test.tsx`. Test: `npx vitest run js/admin/__tests__/App.test.tsx`.

### Slice Z2: gate-bulk-cta

Merge-gate wave. BulkDescribeCta lost Cancel describe run, the waiting and cooldown copy and the Review drafts link after the strip moved; BulkDescribeProgress Review drafts href changed. Owned: `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/BulkDescribeCta.test.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/BulkDescribeProgress.phases.test.tsx`, `apps/prototype-wp-alt-context/js/admin/hooks/useBulkDescribe.ts`, `apps/prototype-wp-alt-context/js/admin/hooks/__tests__/useBulkDescribe.test.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/workbench/MediaSelection.tsx`. Test: `npx vitest run js/admin/pages/workbench/__tests__/BulkDescribeCta.test.tsx js/admin/pages/workbench/__tests__/BulkDescribeProgress.phases.test.tsx js/admin/hooks/__tests__/useBulkDescribe.test.tsx`.

### Slice Z3: gate-suggest-ceiling

Merge-gate wave. MediaAltSuggest warming tests still assume the 120 s ceiling and an ETA auto-retry; spa-suggest-ceiling moved the ceiling to the server startup budget. Owned: `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/MediaAltSuggest.test.tsx`, `apps/prototype-wp-alt-context/js/admin/hooks/useDescribeMedia.ts`. Test: `npx vitest run js/admin/pages/workbench/__tests__/MediaAltSuggest.test.tsx js/admin/hooks/__tests__/useDescribeMedia.test.tsx`.

### Slice Z4: gate-cluster-unlink

Merge-gate wave. IdentityClusterList expects Unlink only for singleton clusters; the person-card change now shows it on multi-member clusters. Owned: `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/IdentityClusterList.test.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/workbench/identity-clusters/IdentityClusterItem.tsx`. Test: `npx vitest run js/admin/pages/workbench/__tests__/IdentityClusterList.test.tsx js/admin/pages/workbench/identity-clusters/__tests__/IdentityClusterItem.test.tsx`.

### Slice Z5: gate-scan-cancel

Merge-gate wave. WorkbenchPage integration cannot find the scan Cancel button after Panels.tsx swapped its progress bar for the status strip. Owned: `apps/prototype-wp-alt-context/js/admin/pages/workbench/__tests__/WorkbenchPage.integration.test.tsx`, `apps/prototype-wp-alt-context/js/admin/pages/workbench/Panels.tsx`. Test: `npx vitest run js/admin/pages/workbench/__tests__/WorkbenchPage.integration.test.tsx js/admin/pages/workbench/__tests__/Panels.test.tsx`.

### Slice Z6: fin01-php-recognition-flag

Merge-gate wave. Single-image describe forwards the site's recognition policy (recognition_enabled) on the multipart request, like the bulk run path. Owned: `apps/prototype-wp-alt-context/src/api/services/class-describe-media-service.php`, `apps/prototype-wp-alt-context/tests/Unit/DescribeMediaServiceTest.php`. Test: `vendor/bin/phpunit --filter "DescribeMediaService"`.

### Slice Z7: fin01-svc-recognition-default

Merge-gate wave. The describe service treats an omitted recognition_enabled as false on both the single-image and the describe-run routes. Owned: `apps/prototype-description-service/scene/interface_adapters/http/routers/describe_run.py`, `apps/prototype-description-service/scene/interface_adapters/http/routers/describe.py`, `apps/prototype-description-service/scene/tests/test_describe_run_routes.py`, `apps/prototype-description-service/scene/tests/test_describe_route.py`. Test: `python3 -m pytest apps/prototype-description-service/scene/tests/test_describe_run_routes.py apps/prototype-description-service/scene/tests/test_describe_route.py -q -p no:cacheprovider`.

### Slice Z8: fin02-php-tombstone-lock

Merge-gate wave. Snapshot tombstoning locks the absent clusters it classifies and re-checks curation, confirmation and person binding inside the delete transaction. Owned: `apps/prototype-wp-alt-context/src/sovereign/repositories/class-cluster-snapshot-merger.php`, `apps/prototype-wp-alt-context/tests/Unit/ClusterSnapshotMergerTest.php`. Test: `vendor/bin/phpunit --filter "ClusterSnapshotMerger"`.

### Slice Z9: fin03-php-unavailable-vocab

Merge-gate wave. GPU-control and retention controllers pass the service's typed unavailable reason through instead of collapsing unknown reasons to upstream_5xx. Owned: `apps/prototype-wp-alt-context/src/api/class-gpu-control-controller.php`, `apps/prototype-wp-alt-context/src/api/class-retention-controller.php`, `apps/prototype-wp-alt-context/tests/Unit/GpuControlControllerTest.php`, `apps/prototype-wp-alt-context/tests/Unit/RetentionControllerTest.php`. Test: `vendor/bin/phpunit --filter "GpuControlController|RetentionController"`.

### Slice Z10: fin03-spa-unavailable-vocab

Merge-gate wave. The SPA maps every service unavailable reason to specific, actionable copy instead of generic server-error text. Owned: `apps/prototype-wp-alt-context/js/admin/utils/serviceUnavailable.ts`, `apps/prototype-wp-alt-context/js/admin/utils/__tests__/serviceUnavailable.test.ts`. Test: `npx vitest run js/admin/utils/__tests__/serviceUnavailable.test.ts`.

### Slice Z11: fin04-svc-positional-gate

Merge-gate wave. Ungrounded naming with two or more people abstains (generic draft) until the n>=2 position evaluation is accepted; the n==1 substitution stays. Owned: `apps/prototype-description-service/scene/application/identity_merge/merge.py`, `apps/prototype-description-service/scene/tests/test_identity_merge_merge.py`. Test: `python3 -m pytest apps/prototype-description-service/scene/tests/test_identity_merge_merge.py apps/prototype-description-service/scene/tests/test_identity_merge_grounding.py apps/prototype-description-service/scene/tests/test_describe_run_worker_naming.py -q -p no:cacheprovider`.

### Slice Z12: fin05-php-orphan-discard

Merge-gate wave. Orphan auto-discard re-checks that the entity is really gone (tenant-scoped) inside the discard transaction and writes a durable audit row. Owned: `apps/prototype-wp-alt-context/src/sovereign/sync/class-outbox-maintenance-service.php`, `apps/prototype-wp-alt-context/tests/Unit/OutboxMaintenanceServiceTest.php`. Test: `vendor/bin/phpunit --filter "OutboxMaintenanceService"`.
...
| `fin01-php-recognition-flag` | Z6 | `W/src/api/services/class-describe-media-service.php`<br>`W/tests/Unit/DescribeMediaServiceTest.php` | none | `vendor/bin/phpunit --filter "DescribeMediaService"` |
...
| `fin01-svc-recognition-default` | Z7 | `S/scene/interface_adapters/http/routers/describe_run.py`<br>`S/scene/interface_adapters/http/routers/describe.py`<br>`S/scene/tests/test_describe_run_routes.py`<br>`S/scene/tests/test_describe_route.py` | `svc-warmup-terminal` (S/scene/application/describe_run_worker.py, S/scene/interface_adapters/http/routers/describe_run.py, S/scene/interface_adapters/http/schemas/responses.py, packages/shared-contracts/schemas/scene-describe-run.schema.json) | `python3 -m pytest apps/prototype-description-service/scene/tests/test_describe_run_routes.py apps/prototype-description-service/scene/tests/test_describe_route.py -q -p no:cacheprovider` |
| `fin03-spa-unavailable-vocab` | Z10 | `W/js/admin/utils/serviceUnavailable.ts`<br>`W/js/admin/utils/__tests__/serviceUnavailable.test.ts` | `spa-unavailable-reason` (W/js/admin/pages/RetentionPage.tsx, W/js/admin/pages/settings/GpuControlCard.tsx, W/js/admin/utils/http.ts, W/js/admin/utils/errorTaxonomy.ts, W/js/admin/utils/serviceUnavailable.ts) | `npx vitest run js/admin/utils/__tests__/serviceUnavailable.test.ts` |
| `fin03-php-unavailable-vocab` | Z9 | `W/src/api/class-gpu-control-controller.php`<br>`W/src/api/class-retention-controller.php`<br>`W/tests/Unit/GpuControlControllerTest.php`<br>`W/tests/Unit/RetentionControllerTest.php` | `php-unavailable-reason` (W/src/api/class-retention-controller.php, W/src/api/class-gpu-control-controller.php) | `vendor/bin/phpunit --filter "GpuControlController|RetentionController"` |
| `fin02-php-tombstone-lock` | Z8 | `W/src/sovereign/repositories/class-cluster-snapshot-merger.php`<br>`W/tests/Unit/ClusterSnapshotMergerTest.php` | `php-snapshot-tombstone` (W/src/sovereign/repositories/class-cluster-snapshot-merger.php, W/src/sovereign/repositories/interface-clusters-repository.php, W/src/sovereign/repositories/class-clusters-repository.php, W/src/sovereign/sync/class-snapshot-projector.php) | `vendor/bin/phpunit --filter "ClusterSnapshotMerger"` |
| `fin05-php-orphan-discard` | Z12 | `W/src/sovereign/sync/class-outbox-maintenance-service.php`<br>`W/tests/Unit/OutboxMaintenanceServiceTest.php` | `php-outbox-hygiene` (W/src/sovereign/sync/class-outbox-maintenance-service.php) | `vendor/bin/phpunit --filter "OutboxMaintenanceService"` |
...
| `fin04-svc-positional-gate` | Z11 | `S/scene/application/identity_merge/merge.py`<br>`S/scene/tests/test_identity_merge_merge.py` | `svc-naming-status` (S/scene/application/identity_merge/merge.py, S/scene/application/identity_merge/policy.py); `svc-n1-substitution` (S/scene/application/identity_merge/realizer.py, S/scene/application/identity_merge/merge.py) | `python3 -m pytest apps/prototype-description-service/scene/tests/test_identity_merge_merge.py apps/prototype-description-service/scene/tests/test_identity_merge_grounding.py apps/prototype-description-service/scene/tests/test_describe_run_worker_naming.py -q -p no:cacheprovider` |
...
| `gate-bulk-cta` | Z2 | `W/js/admin/pages/workbench/__tests__/BulkDescribeCta.test.tsx`<br>`W/js/admin/pages/workbench/__tests__/BulkDescribeProgress.phases.test.tsx`<br>`W/js/admin/hooks/useBulkDescribe.ts`<br>`W/js/admin/hooks/__tests__/useBulkDescribe.test.tsx`<br>`W/js/admin/pages/workbench/MediaSelection.tsx` | `spa-status-mount` (W/js/admin/pages/workbench/Panels.tsx, W/js/admin/pages/workbench/MediaSelection.tsx); `spa-queue-drafts-mount` (W/js/admin/pages/workbench/MediaSelectionTableBody.tsx, W/js/admin/pages/workbench/MediaSelection.tsx, W/js/admin/App.tsx); `spa-activity-status` (W/js/admin/hooks/useActivityStatus.ts, W/js/admin/pages/workbench/ActivityStatusStrip.tsx, W/js/admin/hooks/useBulkDescribe.ts) | `npx vitest run js/admin/pages/workbench/__tests__/BulkDescribeCta.test.tsx js/admin/pages/workbench/__tests__/BulkDescribeProgress.phases.test.tsx js/admin/hooks/__tests__/useBulkDescribe.test.tsx` |
...
| `fin01-php-recognition-flag` | 30 | SEC-01 API-09 | Single-image describe forwards the site's recognition policy (recognition_enabled) on the multipart request, like the bulk run path. |
...
| `fin01-svc-recognition-default` | 25 | SEC-01 API-09 | The describe service treats an omitted recognition_enabled as false on both the single-image and the describe-run routes. |
| `fin03-spa-unavailable-vocab` | 35 | API-05 RLSE-04 | The SPA maps every service unavailable reason to specific, actionable copy instead of generic server-error text. |
| `fin03-php-unavailable-vocab` | 40 | API-05 RLSE-04 | GPU-control and retention controllers pass the service's typed unavailable reason through instead of collapsing unknown reasons to upstream_5xx. |
| `fin02-php-tombstone-lock` | 45 | RLSE-05 CARD-02 | Snapshot tombstoning locks the absent clusters it classifies and re-checks curation, confirmation and person binding inside the delete transaction. |
| `fin05-php-orphan-discard` | 45 | RLSE-05 DOM-06 | Orphan auto-discard re-checks that the entity is really gone (tenant-scoped) inside the discard transaction and writes a durable audit row. |
...
| `fin04-svc-positional-gate` | 30 | ATTRIB-05 SEC-01 | Ungrounded naming with two or more people abstains (generic draft) until the n>=2 position evaluation is accepted; the n==1 substitution stays. |
