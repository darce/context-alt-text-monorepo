...
| `apps/prototype-description-service/recognition/infrastructure/repositories/cluster_repository.py` | Extend `get_snapshot()` to: (1) generate a `snapshot_generation_id` UUID per call and stamp each exported row's `last_exported_snapshot_id` column, (2) include `representative_id` and `is_user_selected` (pin state) in the per-cluster snapshot data, (3) return `snapshot_generation_id` in the snapshot response envelope so the WP projector can echo it back during acknowledgement, (4) filter out rows where `disposed_at IS NOT NULL` so disposed rows are excluded from subsequent snapshot exports and tenant data exports. Extend `_to_domain()` if needed so representative metadata flows through to snapshot serialization. |
| `apps/prototype-description-service/recognition/interface_adapters/http/routers/analyze.py`        | Extend `acknowledge-projection` handler: accept optional `snapshot_generation_id` in the acknowledgement payload. When `tenant.retention_mode == 'dispose_after_ack'` **and** `snapshot_generation_id` is present and valid, invoke the disposal marking service to `SET disposed_at = now() WHERE last_exported_snapshot_id = :snapshot_generation_id` on `media_identities`, `identity_clusters`, and `identity_cluster_representatives`. When `retention_mode == 'dispose_after_ack'` but `snapshot_generation_id` is missing or unknown, reject the acknowledgement with HTTP 422 (the dispose contract requires provenance). When `retention_mode` is not `dispose_after_ack`, accept the acknowledgement regardless of whether `snapshot_generation_id` is present (backward compatibility with older WP clients during rollout). |
...
| `apps/prototype-wp-alt-context/src/sovereign/repositories/class-clusters-repository.php` | Update `merge_snapshot_for_tenant()` to write `representative_id` and `is_representative_pinned` from snapshot data. Add `pin_representative(cluster_uuid, representative_id, is_pinned, tenant_id)` method for **immediate** local state update (called by `ClusterMutationsController` before outbox enqueue, matching the optimistic-write contract used by label and merge mutations). |
| `apps/prototype-wp-alt-context/src/sovereign/mappers/class-cluster-response-mapper.php` | When building cluster responses for the admin UI, prefer `representative_id` over `representative_thumb_path` for representative selection. The mapper (not the repository) is the correct seam for response shaping -- `resolve_representative_thumb_path()` is a snapshot-merge helper and should not own representative-selection logic. |
...
- [ ] Update `contracts/cluster-snapshot-api.md` with: (1) `representative_id` and `representative_is_pinned` on cluster items, (2) `snapshot_generation_id` (UUID) in the response envelope, (3) optional `snapshot_generation_id` in the acknowledge-projection request payload.
...
- [ ] Implement `RetentionController::trigger_purge()`: proxies backend `POST /retention/purge`, requires `confirm=true` parameter, returns purge summary. Invalidates transient cache on success. After a successful purge, trigger a forced sync pull (`SyncPullJob::execute()` or equivalent) so the local projection tables are reconciled against the now-empty backend state before cluster queries refetch. Without this step, invalidating frontend query keys would re-read stale local projection rows until the next scheduled sync.
- [ ] Add endpoint URLs to `localize_spa_config()`: `retentionStatus`, `retentionPolicy`, `retentionExport`, `retentionPurge`.
- [ ] Add PHPUnit tests: status endpoint proxies and caches correctly; cache invalidation on writes; unreachable backend returns graceful fallback; policy update forwards to backend; export and purge proxy correctly with confirmation enforcement; successful purge triggers a forced sync pull that reconciles local projection tables.
