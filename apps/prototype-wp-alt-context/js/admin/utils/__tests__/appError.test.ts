...
describe('AppError tags / mutant-killing pins [TEST-15]', () => {
  ...
  it('plain-object AbortError classifies as abort (F3)', () => {
    ...
    expect(classifyError({ name: 'TimeoutError', message: 'timed out' })._tag).toBe('abort');
  });
});
