...
- **License isolation (hard rule, [SC-1])**:
  ...
  - **Publishability is a two-stage boundary — score first, redact only at publish (PROV-01)**: `report._filter_for_public_audience` (`report.py:65-80`) drops non-publishable items **before** scoring in the `Audience.PUBLIC` path (`report.py:400-413`), and `PRIVATE_SOURCES={LOCALWP,OPERATOR}` (`manifest.py:137,253-254`) are hard non-publishable. Unknown-rejection is a **gating** slice whose strangers come from **unpublishable uploads** — so routing the SCORER through the pre-score filter would zero that gate (vacuous). Therefore: **score the full corpus under `Audience.LOCAL`** (no pre-score filter); apply `is_publishable` fail-closed **only to the shared/published artifact** (`Audience.PUBLIC` hub split, `report.py:28-32`), which drops private-source crops/metadata **after** scoring. The published report shows aggregate unknown-rejection rates without exposing private stranger crops; the gating computation runs on the unfiltered local corpus. Never conflate compute-then-redact with the existing compute-on-filtered flow.
...
- **Synthetic firewall**: synthetically-occluded twins (S4) **never** enter the headline (celebs01) gallery or probe. They are scored only in the paired-occlusion measurement (S4/§F) as probes against a gallery built from the identity's **un-occluded enrollment** faces; a twin never shares a gallery with its own original. This prevents an original-in-enroll / twin-in-probe near-duplicate from inflating headline recall or the occlusion Δ.

### E. Assignment — binary open-set (score-time, held-out τ selection)
...
- **τ selection is held-out** (no in-sample optimism): partition probe faces into a **τ-selection fold** and a **reporting fold** by a deterministic seeded ordering (e.g. even/odd rank of `media_id` within each identity). Sweep a fixed grid `τ ∈ {0.20, 0.25, …, 0.90}`; `τ_op = argmax open-set F1 on the SELECTION fold` (ties → smallest τ). **Report identification P/R and unknown-rejection at `τ_op` on the REPORTING fold only.** Full ROC / P-R curves are reported for context.
- If a slice is too small to split above its floor, it is reported **DIRECTIONAL** and the in-sample bias is disclosed — never publish a gate number at a τ fit on the same faces it scores. `τ_op` is **PROVISIONAL — not a product default**.
...
# Fair-SA cohort is a PERSON attribute -> keyed by roster identity, not the image:
Manifest.roster_cohorts: dict[str, str] = {}            # NEW: roster name -> cohort label (S3d)
...
- New `SliceTag` StrEnum in `manifest.py`. Its values are the **per-slice strata named in scope Success criterion 3's slice list** (masked, sunglasses, occlusion_other, profile, low-res, blur, similar-people) — **seven** values: `masked`, `sunglasses`, `occlusion_other`, `profile`, `low_res`, `blur`, `similar_people`.
  - Scope's SC3 slice list also names **`demographic`** — it is **not** a `SliceTag`. Because a cohort is an attribute of a **person, not an image**, cohort labels are keyed by **roster identity** (`Manifest.roster_cohorts: roster_name → cohort`); a probe face's cohort is resolved via its matched named GT box (§C). The image-level `GoldenEntry.demographic_cohort` is an **optional fallback for single-subject celebs01 entries only** and must not be read for multi-face entries (S3d asserts the exclusion) — this stops Fair-SA mis-attributing one identity's error to another's cohort on `similar_people` images.
...
| **S3 Association + gallery + assignment + metrics** | Implement **§C–§F** in `face_assignment.py` (IoU/Hungarian association, seeded gallery split, mean prototypes, binary open-set τ sweep, per-leg space) and `face_metrics.py` (face-level ID P/R, unknown-rejection, single-linkage clustering purity/false-merge/false-split with the exact formulas in §F). Product calibration remains FIR-6. | Can-fail fixtures per metric (TEST-15), each proven to go red: forced wrong-name (drops **both** precision and recall, §F), forced **recall miss** (reject an enrolled identity), forced merge, forced split, forced **impure cluster** (purity < 1), and a **mandatory failing stranger-label fixture** (candidate labels a stranger → unknown-rejection red). IoU matcher unit tests incl. Hungarian on a 3-face fixture + the pixel-corner↔normalized-centre conversion (§A0/§C). Determinism: gallery split + held-out τ sweep + clustering re-derive identically cross-process |
| **S3d Demographic (Fair-SA)** | Per-cohort ID P/R macro keyed by **roster identity** (`Manifest.roster_cohorts`), resolving each probe face's cohort via its matched named GT box (§C); image-level `demographic_cohort` read **only** for single-subject celebs01 entries. Sizing table has **no demographic n-floor** → **DIRECTIONAL always** in this task. Fair-SA sensitivity: one person/cohort must not mask another. No gate role until a floor is scoped later. | Rollup renders per-cohort rates; a multi-face entry carrying only image-level cohort is **excluded** (asserted), never mis-attributed; missing cohorts → empty DIRECTIONAL section with header, not a silent skip |
...
- **Demotion authority is operator-only, with a technical seam (not honor-system).** The bake-off report may only mark an under-floor slice `UNDER-FLOOR / DIRECTIONAL — awaiting operator demotion`; it **cannot** self-mark a slice `demoted`. Demotion is an **operator-recorded MCP decision** under a reserved `operator_gate_*` decision-id namespace whose `actor` is the operator, **distinct from the FIR-5 implementing agent**. FIR-5 `handoff_close_check` and the FIR-6 gate **assert the demotion decision's author ≠ the implementing agent** and that it names each demoted slice + evidence (achieved n vs floor). On the current 0-box corpus **every** gating slice is under-floor, so absent that operator decision the task **cannot** close — this closes the whole-gate self-demotion loophole and preserves SC4 ("never judges its own gate").
...
| Path | Change |
| --- | --- |
| `scripts/eval_harness/manifest.py` | `SliceTag` enum (7 values); `GoldenEntry.tags`; roster-keyed `Manifest.roster_cohorts` + optional single-subject `GoldenEntry.demographic_cohort`; **no** landmark field on `FaceBox` (artifact-cache landmarks instead) |
...
| `scripts/eval_harness/report.py` | `score_face_run_record` (scores the full corpus, `Audience.LOCAL`) / face report builder; floor rollup; gate-proposal section; publishability redaction applied **only at the `Audience.PUBLIC` publish split, post-scoring** (never pre-score — §Constraints two-stage); **do not** force caption path |
...
- [ ] S1: `SliceTag` (7 values) + additive `tags` + roster-keyed `roster_cohorts` (+ optional single-subject `demographic_cohort`); Domain unchanged; tags via manifest authoring; loader rejects unknown tags; enrich_entry identity+detection-GT only (no tags/domain/cohort)
...
- [ ] S3: §A0/§C pixel-corner↔normalized-centre conversion + IoU/Hungarian (Hungarian for ≥2 boxes) association; §D matched-detection gallery split (empty-enrollment → `excluded_no_enrollment`, no NaN) + synthetic firewall; §E **held-out** binary open-set τ sweep (per-leg space); §F ID P/R where wrong-name hits both precision & recall, unknown-rejection, single-linkage clustering (distance cut `1−τ`, local floor) with exact formulas + can-fail wrong-name/recall-miss/merge/split/purity/stranger fixtures; product thresholds deferred to FIR-6
...
- [ ] S4: `landmark_cache.py` fixed offline pass (pinned model_id+sha, no live leg call); seed+affine masked/sunglasses/occlusion_other generators from the cache; both legs re-detect on identical occluded pixels; twins firewalled from headline gallery/probe (§D); ≥90-pair floors; hash-identical pixels
- [ ] S5: `score_face_run_record` + floor-gated rollup; pure score `--check-determinism` (cross-process, sorted lists); detect+embed perf + budget JSON (not full-scan p95); **two-stage publishability** (score `Audience.LOCAL` full corpus, redact only at `Audience.PUBLIC`); operational synthetic↔real divergence; gate proposal only
...
- [ ] Demotion has a technical seam: operator-only `operator_gate_*` decision, author ≠ implementer, asserted by close-check (no whole-gate self-demotion)
