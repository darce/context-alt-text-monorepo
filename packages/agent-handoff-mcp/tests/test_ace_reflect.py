...
from agent_handoff_mcp.orchestration import dashboard_live
...
class TestClassifyRuleReference:
    ...
    def test_multiple_contradiction_keywords(self) -> None:
        for kw in ("missing", "breaks", "fail", "bypass", "incorrect"):
            ...
            assert classify_rule_reference(text, "sr-001") is True, (
                f"keyword '{kw}' should trigger contradiction"
            )
...
def _compute_pending(log_path: Path) -> int:
    ...
    if offset_file.exists():
        try:
            processed = json.loads(offset_file.read_text(encoding="utf-8")).get(
...
class TestAceReflectPendingAdvisory:
    ...
    def test_pending_counts_all_lines_before_apply(self, tmp_path: Path) -> None:
        ...
        log.write_text(
            "\n".join(
                json.dumps({"finding_id": f"F-{i}", "rule_id": "sr-001", "contradicts": False})
                for i in range(8)
            ) + "\n",
            encoding="utf-8",
        )
...
class TestMetricsSummaryLine:
    def test_returns_string_when_modules_unavailable(self, tmp_path: Path) -> None:
        ...
        with patch.dict("sys.modules", {
            "agent_handoff_mcp.orchestration.ace_metrics": None,
            "agent_handoff_mcp.orchestration.ace_reflect": None,
        }):
    ...
    def test_returns_formatted_string_on_success(self, tmp_path: Path) -> None:
        ...
        mock_snap = {
            ...
            "fts5_retrieval": {"data_available": False, "artifact_sources_indexed": 0,
                               "artifact_chunks_fts_count": 0, "handoff_record_counts": {}},
            "lane_health": {"data_available": False},
            "ace_documentation": {"data_available": True, "total_strategy_bullets": 20,
                                  "pruning_candidates": 0, "pruning_candidate_ids": [],
                                  "total_helpful": 10, "total_harmful": 1,
        ...
        with patch.dict("sys.modules", {
            "agent_handoff_mcp.orchestration.ace_reflect": mock_ace_reflect,
            "agent_handoff_mcp.orchestration.ace_metrics": mock_ace_metrics,
        }):
            result = dashboard_live._metrics_summary_line("test-task", tmp_path, tmp_path)
...
class TestAceReflectOnFindings:
    ...
    def test_daemon_path_no_state_dir(self, tmp_path: Path) -> None:
        ...
        inst = self._instruction_file(tmp_path)
        findings = [
            {"id": "F-1", "description": "Rule [sr-001] was violated here."}
        ]
    ...
    def test_manual_path_writes_log(self, tmp_path: Path) -> None:
        ...
        state_dir = tmp_path / "state"
        findings = [
            {"id": "F-2", "description": "[sr-002] is contradicted by this decision."}
        ]
    ...
    def test_manual_path_applies_counters(self, tmp_path: Path) -> None:
        ...
        state_dir = tmp_path / "state"
        findings = [
            {"id": "F-3", "description": "This finding references [rg-001] as contradicted."}
        ]
        ace_reflect_on_findings(findings, [inst], state_dir=state_dir)
    ...
    def test_unknown_rule_ids_filtered(self, tmp_path: Path) -> None:
        ...
        inst = self._instruction_file(tmp_path)
        findings = [
            {"id": "F-4", "description": "This mentions [xx-999] which does not exist."}
        ]
        records = ace_reflect_on_findings(findings, [inst])
        assert records == []
    ...
    def test_multiple_rule_references_in_one_finding(self, tmp_path: Path) -> None:
        ...
        inst = self._instruction_file(tmp_path)
        findings = [
            {"id": "F-5", "description": "Both [sr-001] and [sr-002] are cited here."}
        ]
        records = ace_reflect_on_findings(findings, [inst])
