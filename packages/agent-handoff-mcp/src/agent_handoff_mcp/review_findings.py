...
from . import _shared as _shared_mod
from ._shared import (
...
def record_review_finding(
    ...
    with _get_db_connection() as conn:
        ...
        conn.execute(
            ...
            """,
            (resolved_task_ref, ctx.lane_id, finding_id, severity, file_path, line_start, line_end, description, fix, normalized_review_mode, session, ctx.agent, ctx.branch, ctx.commit_sha),
        )
...
def batch_record_review_findings(
    ...
    for i, item in enumerate(findings):
        ...
        if sev not in REVIEW_FINDING_SEVERITIES:
            return _json_response({"ok": False, "error": f"Item {i} (finding_id={fid!r}): Invalid severity. Valid: {', '.join(sorted(REVIEW_FINDING_SEVERITIES))}"})
        ...
        for item in findings:
            ...
            conn.execute(
                ...
                ON CONFLICT(task_ref, finding_id) DO UPDATE SET
                    ...
                    commit_sha = COALESCE(review_findings.commit_sha, excluded.commit_sha)
                """,
                (resolved_task_ref, ctx.lane_id, finding_id, severity, file_path,
                 line_start, line_end, description, fix, normalized_review_mode,
                 session, ctx.agent, ctx.branch, ctx.commit_sha),
            )
...
def _apply_finding_update(
    ...
    finding_commit_sha = _normalize_optional_text(existing["commit_sha"])
    current_commit_sha = _normalize_optional_text(ctx.commit_sha)  # type: ignore[attr-defined]
    commit_relation = _classify_commit_relation(finding_commit_sha, current_commit_sha)
    ...
    conn.execute(
        ...
        """,
        (status, status, ctx.agent, ctx.branch, ctx.commit_sha, ctx.lane_id, session, status, normalized_resolution_notes, normalized_resolution_notes, status, reopen_transition_int, reopen_transition_int, normalized_reopen_reason, reopen_transition_int, status, normalized_verification_evidence, normalized_verification_evidence, target_db_id, resolved_task_ref),  # type: ignore[attr-defined]
    )
    ...
    payload: dict[str, object] = {
        ...
        "commit_guard": {
            "finding_commit_sha": finding_commit_sha,
            "current_commit_sha": current_commit_sha,
            "current_branch": ctx.branch,  # type: ignore[attr-defined]
            "relation": commit_relation,
...
def _validate_update_finding_input(
    ...
    if normalized_verification_evidence is not None and len(normalized_verification_evidence) > MAX_VERIFICATION_EVIDENCE_LENGTH:
        return {"ok": False, "error": f"verification_evidence must be <= {MAX_VERIFICATION_EVIDENCE_LENGTH} characters."}
    ...
    if status == "open" and normalized_resolution_notes is not None:
        return {"ok": False, "error": "resolution_notes is not supported for status='open'. Use reopen_reason when reopening."}
...
def update_review_finding(
    ...
    input_error = _validate_update_finding_input(
        status, finding_id, finding_db_id, normalized_finding_id,
        normalized_resolution_notes, normalized_reopen_reason,
        normalized_verified_commit_sha, normalized_verification_evidence,
    )
    ...
    with _get_db_connection() as conn:
        ...
        if task_ref is None:
            ...
            if normalized_finding_id is not None:
                rows = conn.execute("SELECT * FROM review_findings WHERE finding_id = ?", (normalized_finding_id,)).fetchall()
            else:
            ...
            if len(rows) > 1:
                ...
                return _json_response({"ok": False, "error": f"Ambiguous finding_id: {len(rows)} rows across task_refs {candidate_scopes}. Pass task_ref explicitly to disambiguate."})
            existing = rows[0]
        ...
        if status == "fixed" and normalized_verification_evidence is None:
            ...
            if guard_error is not None:
                return _json_response(guard_error)
        if status == "fixed":
            guard_error = _check_commit_relation_guard(existing, ctx.commit_sha, normalized_verified_commit_sha, ctx.branch, normalized_resolution_notes)
            if guard_error is not None:
                return _json_response(guard_error)

        return _apply_finding_update(
            conn, existing, status, ctx, session, resolved_task_ref,
            is_reopen_transition, normalized_resolution_notes,
            normalized_reopen_reason, normalized_verified_commit_sha,
...
def list_review_findings(
        ...
        with _get_db_connection() as conn:
            if task_ref is None:
                ...
                if finding_db_id is not None:
                    ...
                    rows = conn.execute("SELECT * FROM review_findings WHERE finding_id = ?", (normalized_fid,)).fetchall()
                if not rows:
                    return _json_response({"ok": False, "error": "Finding not found."})
                if len(rows) > 1:
                    candidate_scopes = sorted({str(r["task_ref"]) for r in rows})
                ...
                resolved_task_ref = _resolve_task_ref(conn, task_ref)
                if finding_db_id is not None:
                    row = conn.execute("SELECT * FROM review_findings WHERE id = ? AND task_ref = ?", (finding_db_id, resolved_task_ref)).fetchone()
                else:
                    normalized_fid = finding_id.strip() if isinstance(finding_id, str) else None
                    if not normalized_fid:
                        return _json_response({"ok": False, "error": "finding_id must not be empty."})
                    row = conn.execute("SELECT * FROM review_findings WHERE finding_id = ? AND task_ref = ?", (normalized_fid, resolved_task_ref)).fetchone()
                if row is None:
                    return _json_response({"ok": False, "error": "Finding not found for task."})
            workspace_git = _workspace_git_context()
            finding = _annotate_review_finding(dict(row), workspace_branch=workspace_git["branch"], workspace_commit_sha=workspace_git["commit_sha"])
            return _json_response({"ok": True, "task_ref": resolved_task_ref, "workspace_git": workspace_git, "filters": {"finding_id": finding_id, "finding_db_id": finding_db_id}, "total_matching": 1, "returned": 1, "has_more": False, "counts": {"status": {str(row["status"]): 1}, "severity": {str(row["severity"]): 1}}, "findings": [finding]})
    ...
    with _get_db_connection() as conn:
        ...
        total, raw_findings = _paginated_query(conn, "review_findings", where_sql, tuple(params), limit, offset, findings_order)
...
def get_review_findings_summary(
    ...
    workspace_git = _workspace_git_context()
    open_findings = [
        _annotate_review_finding(row, workspace_branch=workspace_git["branch"], workspace_commit_sha=workspace_git["commit_sha"])
...
def record_review_run(
    ...
    if subject_kind not in _REVIEW_RUN_SUBJECT_KINDS:
        return _json_response({"ok": False, "error": f"Invalid subject_kind '{subject_kind}'. Valid: {', '.join(sorted(_REVIEW_RUN_SUBJECT_KINDS))}"})
    try:
        normalized_review_mode = _normalize_review_mode(review_mode)
    except ValueError as exc:
        return _json_response({"ok": False, "error": str(exc)})
    if verdict is not None and verdict not in _REVIEW_RUN_VERDICTS:
        return _json_response({"ok": False, "error": f"Invalid verdict '{verdict}'. Valid: {', '.join(sorted(_REVIEW_RUN_VERDICTS))}"})
    with _get_db_connection() as conn:
        ...
        if existing is not None:
            return _json_response({"ok": False, "error": f"review_run_id '{review_run_id}' already exists (id={existing['id']}). Use a unique id for each run."})
        conn.execute(
            """
            ...
            """,
            (
                review_run_id, task_ref, subject_path, subject_kind, normalized_review_mode,
                verdict_decision, verdict, session,
                resolved_actor.agent, resolved_actor.model,
                resolved_actor.model_label, resolved_actor.branch,
...
def list_review_runs(
    ...
    with _get_db_connection() as conn:
        ...
        total, raw_runs = _paginated_query(conn, "review_runs", where_sql, tuple(params), limit, offset, "reviewed_at DESC, id DESC")
    runs = [_row_to_dict(row) for row in raw_runs]
    return _json_response({"ok": True, "filters": {"task_ref": task_ref, "subject_path": subject_path, "review_mode": normalized_review_mode, "verdict": verdict, "limit": limit, "offset": offset}, "total_matching": total, "returned": len(runs), "has_more": (offset + len(runs)) < total, "runs": runs})
