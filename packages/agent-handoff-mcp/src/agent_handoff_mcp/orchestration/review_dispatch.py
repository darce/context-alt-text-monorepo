...
def _run_make_dispatch(orchestrator_root: Path, task_ref: str, lane_id: str, subject: str, message: str, dry_run: bool) -> None:
...
def _has_open_dispatch(task_ref: str, lane_id: str, subject: str) -> bool:
    payload = _json_load(
        list_lane_messages(task_ref=task_ref, lane_id=lane_id, status="open", limit=100)
    )
...
def main() -> int:
    ...
    dispatched: dict[str, dict[str, list[str]]] = {
        issue_kind: {} for issue_kind in ISSUE_KIND_LABELS
    }
    for issue_kind in ISSUE_KIND_LABELS:
        for lane_id, items in sorted(pending_by_kind_and_lane[issue_kind].items()):
