...
# FIR-12 BR-37 / BR-38 / BR-39 — pin coverage incompleteness, overall foil reject, identity co-present count

Canon: MLDATA-07, EVAL-16, EVAL-19, JANUS 2.3.4, TEST-15.

- BR-37 (medium): no production change. Test `test_coverage_gap_marks_complete_searches_incomplete` uses `_plan(a_images=4)` with both shortfalls 0 (complete per-stratum mated+foil lists and concatenated `overall_nonmated`) and asserts A `point.incomplete=True`, `format_fnir()=="incomplete"`, and the same on `overall`. Mutant (`/tmp/lane-f28-mutants/br37`): deleted the `coverage_incomplete.get(name, False)` arm. RED: `assert False is True` (`point.incomplete`); row still `incomplete=True` while the published FNIR would be `0.000`.
