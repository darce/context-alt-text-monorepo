...
Paths beginning `scene/`, `recognition/` or `db/` are under `apps/prototype-description-service/`; `src/` and `js/` are under `apps/prototype-wp-alt-context/`. Other paths are repository-relative. Each lane owns at most three source/artifact files plus its tests. New files are explicitly marked. Required pytest groups below are expanded to exact existing or new test paths in the frozen dispatch manifest; no wildcard ownership or unresolved test placeholder may be dispatched. The service pytest root is `apps/prototype-description-service/scene/tests`; recognition unit/integration tests are under `apps/prototype-description-service/recognition/tests`. Python commands use the provisioned lane-root interpreter, with package-origin verification for Python package lanes.

| Lane ID | Slice | Owned Paths | Upstream Dependencies (artifact transferred) | Required Tests |
| --- | --- | --- | --- | --- |
| `rebaseline` | 0 | `docs/assessments/GPUFLOW-1-wrong-match-and-occlusion-calibration-20260913.md` | operator promotion/evidence gate | recorded SHA/routes and symptom evidence |
| `contracts` | A1/B1 | `packages/shared-contracts/schemas/scene-describe-multipart.schema.json (new)`, `packages/shared-contracts/schemas/scene-describe-run.schema.json`, `docs/workbay/contracts/gpu-lifecycle.md` | rebaseline: persisting-defect assessment; operator: P/J/D/F bounds | schema fixtures; lease/freshness contract review |
| `timing-models` | B1 | `db/migrations/versions/001_identity_schema.py`, `db/models/scene.py` | contracts: timing/operation/lease schema | DB round-trip and expiry tests |
| `timing-repository` | B1 | `scene/application/describe_run_repository.py`, `scene/domain/describe_run.py`, `scene/application/describe_operation_repository.py (new)` | timing-models: durable models; contracts: wire schema | repository concurrency, correlation and expiry pytest |
| `response-models` | B1 | `scene/interface_adapters/http/schemas/responses.py` | contracts: both JSON schemas | strict response serialization pytest |
| `svc-demand` | A1 | `scene/application/describe_load.py` | timing-repository: operation/lease repository; contracts: publication contract | describe_load/controller fake-clock pytest |
| `svc-instrumentation` | B1 | `scene/application/visual_facts_service.py`, `recognition/interface_adapters/http/middleware/metrics.py` | contracts: phase definitions | adapter dispatch/retry and histogram pytest |
| `svc-cold-gpu` | A1/B1 | `scene/interface_adapters/http/routers/describe.py` | svc-demand: lease API; svc-instrumentation: before_compute/attempt timing; response-models: multipart model; timing-repository: retained operation API | multipart route/schema and eligibility pytest |
| `svc-run-timing` | B1 | `scene/application/describe_run_worker.py`, `scene/interface_adapters/http/routers/describe_run.py` | timing-repository: persisted run/item API; response-models: run/item models; svc-instrumentation: attempt timings; svc-demand: aggregate publisher | describe_run phases/routes/items/contract pytest |
| `php-breaker-passthrough` | A2/B2 | `src/api/class-abstract-recognition-proxy-controller.php`, `src/api/services/class-describe-media-service.php`, `src/api/class-settings-controller.php` | svc-cold-gpu: serialized multipart fixtures; svc-run-timing: serialized run/item fixtures | vendor/bin/phpunit --filter "Proxy|DescribeMedia|Settings" |
| `spa-description-service` | A3 | `js/admin/pages/settings/GpuControlCard.tsx`, `js/admin/pages/retention/RetentionPage.tsx` | rebaseline: persisting UI defect evidence | npx vitest run js/admin/pages/settings js/admin/pages/retention |
| `spa-describe-client` | A3/B2 | `js/admin/api/describeApi.ts` | php-breaker-passthrough: final PHP wire fixtures | describe API passthrough/nullable timing vitest |
| `spa-suggest-warming-timing` | A3/B2 | `js/admin/hooks/useDescribeMedia.ts`, `js/admin/pages/workbench/MediaAltSuggest.tsx` | spa-describe-client: typed response/retry operation API | npx vitest run js/admin/hooks/__tests__/useDescribeMedia.test.tsx js/admin/pages/workbench/__tests__/MediaAltSuggest.test.tsx |
| `spa-bulk-timing` | B2 | `js/admin/hooks/useDescribeRunProgress.ts`, `js/admin/pages/workbench/MediaSelection.tsx` | spa-describe-client: typed run/item timing | npx vitest run js/admin/hooks/__tests__/useDescribeRunProgress.test.tsx js/admin/pages/workbench/__tests__/MediaSelection.statusAnnouncement.test.tsx |
| `spa-naming-control` | C1 | `js/admin/pages/workbench/identity-clusters/NameFaceControl.tsx`, `js/admin/pages/workbench/identity-clusters/buildNamingOptions.ts` | rebaseline: persisting naming evidence | npx vitest run js/admin/pages/workbench/identity-clusters/__tests__/NameFaceControl.test.tsx js/admin/pages/workbench/identity-clusters/__tests__/buildNamingOptions.test.ts |
| `svc-suggestion-rep` | C2 | `recognition/infrastructure/repositories/suggestion_repository.py` | rebaseline: candidate/replay evidence | suggestion repository normal/replay/tenant/null pytest |
| `spa-suggestion-cards` | C2 | `js/admin/pages/workbench/identity-clusters/SuggestionCards.tsx` | svc-suggestion-rep: nullable representative fixture; spa-naming-control: naming/group-preview fixtures | npx vitest run js/admin/pages/workbench/identity-clusters/__tests__/SuggestionCards.test.tsx |
| `calibration` | C3 | `docs/assessments/GPUFLOW-1-wrong-match-and-occlusion-calibration-20260913.md` | rebaseline: live-row and quality evidence | numeric calibration report review |
| `svc-rep-settings` | C4 | `recognition/application/settings/clustering.py` | calibration: operator-accepted factor values and precision evidence | representative settings propagation pytest |
| `svc-rep-quality` | C4 | `recognition/application/persistence/representative_selector.py`, `recognition/application/persistence/assignment_writer.py` | svc-rep-settings: representative-only settings contract | five C4 contract tests listed below |
| `spa-picker-undo` | D1 | `js/admin/pages/workbench/identity-clusters/IdentityClusterItem.tsx`, `js/admin/pages/workbench/identity-clusters/MergeUndoBanner.tsx` | spa-suggestion-cards: integrated candidate/null/group-preview fixtures | npx vitest run js/admin/pages/workbench/identity-clusters |
| `php-merge-cap` | D2 | `src/api/services/class-person-merge-service.php` | rebaseline: persisting cap evidence; operator/API-R-23: accepted cap/error contract | vendor/bin/phpunit --filter PersonMerge |
| `ux-service` | A3/B2/C/D | `apps/prototype-wp-alt-context/docs/ux-maps/gpu-operator-control.uxmap.json`, `apps/prototype-wp-alt-context/docs/ux-maps/gpu-operator-control.md` | spa-description-service: final service-state/copy fixtures | render_ux_maps.py --check |
| `ux-suggest` | A3/B2/C/D | `apps/prototype-wp-alt-context/docs/ux-maps/describe-gpu-tier.uxmap.json`, `apps/prototype-wp-alt-context/docs/ux-maps/describe-gpu-tier.md` | spa-suggest-warming-timing: warming/retry/timing fixtures; spa-bulk-timing: bulk display fixtures | render_ux_maps.py --check |
| `ux-identity` | A3/B2/C/D | `apps/prototype-wp-alt-context/docs/ux-maps/workbench-identity-chips.uxmap.json`, `apps/prototype-wp-alt-context/docs/ux-maps/workbench-identity-chips.md` | spa-picker-undo: integrated naming/suggestion/undo fixtures | render_ux_maps.py --check |
| `ux-public` | A3/B2/C/D | `apps/prototype-wp-alt-context/docs/ux-maps/public-demo-describe.uxmap.json`, `apps/prototype-wp-alt-context/docs/ux-maps/public-demo-describe.md` | spa-suggest-warming-timing: public ceiling/error fixtures | render_ux_maps.py --check |

The five C4 tests are recognition `unit/test_representative_quality_gate.py`, `unit/test_representative_selector.py`, `unit/test_fir2_br_postmerge_contracts.py`, `unit/test_fir_final_postmerge_runtime_contracts.py`, and `integration/test_assignment_writer.py`. Settings tests belong to svc-rep-settings; selector/writer contract tests belong to svc-rep-quality. The fixture producer owns edits to its tests; downstream lanes consume committed fixtures read-only and add their own component tests. Materialize test ownership explicitly before dispatch.
...
- Naming-control → suggestion-cards → picker-undo carries the naming/group-preview then integrated candidate/null fixtures listed in the table. Shared-directory location alone creates no edge. The producer owns each fixture; downstream tests must not rewrite it concurrently.
...
- Each UX lane owns its canonical JSON and generated Markdown as a pair and consumes final UI fixtures. No component lane edits those files. Each shared writer is serialized by its named artifact dependency, not an ordering-only edge.
...
Only after freeze/re-review: materialize, re-pin `config/lane-orchestration/GPUFLOW-1.json` (materialize rewrites pins), verify row/model/effort match and validate with `lane_dag`. Expand each test group to exact files and pin fixture ownership before dispatch. This plan-revision lane edits only this document, not the manifest or sibling reports.
