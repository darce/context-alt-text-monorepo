...
#
# Prototype for Plan 0144 S2. Flow proven end-to-end 2026-07-15 (decisions
...
# Concurrent named grok-lane-* scopes on the VM (Plan 0144 S5; [RES-14] backpressure).
...
# Retention cap for PERSISTED per-lane venvs (WB-OFFLOAD-SANDBOX-VENV-REUSE-01 S4):
...
doctor)
    ...
    ;;
build)
    # Plan 0149 / decision 4134 (RES-13 crumple zone): close inherited stdin.
    ...
    # countable + debuggable ([RES-14] concurrency ceiling; Plan 0144 S5).
    LANE_UNIT="grok-lane-${LANE_KEY}"

    # Wall-clock for residual --timeout after pre-dispatch work (probe/push/scp).
    BUILD_START_TS="$(date +%s)"
    # Per-phase progress with elapsed seconds (Plan 0149 observability delta): a
...
# does not destroy it (WB-OFFLOAD-SANDBOX-VENV-REUSE-01). A warm re-dispatch
...
# Bounded disk growth (WB-OFFLOAD-SANDBOX-VENV-REUSE-01 S4): persisted lane venvs
...
printf '%s\n' .brief.md .schema.json .grok-result.json .grok-run.log .grok-debug.log .venv > .git/info/exclude
git add -A
...
# Lockfile-hash sync gate (WB-OFFLOAD-SANDBOX-VENV-REUSE-01 S2): skip 'uv sync'
...
if [ -n "\$_dep_hash" ] && [ -d "\$LANE_VENV" ] && [ "\$(cat "\$SYNC_STAMP" 2>/dev/null)" = "\$_dep_hash" ]; then
    ...
    if ! "\$HOME/.local/bin/uv" sync -q >&2 </dev/null; then
...
# grok off mid-turn (WB-OFFLOAD-SANDBOX-VENV-REUSE-01 S3). When setup alone
