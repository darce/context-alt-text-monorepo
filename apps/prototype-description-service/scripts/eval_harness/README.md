...
| --- | --- | --- |
| `determinism check passed [label]: … (baseline=…; child_seeds=…)` | Cross-process re-score bit-identical under the named child seeds. With `--expect-report`, the line also says `matches --expect-report <path-text>`. **This certifies scoring-path byte-stability only** — not caption quality, not face recognition, not adoption readiness. | None for the scoring path. Do **not** read this as model adoption or as face/caption quality evidence. |
