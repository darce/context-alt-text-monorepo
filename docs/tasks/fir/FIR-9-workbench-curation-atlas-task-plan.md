...
- **Stale run**: run whose `embedding_model` ≠ current active model id (`active_embedding_model_id()` in `recognition/application/embedding/manifest.py:71`). Computed at list time; no background job.
