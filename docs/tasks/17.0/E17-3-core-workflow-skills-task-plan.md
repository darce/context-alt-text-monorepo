...
- Create `scripts/hooks/regenerate-task-views.sh`:
  ...
  - Calls `agent-handoff-mcp --workspace-root "$WORKSPACE_ROOT" dashboard` to regenerate DASHBOARD.md
...
- [ ] Verify hook fires after `record_event` — `DASHBOARD.md` updates while `CURRENT_TASK.md` remains unchanged unless explicitly regenerated
...
- [ ] PostToolUse hook regenerates `DASHBOARD.md` after any covered state-changing write; `CURRENT_TASK.md` remains on-demand only
