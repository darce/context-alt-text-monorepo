...
def test_controller_emits_start_for_stopped_instance_when_work_waiting() -> None:
    ...
    actions = controller.start_needed_instances([instance], queue_depth=1, in_flight=0)
...
def test_controller_does_not_emit_start_when_idle() -> None:
    ...
    assert controller.start_needed_instances([instance], queue_depth=0, in_flight=0) == []
...
def test_controller_does_not_emit_start_for_running_instance() -> None:
    ...
    assert controller.start_needed_instances([instance], queue_depth=4, in_flight=0) == []
...
def test_start_stop_cli_commands_are_symmetric() -> None:
    ...
    normalized_start = ["STOP" if part == "START" else "STOPPED" if part == "RUNNING" else part for part in start_cmd]
...
def test_corrupt_load_json_yields_zero_start_and_zero_stop(tmp_path: Path) -> None:
    ...
    controller = GpuLifecycleController(idle_seconds=60)
    stopped = GpuInstance(instance_id="ocid1.gpu", state="STOPPED", idle_for_seconds=0)
    running = GpuInstance(instance_id="ocid1.gpu", state="RUNNING", idle_for_seconds=90)
