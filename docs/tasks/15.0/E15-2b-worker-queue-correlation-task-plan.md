...
3. **Enqueue path has no hook to forward the ID.** `scan_queue_service.enqueue` and `SqlAlchemyScanQueueRepository.enqueue_items` accept only `(media_id, media_url)` tuples (`apps/prototype-description-service/recognition/application/scan/scan_queue_service.py:103`, `apps/prototype-description-service/recognition/infrastructure/repositories/scan_queue_repository.py:74-100`).
...
- **Worker-origin correlation**: a `correlation_id` generated inside the worker when the claimed row has `correlation_id IS NULL`. Logged with a structured field `correlation_source: "worker"` so downstream log aggregation can distinguish inherited IDs from fallback IDs. (API-origin rows emit `correlation_source: "api"`.)
...
2. **Application repository contract**: change `ScanQueueRepository.enqueue_items` to accept `items: Iterable[tuple[int, str, str | None]]` (media_id, media_url, correlation_id) and persist correlation_id + correlation_source="api" when the id is non-null; "worker" is never written at enqueue time.
3. **Scan queue service**: `ScanQueueService.enqueue` reads `get_correlation_id()` from the E15-2 middleware contextvar once, and threads the same correlation_id through all chunked `enqueue_items` calls for that request. If the contextvar is unset (e.g., unit-test path with no middleware), the column stays NULL and the worker fallback path owns generation.
4. **Worker handler**: `ScanItemHandler._process_item` replaces `request_id = uuid.uuid4()` with:
   - If `item.correlation_id` is non-null → use it with `correlation_source="api"`.
   - Else → `generate_correlation_id()` (imported from the middleware module) with `correlation_source="worker"`.
...
5. **Worker claim path**: `claim_pending_items` / `claim_pending_items_any` return the new `correlation_id` field on `ScanQueueItem`. Add `correlation_id: str | None` to the dataclass.
6. **Fake repository + test doubles**: `recognition/tests/api/conftest.py` and unit-test fakes gain the new column. Call-site hook: `create_scan_job_record` at `analyze.py:216` does not change; only the repository/service signatures change.
...
| Service | `apps/prototype-description-service/recognition/application/scan/scan_queue_service.py` | Read `get_correlation_id()` in `enqueue`, pass through to every `enqueue_items` chunk |
| Worker handler | `apps/prototype-description-service/recognition/worker/handlers/scan.py` | Replace local `uuid.uuid4()` with correlation-aware binding; set contextvar for log filter; reset in `finally` |
| Middleware module (public surface) | `apps/prototype-description-service/recognition/interface_adapters/http/middleware/correlation.py` | No code change; worker imports `generate_correlation_id` and `_correlation_id_var` from here |
...
| Tests | `apps/prototype-description-service/recognition/tests/unit/test_scan_queue_correlation.py` (new) | Unit coverage for enqueue propagation + worker fallback |
| Tests | `apps/prototype-description-service/recognition/tests/worker/test_scan_handler_correlation.py` (new) | Worker handler uses inherited vs fallback ID; logs carry correlation_id |
...
- Update `ScanQueueRepository.enqueue_items` protocol and `SqlAlchemyScanQueueRepository.enqueue_items` implementation to accept and persist correlation fields.
- Update `ScanQueueService.enqueue` to read `get_correlation_id()` once per call and thread it through chunked `enqueue_items`.
...
**Proof**:
- `test_scan_queue_correlation.py::test_enqueue_persists_correlation_id_from_contextvar` — with `_correlation_id_var` set, enqueued rows have matching `correlation_id` and `correlation_source="api"`.
- `test_scan_queue_correlation.py::test_enqueue_without_contextvar_leaves_correlation_null` — contextvar unset → row has `correlation_id IS NULL` and `correlation_source IS NULL`.
...
- `ScanItemHandler._process_item`:
  - Resolve correlation: `correlation_id, correlation_source = (item.correlation_id, "api") if item.correlation_id else (generate_correlation_id(), "worker")`.
  ...
  - Emit `correlation_source` via `extra={"correlation_source": correlation_source}` on the START log line.
- Apply the JSON formatter's existing `CorrelationIdFilter` to the worker's logger tree (if not already inherited from root) in `api/logging_config.py::configure_logging`. Verify by direct handler-list inspection in a test.
...
- `test_scan_handler_correlation.py::test_worker_generates_fallback_when_item_has_no_correlation_id` — logs carry a `req-<uuid7>`-format id plus `correlation_source="worker"`.
...
- **Command**: `PYENV_VERSION=description-service pyenv exec python -m pytest recognition/tests/unit/test_scan_queue_correlation.py recognition/tests/worker/test_scan_handler_correlation.py recognition/tests/unit/test_scan_queue_service.py recognition/tests/api/test_correlation.py -v`
...
- [ ] `ScanQueueService.enqueue` reads `get_correlation_id()` once per call
- [ ] All claim paths return correlation fields
- [ ] Slice 1 test file covers: contextvar-set, contextvar-unset, round-trip through claim
- [ ] Slice 2 RED tests land first
- [ ] Worker handler resolves correlation from item or generates fallback
- [ ] Worker handler sets + resets `_correlation_id_var` per item
- [ ] `request_id=` log fragments removed from worker scan handler
- [ ] `correlation_source` emitted in worker START log extra
