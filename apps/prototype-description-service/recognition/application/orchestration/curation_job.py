    ...
    *,
    ...
    if refresh_service is None:
        ...
        if refresh_after_curation is None:
            logger.warning(
                "[curation_job] refresh_after_curation unavailable tenant_id=%s cluster_ids=%s",
                tenant_id,
                unique_cluster_ids,
            )
            refresh_failed = True
        ...
        for cluster_id in unique_cluster_ids:
            ...
            try:
                ...
                logger.warning(
                    "[curation_job] refresh_after_curation timed out tenant_id=%s cluster_id=%s: %s",
                    tenant_id,
                    cluster_id,
                    exc,
                )
                ...
                logger.warning(
                    "[curation_job] refresh_after_curation failed tenant_id=%s cluster_id=%s: %s",
                    tenant_id,
                    cluster_id,
                    exc,
                )
