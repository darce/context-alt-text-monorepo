...
def test_ssh_invocations_use_l_and_double_dash() -> None:
    ...
    # Judge whole invocations: an ssh command may be split over backslash
    # continuations, and the -l/-- identity form then lands on a later physical
    # line. Joining first keeps the gate strict without flagging correct code.
    script = re.sub(r"\\\n\s*", " ", script)
