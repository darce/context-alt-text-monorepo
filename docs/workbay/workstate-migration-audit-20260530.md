...
Canonical compared at `agentic-protocol-monorepo` v0.1.20 (`packages/workbay-system`).
...
- Canonical wires PreToolUse/PostToolUse guards via `.github/hooks/terminal-guard.json` (VS Code/
  Codex manifest) using the **new `mcp__workbay-handoff-mcp__*` matchers**, and *does* wire
