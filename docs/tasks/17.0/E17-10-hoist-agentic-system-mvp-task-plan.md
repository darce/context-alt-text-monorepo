...
- **Review run 246** (`planning-review-e17-10-2026-04-17-run-02`, this pass) — verdict `conditional_pass`; findings `E17-10-PR2-H-01` / `PR2-M-01..02` / `PR2-L-01..04` addressed by the revision commit that introduces this Prior Review Disposition section.
...
5. **Daemon token cost is invisible and polling is architecturally load-bearing, with no rework note in the repo.** `orchestrator_daemon.py` polls every 60s (sleep at [orchestrator_daemon.py:1204](../../../packages/agent-orchestrator-mcp/src/agent_orchestrator_mcp/orchestration/orchestrator_daemon.py), inside `orchestrator_loop()` at line 1208) and `worker_daemon.py` polls every 30s (`worker_loop` def at [worker_daemon.py:1762](../../../packages/agent-orchestrator-mcp/src/agent_orchestrator_mcp/orchestration/worker_daemon.py)). Each orchestrator cycle runs ≈10–15 MCP queries across `_worker_management_phase()` (def at line 965), `_poll_merge_ready_lanes()` (def at line 98, called from `_worker_management_phase` at line 991), lane-inbox reads, plan-dispatch phase (line 953), and guidance phase (line 908). Worker daemons spawn `lane_prompt.py --check` subprocesses per poll via `poll_lane_state()` at [worker_daemon.py:143](../../../packages/agent-orchestrator-mcp/src/agent_orchestrator_mcp/orchestration/worker_daemon.py). A consumer enabling daemons inherits that cost with no in-product warning. No `TODO`/`FIXME` in the daemon modules flags the pull-vs-push architecture question — the rework signal is load-bearing missing.
...
- `orchestrator_daemon.py:1208` opens `orchestrator_loop()` with an infinite `while True` at line 1234; default poll interval 60s; main-loop sleep at [orchestrator_daemon.py:1204](../../../packages/agent-orchestrator-mcp/src/agent_orchestrator_mcp/orchestration/orchestrator_daemon.py) (additional sleeps at lines 1162, 1188).
- Per-cycle reads dominate token cost: `_worker_management_phase()` (def at line 965) invokes `manage_worker(action="status")` per lane, then `_poll_merge_ready_lanes()` (def at line 98, called at line 991 inside `_worker_management_phase`) iterates all lanes calling `worker_reports(operation="list", fields="merge_ready")`. Dispatch phase (line 887) reads lane inbox + decisions per cycle. Total ≈10–15 MCP queries per 60s.
- `worker_daemon.py:1762` runs `worker_loop()` at 30s per cycle; sleep call sites at lines 1783, 1790, 1796, 1806. `_poll_phase()` (def at line 857, invoked from `worker_loop` at line 1786) calls `poll_lane_state()` ([worker_daemon.py:143](../../../packages/agent-orchestrator-mcp/src/agent_orchestrator_mcp/orchestration/worker_daemon.py)) which spawns `lane_prompt.py --check` subprocesses per poll.
...
- **Aggregator repo and PyPI mirror** — a future `darce/mcp-servers` aggregator repo or a private PyPI mirror remains out of scope and is revisit-post-MVP. Slice 0 itself commits to creating `darce/mcp-agent-orchestrator` and `darce/agentic-bootstrap` as new standalone private repos (mirroring the existing `darce/mcp-agent-handoff`) so every consumer-facing install URL targets a standalone repo and none reference `context-alt-text-monorepo`.
...
- The polling rework note exists at `packages/agent-orchestrator-mcp/docs/reworks/event-driven-daemon-design-note.md` with four alternatives enumerated (sqlite `update_hook`, filesystem watcher, unix-domain-socket pub-sub, hybrid push-with-fallback-poll), trade-offs, and exact poll-site anchors (`orchestrator_daemon.py:1204` main-loop sleep, `orchestrator_daemon.py:965` `_worker_management_phase` def, `orchestrator_daemon.py:98` `_poll_merge_ready_lanes` def with call at line 991, `worker_daemon.py:857` `_poll_phase` def with call at line 1786, `worker_daemon.py:143` `poll_lane_state`). In-code `TODO(E17-10-REWORK)` comments reference the design note by path.
...
- **MCP packages and the bootstrap CLI ship from standalone repos**, not from `context-alt-text-monorepo`. `agent-handoff-mcp` already lives at `darce/mcp-agent-handoff` (standalone, external); this slice creates `darce/mcp-agent-orchestrator` (extracted from `packages/agent-orchestrator-mcp/`) and `darce/agentic-bootstrap` (new, scaffolded for Slice 3 to populate) so both MCP servers plus the bootstrap CLI ship independently. Consumers install via `pip install "git+ssh://git@github.com/darce/<repo>.git@<tag>"` — no `#subdirectory=` pattern and no install URL references `context-alt-text-monorepo`.
...
- Create `darce/agentic-bootstrap` via `gh repo create darce/agentic-bootstrap --private --description "Bootstrap CLI that clones darce/agentic-system into consumer repos and manages MCP harness config"`. Seed with a README-only initial commit; Slice 3 implements the CLI package directly in this repo (not under `packages/` in `context-alt-text-monorepo`).
- Verify `gh auth status` on account `darce` carries `repo` + `delete_repo` scopes for all three new repos (`darce/agentic-system`, `darce/mcp-agent-orchestrator`, `darce/agentic-bootstrap`) before extraction begins.
...
- Unit tests cover the precedence and current defaults for the existing path surface: explicit `state_dir`, env-driven `AGENT_HANDOFF_STATE_DIR`, consumer-root default `<consumer-root>/.task-state/`, linked-worktree convergence, and the non-git-repo fail path (`ConsumerRootResolutionError` raised with expected message substrings naming the existing env vars).
...
- New package `agentic-bootstrap` (developed in `darce/agentic-bootstrap` repo created in Slice 0; pip-installable, zero runtime deps beyond stdlib + `tomllib` (reader) + `tomlkit` (round-trip writer)) exposing:
  - `agentic-bootstrap install --target <path> [--remote-ref <tag>]`: clones (or `git worktree add`s) the remote agentic repo into `<target>/.agentic/remote/`, then symlinks `.claude/skills/`, `.github/hooks/`, `scripts/hooks/`, `.github/prompts/`, `docs/agentic/contracts/`, `.claude/commands/` from the clone into `<target>/`, writes `<target>/.agentic-overlay.json`, and invokes the three MCP-harness-config writers below in sequence.
  ...
  - `agentic-bootstrap repair` (PA-07): detects a missing, corrupt, or SHA-mismatched `<target>/.agentic/remote/` and re-clones + relinks without the user re-running `install` from scratch. Idempotent: if nothing is broken, it exits zero with "overlay healthy." If the clone exists but is at the wrong ref, fast-forwards or checks out the ref from `.agentic-overlay.json`. Does NOT touch `local/` surfaces. **Dirty-clone guard (rg-017)**: before any delete-and-re-clone path, `repair` runs `git -C <target>/.agentic/remote/ status --short --untracked-files=all`. If that returns any output, `repair` aborts non-zero with a message naming the dirty files and the `--force-dirty` opt-in flag; it does not destroy uncommitted work. `--force-dirty` is the documented opt-in for the case where the consumer knows the dirty state is recoverable (or empty).
...
- Create `packages/agent-orchestrator-mcp/docs/reworks/event-driven-daemon-design-note.md` with:
  ...
  - Anchors: `orchestrator_daemon.py:1204` (main-loop sleep; additional sleeps at 1162, 1188), `orchestrator_daemon.py:965` (`_worker_management_phase` def), `orchestrator_daemon.py:98` (`_poll_merge_ready_lanes` def; call at 991 inside `_worker_management_phase`), `worker_daemon.py:857` (`_poll_phase` def; call at 1786 inside `worker_loop`), `worker_daemon.py:143` (`poll_lane_state`), `orchestrator_daemon.py:530` (`OrchestratorLock`), `worker_daemon.py:98` (`WorkerLock`).
...
- Insert `# TODO(E17-10-REWORK): Pull-based poll — see packages/agent-orchestrator-mcp/docs/reworks/event-driven-daemon-design-note.md` comments at:
  - `orchestrator_daemon.py` immediately above the `time.sleep(poll_interval)` call at line 1204 (i.e. the TODO lands on what is currently line 1204; the sleep is pushed to line 1205 after insertion).
  - `worker_daemon.py` immediately above each `time.sleep(cfg.poll_interval)` call at lines 1783, 1790, 1796, 1806.
...
- Verify: overlay-resolver reports expected surfaces; `check-skills` + `check-harness-sync` + `lint-hoisted-paths` pass; a `configure_runtime` + `set_handoff_state` + `record_event` cycle writes to `~/Development/hoist-mvp-consumer/.task-state/handoff.db` (assert path via SQL `PRAGMA database_list;` or Python `_get_active_db_path()`) and not the monorepo DB.
...
- [ ] Slice 1: existing path surface (`AGENT_HANDOFF_WORKSPACE_ROOT`, `AGENT_HANDOFF_STATE_DIR`, `AGENT_HANDOFF_CURRENT_TASK_PATH`, `AGENT_HANDOFF_DASHBOARD_PATH`, `AGENT_HANDOFF_EXPORTS_DIR`) is documented and tested; default state remains `<main-worktree-root>/.task-state/handoff.db`; linked-worktree convergence and non-git-repo `ConsumerRootResolutionError` fail path covered.
- [ ] Slice 1: packaged `run_doctor()` no longer assumes a monorepo checkout at `Path(__file__).resolve().parents[4]`; tests cover site-packages-style execution with no sibling `packages/` tree.
...
- [ ] Slice 2: `scripts/overlay_resolver.py` returns tagged `ResolvedPath` entries across skills/hooks/commands/prompts/contracts, with the hook surface covering both `.github/hooks/` and `scripts/hooks/`.
...
- [ ] Slice 3: three MCP-harness-config writers implemented (`claude_mcp_config`, `vscode_mcp_config`, `codex_mcp_config`) with per-writer unit tests for merge semantics, byte-for-byte idempotency, and default symbolic console-script commands. The `codex_mcp_config` writer uses `tomlkit` so comments and key order in existing `.codex/config.toml` files are preserved across re-runs.
...
- [ ] Slice 3: consumer overlay includes both `.github/hooks/` and `scripts/hooks/`; generated config never references monorepo-local `scripts/mcp/mcp-server.sh`.
...
- [ ] Slice 4: `TODO(E17-10-REWORK)` comment inserted immediately above the main-loop sleep in `orchestrator_daemon.py` (currently line 1204) and above each `time.sleep(cfg.poll_interval)` in `worker_daemon.py` (lines 1783, 1790, 1796, 1806); grep-based lint test green.
